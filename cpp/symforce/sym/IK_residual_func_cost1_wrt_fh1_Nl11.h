// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl11
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl11(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 657

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _tmp0 * fv1;
  const Scalar _tmp2 = std::asinh(_tmp1);
  const Scalar _tmp3 = Scalar(1.0) * _tmp2;
  const Scalar _tmp4 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp5 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp7 = 2 * _tmp5 * _tmp6;
  const Scalar _tmp8 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp9 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = _tmp8 * _tmp9;
  const Scalar _tmp11 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp12 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp13 = -2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp14 = Scalar(0.20999999999999999) * _tmp12 +
                        Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999);
  const Scalar _tmp15 = 2 * _tmp8;
  const Scalar _tmp16 = _tmp15 * _tmp5;
  const Scalar _tmp17 = _tmp6 * _tmp9;
  const Scalar _tmp18 = _tmp16 - _tmp17;
  const Scalar _tmp19 = -Scalar(0.010999999999999999) * _tmp18;
  const Scalar _tmp20 = -_tmp14 + _tmp19;
  const Scalar _tmp21 = _tmp11 + _tmp20;
  const Scalar _tmp22 = _tmp21 + position_vector(1, 0);
  const Scalar _tmp23 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp24 = -_tmp23;
  const Scalar _tmp25 = _tmp15 * _tmp6;
  const Scalar _tmp26 = _tmp5 * _tmp9;
  const Scalar _tmp27 = _tmp25 + _tmp26;
  const Scalar _tmp28 = -Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp29;
  const Scalar _tmp31 = _tmp28 + _tmp30;
  const Scalar _tmp32 = _tmp24 + _tmp31;
  const Scalar _tmp33 = _tmp32 + position_vector(0, 0);
  const Scalar _tmp34 = Scalar(9.6622558468725703) * _tmp2;
  const Scalar _tmp35 =
      -Scalar(0.1034955) * _tmp34 * fh1 -
      Scalar(0.868210813597455) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp33), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp22 - 1), Scalar(2))));
  const Scalar _tmp36 =
      std::pow(Scalar(_tmp4 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp37 = _tmp0 * _tmp35;
  const Scalar _tmp38 = _tmp28 - _tmp30;
  const Scalar _tmp39 = _tmp23 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = -_tmp11;
  const Scalar _tmp42 = _tmp14 + _tmp19;
  const Scalar _tmp43 = _tmp41 + _tmp42;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp46 = _tmp24 + _tmp38;
  const Scalar _tmp47 = _tmp46 + position_vector(0, 0);
  const Scalar _tmp48 = _tmp47 + Scalar(1.9874742000000001);
  const Scalar _tmp49 = _tmp20 + _tmp41;
  const Scalar _tmp50 = _tmp49 + position_vector(1, 0);
  const Scalar _tmp51 = _tmp50 + Scalar(8.3196563700000006);
  const Scalar _tmp52 = std::pow(Scalar(std::pow(_tmp48, Scalar(2)) + std::pow(_tmp51, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp53 = _tmp48 * _tmp52;
  const Scalar _tmp54 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp55 = -_tmp54;
  const Scalar _tmp56 =
      -Scalar(0.010999999999999999) * _tmp12 - Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp57 = Scalar(0.20999999999999999) * _tmp25 - Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp58 = _tmp56 - _tmp57;
  const Scalar _tmp59 = _tmp55 + _tmp58;
  const Scalar _tmp60 = _tmp51 * _tmp52;
  const Scalar _tmp61 = _tmp44 + Scalar(-4.8333311099999996);
  const Scalar _tmp62 = _tmp40 + Scalar(1.79662371);
  const Scalar _tmp63 = Scalar(1.0) / (_tmp62);
  const Scalar _tmp64 = _tmp61 * _tmp63;
  const Scalar _tmp65 = _tmp54 + _tmp58;
  const Scalar _tmp66 = _tmp53 * _tmp65;
  const Scalar _tmp67 = _tmp11 + _tmp42;
  const Scalar _tmp68 = _tmp67 + position_vector(1, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(-4.7752063900000001);
  const Scalar _tmp70 = _tmp23 + _tmp31;
  const Scalar _tmp71 = _tmp70 + position_vector(0, 0);
  const Scalar _tmp72 = _tmp71 + Scalar(-2.71799795);
  const Scalar _tmp73 = std::pow(Scalar(std::pow(_tmp69, Scalar(2)) + std::pow(_tmp72, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp74 = _tmp72 * _tmp73;
  const Scalar _tmp75 = _tmp65 * _tmp74;
  const Scalar _tmp76 = _tmp56 + _tmp57;
  const Scalar _tmp77 = _tmp54 + _tmp76;
  const Scalar _tmp78 = _tmp69 * _tmp73;
  const Scalar _tmp79 = -_tmp64 * _tmp75 + _tmp77 * _tmp78;
  const Scalar _tmp80 = Scalar(1.0) / (_tmp64 * _tmp74 - _tmp78);
  const Scalar _tmp81 = _tmp53 * _tmp64 - _tmp60;
  const Scalar _tmp82 = _tmp80 * _tmp81;
  const Scalar _tmp83 = _tmp59 * _tmp60 - _tmp64 * _tmp66 - _tmp79 * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) * _tmp43;
  const Scalar _tmp85 = -_tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp67 + _tmp85);
  const Scalar _tmp87 = Scalar(1.0) * _tmp39;
  const Scalar _tmp88 = _tmp86 * (-_tmp70 + _tmp87);
  const Scalar _tmp89 = -_tmp74 * _tmp77 + _tmp75;
  const Scalar _tmp90 = -_tmp53 * _tmp59 + _tmp66 - _tmp82 * _tmp89 - _tmp83 * _tmp88;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp84 * _tmp88 + _tmp87;
  const Scalar _tmp93 = 0;
  const Scalar _tmp94 = _tmp91 * _tmp93;
  const Scalar _tmp95 =
      std::sqrt(Scalar(std::pow(_tmp61, Scalar(2)) + std::pow(_tmp62, Scalar(2))));
  const Scalar _tmp96 = _tmp63 * _tmp95;
  const Scalar _tmp97 = _tmp22 + Scalar(8.3888750099999996);
  const Scalar _tmp98 = _tmp33 + Scalar(-2.5202214700000001);
  const Scalar _tmp99 = std::pow(Scalar(std::pow(_tmp97, Scalar(2)) + std::pow(_tmp98, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp100 = _tmp97 * _tmp99;
  const Scalar _tmp101 = _tmp98 * _tmp99;
  const Scalar _tmp102 = _tmp100 * _tmp32 - _tmp101 * _tmp21;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp104 = _tmp96 * (_tmp103 * _tmp39 * _tmp61 - _tmp103 * _tmp43 * _tmp62);
  const Scalar _tmp105 = _tmp104 * _tmp74 + _tmp67 * _tmp74 - _tmp70 * _tmp78;
  const Scalar _tmp106 = _tmp104 * _tmp53 - _tmp105 * _tmp82 - _tmp46 * _tmp60 + _tmp49 * _tmp53;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = Scalar(1.0) * _tmp107;
  const Scalar _tmp109 = Scalar(1.0) * _tmp80;
  const Scalar _tmp110 = _tmp107 * _tmp109 * _tmp81;
  const Scalar _tmp111 = _tmp102 * _tmp96 * (_tmp108 * _tmp53 - _tmp110 * _tmp74);
  const Scalar _tmp112 = _tmp64 * _tmp80;
  const Scalar _tmp113 = _tmp112 * _tmp79 + _tmp64 * _tmp65;
  const Scalar _tmp114 = _tmp112 * _tmp89 - _tmp113 * _tmp88 - _tmp65;
  const Scalar _tmp115 = _tmp106 * _tmp91;
  const Scalar _tmp116 = _tmp107 * _tmp90;
  const Scalar _tmp117 = _tmp116 * (-_tmp104 + _tmp105 * _tmp112 - _tmp114 * _tmp115);
  const Scalar _tmp118 = _tmp114 + _tmp117;
  const Scalar _tmp119 = _tmp53 * _tmp91;
  const Scalar _tmp120 = _tmp81 * _tmp91;
  const Scalar _tmp121 = _tmp80 * (-_tmp118 * _tmp120 - _tmp64);
  const Scalar _tmp122 = _tmp101 * _tmp96 * (_tmp118 * _tmp119 + _tmp121 * _tmp74 + Scalar(1.0));
  const Scalar _tmp123 = _tmp109 * _tmp79;
  const Scalar _tmp124 = -_tmp109 * _tmp89 + _tmp123 * _tmp88;
  const Scalar _tmp125 = _tmp116 * (-_tmp105 * _tmp109 - _tmp115 * _tmp124);
  const Scalar _tmp126 = _tmp124 + _tmp125;
  const Scalar _tmp127 = _tmp80 * (-_tmp120 * _tmp126 + Scalar(1.0));
  const Scalar _tmp128 = _tmp100 * _tmp96 * (_tmp119 * _tmp126 + _tmp127 * _tmp74);
  const Scalar _tmp129 = -_tmp111 * fh1 - _tmp122 * fh1 - _tmp128 * fh1 -
                         _tmp45 * _tmp96 * (_tmp53 * _tmp94 - _tmp74 * _tmp82 * _tmp94);
  const Scalar _tmp130 = _tmp55 + _tmp76;
  const Scalar _tmp131 = _tmp130 * fh1;
  const Scalar _tmp132 = _tmp101 * _tmp131 + Scalar(5.1796800000000003) * _tmp27 + _tmp32 * fv1;
  const Scalar _tmp133 = _tmp49 + _tmp85;
  const Scalar _tmp134 = _tmp133 * _tmp88;
  const Scalar _tmp135 = Scalar(1.0) / (-_tmp134 - _tmp46 + _tmp87);
  const Scalar _tmp136 = Scalar(1.0) * _tmp135;
  const Scalar _tmp137 = _tmp133 * _tmp86;
  const Scalar _tmp138 = _tmp136 * _tmp137;
  const Scalar _tmp139 = -Scalar(1.0) * _tmp136 + Scalar(1.0) * _tmp138;
  const Scalar _tmp140 = _tmp135 * _tmp92;
  const Scalar _tmp141 = _tmp83 * _tmp91;
  const Scalar _tmp142 = -_tmp133 * _tmp140 - _tmp141 * _tmp93 + _tmp85;
  const Scalar _tmp143 = Scalar(1.0) * _tmp86;
  const Scalar _tmp144 = _tmp133 * _tmp135;
  const Scalar _tmp145 = _tmp113 + _tmp117 * _tmp144 - _tmp118 * _tmp141;
  const Scalar _tmp146 = Scalar(1.0) * _tmp101 * (_tmp117 * _tmp136 - _tmp143 * _tmp145);
  const Scalar _tmp147 = _tmp116 * _tmp136;
  const Scalar _tmp148 = _tmp86 * (-_tmp108 * _tmp83 + _tmp133 * _tmp147);
  const Scalar _tmp149 = Scalar(1.0) * _tmp102;
  const Scalar _tmp150 = _tmp149 * (_tmp147 - Scalar(1.0) * _tmp148);
  const Scalar _tmp151 = -_tmp123 + _tmp125 * _tmp144 - _tmp126 * _tmp141;
  const Scalar _tmp152 = Scalar(1.0) * _tmp100 * (_tmp125 * _tmp136 - _tmp143 * _tmp151);
  const Scalar _tmp153 = -_tmp100 * _tmp131 - Scalar(5.1796800000000003) * _tmp18 - _tmp21 * fv1;
  const Scalar _tmp154 = _tmp136 * _tmp88;
  const Scalar _tmp155 = _tmp134 * _tmp136 + Scalar(1.0);
  const Scalar _tmp156 = -Scalar(1.0) * _tmp143 * _tmp155 + Scalar(1.0) * _tmp154;
  const Scalar _tmp157 =
      _tmp132 * _tmp139 + _tmp146 * fh1 + _tmp150 * fh1 + _tmp152 * fh1 + _tmp153 * _tmp156 +
      Scalar(1.0) * _tmp45 * (-_tmp136 * _tmp92 - _tmp142 * _tmp143 + Scalar(1.0));
  const Scalar _tmp158 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp159 = std::asinh(_tmp157 * _tmp158);
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp161 =
      -_tmp129 * _tmp160 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp44), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp40 - 1), Scalar(2))));
  const Scalar _tmp162 = Scalar(0.1034955) * _tmp158;
  const Scalar _tmp163 = _tmp161 * _tmp162;
  const Scalar _tmp164 = Scalar(1.0) * _tmp159;
  const Scalar _tmp165 = -_tmp111 - _tmp122 - _tmp128;
  const Scalar _tmp166 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp167 = _tmp101 * _tmp130;
  const Scalar _tmp168 = _tmp100 * _tmp130;
  const Scalar _tmp169 = _tmp165 * _tmp166;
  const Scalar _tmp170 = (-_tmp157 * _tmp169 + _tmp158 * (_tmp139 * _tmp167 + _tmp146 + _tmp150 +
                                                          _tmp152 - _tmp156 * _tmp168)) /
                         std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp166 + 1));
  const Scalar _tmp171 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp172 = _tmp101 * _tmp121;
  const Scalar _tmp173 = _tmp45 * _tmp94;
  const Scalar _tmp174 = _tmp102 * _tmp110;
  const Scalar _tmp175 = _tmp100 * _tmp127;
  const Scalar _tmp176 = _tmp172 * fh1 - _tmp173 * _tmp82 - _tmp174 * fh1 + _tmp175 * fh1;
  const Scalar _tmp177 = Scalar(1.0) / (_tmp176);
  const Scalar _tmp178 = _tmp101 * _tmp145 * _tmp86;
  const Scalar _tmp179 = _tmp102 * _tmp148;
  const Scalar _tmp180 = _tmp132 * _tmp136;
  const Scalar _tmp181 = _tmp100 * _tmp151 * _tmp86;
  const Scalar _tmp182 = _tmp155 * _tmp86;
  const Scalar _tmp183 = -_tmp137 * _tmp180 + _tmp142 * _tmp45 * _tmp86 + _tmp153 * _tmp182 +
                         _tmp178 * fh1 + _tmp179 * fh1 + _tmp181 * fh1;
  const Scalar _tmp184 = std::asinh(_tmp177 * _tmp183);
  const Scalar _tmp185 = _tmp172 - _tmp174 + _tmp175;
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp185;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp176;
  const Scalar _tmp188 = std::pow(_tmp176, Scalar(-2));
  const Scalar _tmp189 = _tmp185 * _tmp188;
  const Scalar _tmp190 =
      (_tmp177 * (-_tmp138 * _tmp167 - _tmp168 * _tmp182 + _tmp178 + _tmp179 + _tmp181) -
       _tmp183 * _tmp189) /
      std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp188 + 1));
  const Scalar _tmp191 = Scalar(0.1034955) * _tmp177;
  const Scalar _tmp192 =
      -_tmp184 * _tmp187 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp68), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp71), Scalar(2))));
  const Scalar _tmp193 = _tmp191 * _tmp192;
  const Scalar _tmp194 = Scalar(1.0) * _tmp184;
  const Scalar _tmp195 = _tmp101 * _tmp118 * _tmp91;
  const Scalar _tmp196 = _tmp100 * _tmp126 * _tmp91;
  const Scalar _tmp197 = _tmp107 * _tmp149;
  const Scalar _tmp198 = _tmp173 + _tmp195 * fh1 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp199 = Scalar(1.0) / (_tmp198);
  const Scalar _tmp200 = _tmp101 * _tmp117 * _tmp135;
  const Scalar _tmp201 = _tmp100 * _tmp125 * _tmp135;
  const Scalar _tmp202 = _tmp102 * _tmp147;
  const Scalar _tmp203 = _tmp140 * _tmp45 - _tmp153 * _tmp154 + _tmp180 - _tmp200 * fh1 -
                         _tmp201 * fh1 - _tmp202 * fh1;
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(1.0) * _tmp204;
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp198;
  const Scalar _tmp207 =
      -_tmp204 * _tmp206 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp47 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp50 - 1), Scalar(2))));
  const Scalar _tmp208 = Scalar(0.1034955) * _tmp199;
  const Scalar _tmp209 = _tmp207 * _tmp208;
  const Scalar _tmp210 = _tmp195 + _tmp196 + _tmp197;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp210;
  const Scalar _tmp212 = std::pow(_tmp198, Scalar(-2));
  const Scalar _tmp213 = _tmp210 * _tmp212;
  const Scalar _tmp214 =
      (_tmp199 * (_tmp136 * _tmp167 + _tmp154 * _tmp168 - _tmp200 - _tmp201 - _tmp202) -
       _tmp203 * _tmp213) /
      std::sqrt(Scalar(std::pow(_tmp203, Scalar(2)) * _tmp212 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -Scalar(8.4693136200000012) * _tmp0 -
      Scalar(9.6622558468725703) * fh1 *
          (-Scalar(1.0) * _tmp36 * _tmp4 * fv1 * std::sinh(_tmp3) -
           Scalar(0.87653584775870996) * _tmp4 -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp1 * _tmp36 - _tmp34) -
            _tmp35 * _tmp4) *
               std::sinh(_tmp37)) -
      Scalar(9.6622558468725703) * std::cosh(_tmp3) +
      Scalar(9.6622558468725703) * std::cosh(_tmp37);
  _res(1, 0) =
      -Scalar(9.6622558468725703) * _tmp165 *
          (Scalar(0.86625939559540499) * _tmp158 - std::cosh(_tmp163) + std::cosh(_tmp164)) -
      _tmp171 *
          (-Scalar(0.86625939559540499) * _tmp169 + Scalar(1.0) * _tmp170 * std::sinh(_tmp164) -
           (-Scalar(0.1034955) * _tmp161 * _tmp169 +
            _tmp162 * (-_tmp160 * _tmp165 - _tmp170 * _tmp171)) *
               std::sinh(_tmp163));
  _res(2, 0) =
      -_tmp186 * (Scalar(0.86565325453551001) * _tmp177 - std::cosh(_tmp193) + std::cosh(_tmp194)) -
      _tmp187 *
          (-Scalar(0.86565325453551001) * _tmp189 + Scalar(1.0) * _tmp190 * std::sinh(_tmp194) -
           (-Scalar(0.1034955) * _tmp189 * _tmp192 +
            _tmp191 * (-_tmp184 * _tmp186 - _tmp187 * _tmp190)) *
               std::sinh(_tmp193));
  _res(3, 0) =
      -_tmp206 *
          (-Scalar(0.87679799772039002) * _tmp213 + Scalar(1.0) * _tmp214 * std::sinh(_tmp205) -
           (-Scalar(0.1034955) * _tmp207 * _tmp213 +
            _tmp208 * (-_tmp204 * _tmp211 - _tmp206 * _tmp214)) *
               std::sinh(_tmp209)) -
      _tmp211 * (Scalar(0.87679799772039002) * _tmp199 + std::cosh(_tmp205) - std::cosh(_tmp209));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
