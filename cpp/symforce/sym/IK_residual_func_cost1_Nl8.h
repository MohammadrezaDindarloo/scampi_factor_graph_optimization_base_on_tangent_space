// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_Nl8
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1Nl8(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 501

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (154)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp5 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp7 = -_tmp6;
  const Scalar _tmp8 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp9 = 2 * _tmp4;
  const Scalar _tmp10 = _tmp8 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp2;
  const Scalar _tmp13 = _tmp10 - _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp2 * _tmp8;
  const Scalar _tmp16 = _tmp11 * _tmp4;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp7;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp5;
  const Scalar _tmp23 = _tmp2 * _tmp9;
  const Scalar _tmp24 = _tmp11 * _tmp8;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp32 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp33 =
      -Scalar(0.010999999999999999) * _tmp21 - Scalar(0.010999999999999999) * _tmp3;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp35 = _tmp33 - _tmp34;
  const Scalar _tmp36 = _tmp32 + _tmp35;
  const Scalar _tmp37 = _tmp18 + _tmp6;
  const Scalar _tmp38 = Scalar(1.0) * _tmp37;
  const Scalar _tmp39 = -_tmp38;
  const Scalar _tmp40 = _tmp14 - _tmp17;
  const Scalar _tmp41 = _tmp40 + _tmp6;
  const Scalar _tmp42 = _tmp39 + _tmp41;
  const Scalar _tmp43 = _tmp40 + _tmp7;
  const Scalar _tmp44 = Scalar(1.0) / (_tmp39 + _tmp43);
  const Scalar _tmp45 = -_tmp22;
  const Scalar _tmp46 = _tmp28 + _tmp45;
  const Scalar _tmp47 = _tmp26 + _tmp27;
  const Scalar _tmp48 = _tmp22 + _tmp47;
  const Scalar _tmp49 = Scalar(1.0) * _tmp48;
  const Scalar _tmp50 = -_tmp46 + _tmp49;
  const Scalar _tmp51 = _tmp44 * _tmp50;
  const Scalar _tmp52 = _tmp42 * _tmp51;
  const Scalar _tmp53 = _tmp45 + _tmp47;
  const Scalar _tmp54 = Scalar(1.0) / (_tmp49 - _tmp52 - _tmp53);
  const Scalar _tmp55 = Scalar(1.0) * _tmp54;
  const Scalar _tmp56 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp57 = _tmp56 + Scalar(1.79662371);
  const Scalar _tmp58 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp59 = _tmp58 + Scalar(-4.8333311099999996);
  const Scalar _tmp60 = std::pow(Scalar(std::pow(_tmp57, Scalar(2)) + std::pow(_tmp59, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp61 = _tmp57 * _tmp60;
  const Scalar _tmp62 = _tmp37 + position_vector(1, 0);
  const Scalar _tmp63 = _tmp62 + Scalar(-4.7752063900000001);
  const Scalar _tmp64 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp65 = _tmp64 + Scalar(-2.71799795);
  const Scalar _tmp66 =
      std::sqrt(Scalar(std::pow(_tmp63, Scalar(2)) + std::pow(_tmp65, Scalar(2))));
  const Scalar _tmp67 = Scalar(1.0) / (_tmp66);
  const Scalar _tmp68 = Scalar(1.0) / (_tmp65);
  const Scalar _tmp69 = _tmp66 * _tmp68;
  const Scalar _tmp70 = _tmp69 * (-_tmp37 * _tmp65 * _tmp67 + _tmp48 * _tmp63 * _tmp67);
  const Scalar _tmp71 = _tmp59 * _tmp60;
  const Scalar _tmp72 = _tmp63 * _tmp68;
  const Scalar _tmp73 = _tmp61 * _tmp72 - _tmp71;
  const Scalar _tmp74 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp75 = _tmp74 + Scalar(8.3196563700000006);
  const Scalar _tmp76 = _tmp46 + position_vector(0, 0);
  const Scalar _tmp77 = _tmp76 + Scalar(1.9874742000000001);
  const Scalar _tmp78 = std::pow(Scalar(std::pow(_tmp75, Scalar(2)) + std::pow(_tmp77, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp79 = _tmp75 * _tmp78;
  const Scalar _tmp80 = _tmp77 * _tmp78;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp72 * _tmp80 - _tmp79);
  const Scalar _tmp82 = _tmp81 * (_tmp43 * _tmp80 - _tmp46 * _tmp79 + _tmp70 * _tmp80);
  const Scalar _tmp83 = _tmp41 * _tmp61 - _tmp53 * _tmp71 + _tmp61 * _tmp70 - _tmp73 * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) / (_tmp83);
  const Scalar _tmp85 = -_tmp32;
  const Scalar _tmp86 = _tmp35 + _tmp85;
  const Scalar _tmp87 = _tmp33 + _tmp34;
  const Scalar _tmp88 = _tmp32 + _tmp87;
  const Scalar _tmp89 = _tmp80 * _tmp88;
  const Scalar _tmp90 = -_tmp80 * _tmp86 + _tmp89;
  const Scalar _tmp91 = _tmp73 * _tmp81;
  const Scalar _tmp92 = _tmp85 + _tmp87;
  const Scalar _tmp93 = _tmp61 * _tmp88;
  const Scalar _tmp94 = -_tmp72 * _tmp89 + _tmp79 * _tmp86;
  const Scalar _tmp95 = _tmp71 * _tmp92 - _tmp72 * _tmp93 - _tmp91 * _tmp94;
  const Scalar _tmp96 = -_tmp51 * _tmp95 - _tmp61 * _tmp92 - _tmp90 * _tmp91 + _tmp93;
  const Scalar _tmp97 = _tmp84 * _tmp96;
  const Scalar _tmp98 = _tmp55 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) * _tmp84;
  const Scalar _tmp100 = _tmp42 * _tmp98 - _tmp95 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) * _tmp44;
  const Scalar _tmp102 = _tmp20 + Scalar(8.3888750099999996);
  const Scalar _tmp103 = _tmp30 + Scalar(-2.5202214700000001);
  const Scalar _tmp104 =
      std::pow(Scalar(std::pow(_tmp102, Scalar(2)) + std::pow(_tmp103, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp105 = _tmp102 * _tmp104;
  const Scalar _tmp106 = _tmp103 * _tmp104;
  const Scalar _tmp107 = fh1 * (_tmp105 * _tmp29 - _tmp106 * _tmp19);
  const Scalar _tmp108 = _tmp72 * _tmp81;
  const Scalar _tmp109 = _tmp108 * _tmp94 + _tmp72 * _tmp88;
  const Scalar _tmp110 = _tmp108 * _tmp90 - _tmp109 * _tmp51 - _tmp88;
  const Scalar _tmp111 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp112 = _tmp111 * _tmp83;
  const Scalar _tmp113 = _tmp97 * (-_tmp110 * _tmp112 - _tmp70 + _tmp72 * _tmp82);
  const Scalar _tmp114 = _tmp42 * _tmp54;
  const Scalar _tmp115 = _tmp111 * (_tmp110 + _tmp113);
  const Scalar _tmp116 = _tmp109 + _tmp113 * _tmp114 - _tmp115 * _tmp95;
  const Scalar _tmp117 = _tmp106 * fh1;
  const Scalar _tmp118 = _tmp42 * _tmp44;
  const Scalar _tmp119 = _tmp36 * fh1;
  const Scalar _tmp120 = _tmp106 * _tmp119 + Scalar(5.1796800000000003) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp121 = _tmp52 * _tmp55 + Scalar(1.0);
  const Scalar _tmp122 = _tmp51 * _tmp55;
  const Scalar _tmp123 = -_tmp105 * _tmp119 - Scalar(5.1796800000000003) * _tmp13 - _tmp19 * fv1;
  const Scalar _tmp124 = Scalar(1.0) * _tmp81;
  const Scalar _tmp125 = _tmp101 * _tmp50 * _tmp81 * _tmp94 - _tmp124 * _tmp90;
  const Scalar _tmp126 = _tmp97 * (-_tmp112 * _tmp125 - Scalar(1.0) * _tmp82);
  const Scalar _tmp127 = _tmp111 * (_tmp125 + _tmp126);
  const Scalar _tmp128 = _tmp114 * _tmp126 - _tmp124 * _tmp94 - _tmp127 * _tmp95;
  const Scalar _tmp129 = _tmp105 * fh1;
  const Scalar _tmp130 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp131 = _tmp38 * _tmp51 + _tmp49;
  const Scalar _tmp132 = 0;
  const Scalar _tmp133 = _tmp131 * _tmp54;
  const Scalar _tmp134 = -_tmp132 * _tmp95 - _tmp133 * _tmp42 + _tmp39;
  const Scalar _tmp135 = -_tmp127 * _tmp73 + Scalar(1.0);
  const Scalar _tmp136 = _tmp80 * _tmp81;
  const Scalar _tmp137 = -_tmp115 * _tmp73 - _tmp72;
  const Scalar _tmp138 = _tmp80 * _tmp91;
  const Scalar _tmp139 = -_tmp107 * _tmp69 * (-_tmp138 * _tmp99 + _tmp61 * _tmp99) -
                         _tmp117 * _tmp69 * (_tmp115 * _tmp61 + _tmp136 * _tmp137 + Scalar(1.0)) -
                         _tmp129 * _tmp69 * (_tmp127 * _tmp61 + _tmp135 * _tmp136) -
                         _tmp130 * _tmp69 * (-_tmp132 * _tmp138 + _tmp132 * _tmp61);
  const Scalar _tmp140 = Scalar(1.0) / (_tmp139);
  const Scalar _tmp141 = std::asinh(
      _tmp140 * (Scalar(1.0) * _tmp107 * (-_tmp100 * _tmp101 + _tmp98) +
                 Scalar(1.0) * _tmp117 * (-_tmp101 * _tmp116 + _tmp113 * _tmp55) +
                 Scalar(1.0) * _tmp120 * (_tmp118 * _tmp55 - _tmp55) +
                 Scalar(1.0) * _tmp123 * (-_tmp101 * _tmp121 + _tmp122) +
                 Scalar(1.0) * _tmp129 * (-_tmp101 * _tmp128 + _tmp126 * _tmp55) +
                 Scalar(1.0) * _tmp130 * (-_tmp101 * _tmp134 - _tmp131 * _tmp55 + Scalar(1.0))));
  const Scalar _tmp142 = Scalar(9.6622558468725703) * _tmp139;
  const Scalar _tmp143 = _tmp107 * _tmp99;
  const Scalar _tmp144 = _tmp130 * _tmp132;
  const Scalar _tmp145 =
      _tmp117 * _tmp137 * _tmp81 + _tmp129 * _tmp135 * _tmp81 - _tmp143 * _tmp91 - _tmp144 * _tmp91;
  const Scalar _tmp146 = Scalar(1.0) / (_tmp145);
  const Scalar _tmp147 = _tmp120 * _tmp55;
  const Scalar _tmp148 =
      std::asinh(_tmp146 * (_tmp100 * _tmp107 * _tmp44 + _tmp116 * _tmp117 * _tmp44 -
                            _tmp118 * _tmp147 + _tmp121 * _tmp123 * _tmp44 +
                            _tmp128 * _tmp129 * _tmp44 + _tmp130 * _tmp134 * _tmp44));
  const Scalar _tmp149 = Scalar(9.6622558468725703) * _tmp145;
  const Scalar _tmp150 = _tmp115 * _tmp117 + _tmp127 * _tmp129 + _tmp143 + _tmp144;
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 =
      std::asinh(_tmp151 * (-_tmp107 * _tmp98 - _tmp113 * _tmp117 * _tmp54 - _tmp122 * _tmp123 -
                            _tmp126 * _tmp129 * _tmp54 + _tmp130 * _tmp133 + _tmp147));
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp150;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp31 *
          (Scalar(0.87653584775870996) * _tmp0 + std::cosh(Scalar(1.0) * _tmp1) -
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp31 -
                Scalar(8.3888750099999996) *
                    std::sqrt(Scalar(
                        Scalar(0.090254729040973036) *
                            std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp30), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp20 - 1),
                                 Scalar(2))))))) +
      _tmp36 + position_vector(2, 0);
  _res(1, 0) =
      -_tmp142 *
          (Scalar(0.86565325453551001) * _tmp140 + std::cosh(Scalar(1.0) * _tmp141) -
           std::cosh(
               Scalar(0.1034955) * _tmp140 *
               (-_tmp141 * _tmp142 -
                Scalar(4.7752063900000001) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp62), Scalar(2)) +
                        Scalar(0.32397683292140877) *
                            std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp64),
                                     Scalar(2))))))) +
      _tmp88 + position_vector(2, 0);
  _res(2, 0) =
      -_tmp149 *
          (Scalar(0.87679799772039002) * _tmp146 + std::cosh(Scalar(1.0) * _tmp148) -
           std::cosh(
               Scalar(0.1034955) * _tmp146 *
               (-_tmp148 * _tmp149 -
                Scalar(8.3196563700000006) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp74 - 1), Scalar(2)) +
                        Scalar(0.057067943376852184) *
                            std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp76 - 1),
                                     Scalar(2))))))) +
      _tmp86 + position_vector(2, 0);
  _res(3, 0) =
      -_tmp153 *
          (Scalar(0.86625939559540499) * _tmp151 + std::cosh(Scalar(1.0) * _tmp152) -
           std::cosh(
               Scalar(0.1034955) * _tmp151 *
               (-_tmp152 * _tmp153 -
                Scalar(4.8333311099999996) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp58), Scalar(2)) +
                        Scalar(0.13817235445745474) *
                            std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp56 - 1),
                                     Scalar(2))))))) +
      _tmp92 + position_vector(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
