// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl15
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl15(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 605

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (191)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp10 = 2 * _tmp6 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp4;
  const Scalar _tmp13 = _tmp10 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp4;
  const Scalar _tmp16 = _tmp15 * _tmp9;
  const Scalar _tmp17 = _tmp11 * _tmp6;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = _tmp14 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp8;
  const Scalar _tmp21 = _tmp20 + position_vector(0, 0);
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp23 = _tmp15 * _tmp6;
  const Scalar _tmp24 = _tmp11 * _tmp9;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp26;
  const Scalar _tmp28 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp28 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp30 = _tmp27 + _tmp29;
  const Scalar _tmp31 = _tmp22 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(1, 0);
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp34 = -_tmp33;
  const Scalar _tmp35 = -Scalar(0.010999999999999999) * _tmp28 -
                        Scalar(0.010999999999999999) * _tmp5 + Scalar(-0.010999999999999999);
  const Scalar _tmp36 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp37 = _tmp35 + _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = -_tmp8;
  const Scalar _tmp40 = _tmp19 + _tmp39;
  const Scalar _tmp41 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp42 = _tmp41 + Scalar(1.79662371);
  const Scalar _tmp43 = -_tmp22;
  const Scalar _tmp44 = _tmp30 + _tmp43;
  const Scalar _tmp45 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(-4.8333311099999996);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp42, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp42 * _tmp47;
  const Scalar _tmp49 = _tmp35 - _tmp36;
  const Scalar _tmp50 = _tmp33 + _tmp49;
  const Scalar _tmp51 = -_tmp38 * _tmp48 + _tmp48 * _tmp50;
  const Scalar _tmp52 = _tmp46 * _tmp47;
  const Scalar _tmp53 = _tmp14 - _tmp18;
  const Scalar _tmp54 = _tmp53 + _tmp8;
  const Scalar _tmp55 = _tmp54 + position_vector(0, 0);
  const Scalar _tmp56 = _tmp55 + Scalar(-2.5202214700000001);
  const Scalar _tmp57 = Scalar(1.0) / (_tmp56);
  const Scalar _tmp58 = -_tmp29;
  const Scalar _tmp59 = _tmp22 + _tmp27 + _tmp58;
  const Scalar _tmp60 = _tmp59 + position_vector(1, 0);
  const Scalar _tmp61 = _tmp60 + Scalar(8.3888750099999996);
  const Scalar _tmp62 = _tmp57 * _tmp61;
  const Scalar _tmp63 = Scalar(1.0) / (_tmp48 * _tmp62 - _tmp52);
  const Scalar _tmp64 = _tmp62 * _tmp63;
  const Scalar _tmp65 = _tmp50 * _tmp62;
  const Scalar _tmp66 = _tmp38 * _tmp52 - _tmp48 * _tmp65;
  const Scalar _tmp67 = _tmp64 * _tmp66 + _tmp65;
  const Scalar _tmp68 = Scalar(1.0) * _tmp59;
  const Scalar _tmp69 = -_tmp68;
  const Scalar _tmp70 = Scalar(1.0) / (_tmp44 + _tmp69);
  const Scalar _tmp71 = Scalar(1.0) * _tmp54;
  const Scalar _tmp72 = _tmp70 * (-_tmp40 + _tmp71);
  const Scalar _tmp73 = -_tmp50 + _tmp51 * _tmp64 - _tmp67 * _tmp72;
  const Scalar _tmp74 = _tmp39 + _tmp53;
  const Scalar _tmp75 = _tmp74 + position_vector(0, 0);
  const Scalar _tmp76 = _tmp75 + Scalar(1.9874742000000001);
  const Scalar _tmp77 = _tmp43 + _tmp58;
  const Scalar _tmp78 = _tmp27 + _tmp77;
  const Scalar _tmp79 = _tmp78 + position_vector(1, 0);
  const Scalar _tmp80 = _tmp79 + Scalar(8.3196563700000006);
  const Scalar _tmp81 = std::pow(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp80, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp82 = _tmp76 * _tmp81;
  const Scalar _tmp83 = _tmp34 + _tmp49;
  const Scalar _tmp84 = _tmp80 * _tmp81;
  const Scalar _tmp85 = _tmp62 * _tmp82 - _tmp84;
  const Scalar _tmp86 = _tmp63 * _tmp85;
  const Scalar _tmp87 = -_tmp65 * _tmp82 - _tmp66 * _tmp86 + _tmp83 * _tmp84;
  const Scalar _tmp88 = _tmp50 * _tmp82 - _tmp51 * _tmp86 - _tmp72 * _tmp87 - _tmp82 * _tmp83;
  const Scalar _tmp89 = Scalar(1.0) / (_tmp88);
  const Scalar _tmp90 =
      std::sqrt(Scalar(std::pow(_tmp56, Scalar(2)) + std::pow(_tmp61, Scalar(2))));
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp57 * _tmp90;
  const Scalar _tmp93 = _tmp92 * (_tmp54 * _tmp61 * _tmp91 - _tmp56 * _tmp59 * _tmp91);
  const Scalar _tmp94 = -_tmp40 * _tmp52 + _tmp44 * _tmp48 + _tmp48 * _tmp93;
  const Scalar _tmp95 = -_tmp74 * _tmp84 + _tmp78 * _tmp82 + _tmp82 * _tmp93 - _tmp86 * _tmp94;
  const Scalar _tmp96 = _tmp89 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp88 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (_tmp64 * _tmp94 - _tmp73 * _tmp96 - _tmp93);
  const Scalar _tmp100 = _tmp73 + _tmp99;
  const Scalar _tmp101 = _tmp82 * _tmp89;
  const Scalar _tmp102 = _tmp85 * _tmp89;
  const Scalar _tmp103 = -_tmp100 * _tmp102 - _tmp62;
  const Scalar _tmp104 = _tmp48 * _tmp63;
  const Scalar _tmp105 = _tmp21 + Scalar(-2.71799795);
  const Scalar _tmp106 = _tmp32 + Scalar(-4.7752063900000001);
  const Scalar _tmp107 =
      std::pow(Scalar(std::pow(_tmp105, Scalar(2)) + std::pow(_tmp106, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp108 * fh1;
  const Scalar _tmp110 = Scalar(1.0) * _tmp63;
  const Scalar _tmp111 = _tmp110 * _tmp66;
  const Scalar _tmp112 = -_tmp110 * _tmp51 + _tmp111 * _tmp72;
  const Scalar _tmp113 = _tmp98 * (-_tmp110 * _tmp94 - _tmp112 * _tmp96);
  const Scalar _tmp114 = _tmp112 + _tmp113;
  const Scalar _tmp115 = -_tmp102 * _tmp114 + Scalar(1.0);
  const Scalar _tmp116 = _tmp106 * _tmp107;
  const Scalar _tmp117 = _tmp116 * fh1;
  const Scalar _tmp118 = Scalar(1.0) * _tmp97;
  const Scalar _tmp119 = _tmp48 * _tmp86;
  const Scalar _tmp120 = fh1 * (-_tmp108 * _tmp31 + _tmp116 * _tmp20);
  const Scalar _tmp121 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp122 = _tmp68 * _tmp72 + _tmp71;
  const Scalar _tmp123 = 0;
  const Scalar _tmp124 = _tmp92 * (-_tmp119 * _tmp123 + _tmp123 * _tmp82);
  const Scalar _tmp125 = -_tmp109 * _tmp92 * (_tmp100 * _tmp101 + _tmp103 * _tmp104 + Scalar(1.0)) -
                         _tmp117 * _tmp92 * (_tmp101 * _tmp114 + _tmp104 * _tmp115) -
                         _tmp120 * _tmp92 * (-_tmp118 * _tmp119 + _tmp118 * _tmp82) -
                         _tmp121 * _tmp124;
  const Scalar _tmp126 = Scalar(1.0) / (_tmp125);
  const Scalar _tmp127 = _tmp69 + _tmp78;
  const Scalar _tmp128 = _tmp127 * _tmp72;
  const Scalar _tmp129 = Scalar(1.0) / (-_tmp128 + _tmp71 - _tmp74);
  const Scalar _tmp130 = _tmp122 * _tmp129;
  const Scalar _tmp131 = _tmp70 * (-_tmp123 * _tmp87 - _tmp127 * _tmp130 + _tmp69);
  const Scalar _tmp132 = -Scalar(1.0) * _tmp130 - Scalar(1.0) * _tmp131 + Scalar(1.0);
  const Scalar _tmp133 = Scalar(1.0) * _tmp129;
  const Scalar _tmp134 = _tmp133 * _tmp98;
  const Scalar _tmp135 = -_tmp118 * _tmp87 + _tmp127 * _tmp134;
  const Scalar _tmp136 = Scalar(1.0) * _tmp70;
  const Scalar _tmp137 = _tmp87 * _tmp89;
  const Scalar _tmp138 = _tmp127 * _tmp129;
  const Scalar _tmp139 = -_tmp100 * _tmp137 + _tmp138 * _tmp99 + _tmp67;
  const Scalar _tmp140 = -_tmp111 + _tmp113 * _tmp138 - _tmp114 * _tmp137;
  const Scalar _tmp141 = fh1 * (_tmp33 + _tmp37);
  const Scalar _tmp142 = -_tmp116 * _tmp141 - Scalar(5.1796800000000003) * _tmp25 - _tmp31 * fv1;
  const Scalar _tmp143 = _tmp70 * (_tmp128 * _tmp133 + Scalar(1.0));
  const Scalar _tmp144 = _tmp133 * _tmp72;
  const Scalar _tmp145 = -Scalar(1.0) * _tmp143 + Scalar(1.0) * _tmp144;
  const Scalar _tmp146 = _tmp108 * _tmp141 + Scalar(5.1796800000000003) * _tmp13 + _tmp20 * fv1;
  const Scalar _tmp147 = _tmp127 * _tmp70;
  const Scalar _tmp148 = Scalar(1.0) * _tmp133 * _tmp147 - Scalar(1.0) * _tmp133;
  const Scalar _tmp149 = Scalar(1.0) * _tmp109 * (_tmp133 * _tmp99 - _tmp136 * _tmp139) +
                         Scalar(1.0) * _tmp117 * (_tmp113 * _tmp133 - _tmp136 * _tmp140) +
                         Scalar(1.0) * _tmp120 * (_tmp134 - _tmp135 * _tmp136) + _tmp121 * _tmp132 +
                         _tmp142 * _tmp145 + _tmp146 * _tmp148;
  const Scalar _tmp150 = std::asinh(_tmp126 * _tmp149);
  const Scalar _tmp151 = Scalar(1.0) * _tmp150;
  const Scalar _tmp152 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp153 =
      -_tmp150 * _tmp152 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp55), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp60 - 1), Scalar(2))));
  const Scalar _tmp154 = Scalar(0.1034955) * _tmp126;
  const Scalar _tmp155 = _tmp153 * _tmp154;
  const Scalar _tmp156 = Scalar(9.6622558468725703) * _tmp124;
  const Scalar _tmp157 = _tmp26 + _tmp77;
  const Scalar _tmp158 = std::pow(_tmp125, Scalar(-2));
  const Scalar _tmp159 = _tmp124 * _tmp158;
  const Scalar _tmp160 =
      (_tmp126 * (-_tmp132 + _tmp145 * _tmp157 + _tmp148 * _tmp20) - _tmp149 * _tmp159) /
      std::sqrt(Scalar(std::pow(_tmp149, Scalar(2)) * _tmp158 + 1));
  const Scalar _tmp161 = _tmp133 * _tmp146;
  const Scalar _tmp162 = _tmp109 * _tmp139 * _tmp70 + _tmp117 * _tmp140 * _tmp70 +
                         _tmp120 * _tmp135 * _tmp70 + _tmp121 * _tmp131 + _tmp142 * _tmp143 -
                         _tmp147 * _tmp161;
  const Scalar _tmp163 = _tmp121 * _tmp123;
  const Scalar _tmp164 = _tmp118 * _tmp120;
  const Scalar _tmp165 =
      _tmp103 * _tmp109 * _tmp63 + _tmp115 * _tmp117 * _tmp63 - _tmp163 * _tmp86 - _tmp164 * _tmp86;
  const Scalar _tmp166 = Scalar(1.0) / (_tmp165);
  const Scalar _tmp167 = std::asinh(_tmp162 * _tmp166);
  const Scalar _tmp168 = Scalar(1.0) * _tmp167;
  const Scalar _tmp169 = std::pow(_tmp165, Scalar(-2));
  const Scalar _tmp170 = _tmp133 * _tmp20;
  const Scalar _tmp171 = _tmp123 * _tmp169 * _tmp86;
  const Scalar _tmp172 =
      (-_tmp162 * _tmp171 + _tmp166 * (-_tmp131 + _tmp143 * _tmp157 - _tmp147 * _tmp170)) /
      std::sqrt(Scalar(std::pow(_tmp162, Scalar(2)) * _tmp169 + 1));
  const Scalar _tmp173 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp123;
  const Scalar _tmp175 = _tmp174 * _tmp86;
  const Scalar _tmp176 = Scalar(0.1034955) * _tmp166;
  const Scalar _tmp177 =
      -_tmp167 * _tmp173 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp45), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp41 - 1), Scalar(2))));
  const Scalar _tmp178 = _tmp176 * _tmp177;
  const Scalar _tmp179 = -_tmp109 * _tmp129 * _tmp99 - _tmp113 * _tmp117 * _tmp129 -
                         _tmp120 * _tmp134 + _tmp121 * _tmp130 - _tmp142 * _tmp144 + _tmp161;
  const Scalar _tmp180 =
      _tmp100 * _tmp109 * _tmp89 + _tmp114 * _tmp117 * _tmp89 + _tmp163 + _tmp164;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp180);
  const Scalar _tmp182 = std::asinh(_tmp179 * _tmp181);
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp184 =
      -_tmp182 * _tmp183 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp75 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp79 - 1), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = Scalar(1.0) * _tmp182;
  const Scalar _tmp188 = std::pow(_tmp180, Scalar(-2));
  const Scalar _tmp189 = _tmp123 * _tmp188;
  const Scalar _tmp190 = (_tmp179 * _tmp189 + _tmp181 * (-_tmp130 - _tmp144 * _tmp157 + _tmp170)) /
                         std::sqrt(Scalar(std::pow(_tmp179, Scalar(2)) * _tmp188 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp3 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 *
           std::sinh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp3 -
                Scalar(4.7752063900000001) *
                    std::sqrt(Scalar(
                        Scalar(0.32397683292140877) *
                            std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp21), Scalar(2)) +
                        std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp32), Scalar(2)))))));
  _res(1, 0) =
      -_tmp152 *
          (-Scalar(0.87653584775870996) * _tmp159 + Scalar(1.0) * _tmp160 * std::sinh(_tmp151) -
           (-Scalar(0.1034955) * _tmp153 * _tmp159 +
            _tmp154 * (-_tmp150 * _tmp156 - _tmp152 * _tmp160)) *
               std::sinh(_tmp155)) -
      _tmp156 * (Scalar(0.87653584775870996) * _tmp126 + std::cosh(_tmp151) - std::cosh(_tmp155));
  _res(2, 0) =
      -_tmp173 *
          (-Scalar(0.86625939559540499) * _tmp171 + Scalar(1.0) * _tmp172 * std::sinh(_tmp168) -
           (-Scalar(0.1034955) * _tmp171 * _tmp177 +
            _tmp176 * (-_tmp167 * _tmp175 - _tmp172 * _tmp173)) *
               std::sinh(_tmp178)) -
      _tmp175 * (Scalar(0.86625939559540499) * _tmp166 + std::cosh(_tmp168) - std::cosh(_tmp178));
  _res(3, 0) =
      _tmp174 * (Scalar(0.87679799772039002) * _tmp181 - std::cosh(_tmp186) + std::cosh(_tmp187)) -
      _tmp183 *
          (Scalar(0.87679799772039002) * _tmp189 + Scalar(1.0) * _tmp190 * std::sinh(_tmp187) -
           (Scalar(0.1034955) * _tmp184 * _tmp189 +
            _tmp185 * (_tmp174 * _tmp182 - _tmp183 * _tmp190)) *
               std::sinh(_tmp186));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
