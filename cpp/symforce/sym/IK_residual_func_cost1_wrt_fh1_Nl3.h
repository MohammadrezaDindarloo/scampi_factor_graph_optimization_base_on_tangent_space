// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl3
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl3(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 656

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (212)
  const Scalar _tmp0 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp1;
  const Scalar _tmp9 = _tmp7 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp13 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp14 = _tmp1 * _tmp10;
  const Scalar _tmp15 = _tmp13 - _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = _tmp3 * _tmp8;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = Scalar(1.0) / (fh1);
  const Scalar _tmp32 = _tmp31 * fv1;
  const Scalar _tmp33 = std::asinh(_tmp32);
  const Scalar _tmp34 = Scalar(9.6622558468725703) * _tmp33;
  const Scalar _tmp35 =
      -_tmp34 * fh1 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp19 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp30 - 1), Scalar(2))));
  const Scalar _tmp36 = Scalar(0.1034955) * _tmp31;
  const Scalar _tmp37 = _tmp35 * _tmp36;
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp0 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(1.0) * _tmp33;
  const Scalar _tmp40 = _tmp12 + _tmp16;
  const Scalar _tmp41 = _tmp40 + _tmp6;
  const Scalar _tmp42 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(8.3888750099999996);
  const Scalar _tmp44 = _tmp21 + _tmp28;
  const Scalar _tmp45 = _tmp44 + position_vector(0, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(-2.5202214700000001);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp43 * _tmp47;
  const Scalar _tmp49 = _tmp40 + _tmp5;
  const Scalar _tmp50 = _tmp49 + position_vector(1, 0);
  const Scalar _tmp51 = _tmp50 + Scalar(-4.7752063900000001);
  const Scalar _tmp52 = _tmp26 + _tmp27;
  const Scalar _tmp53 = _tmp21 + _tmp52;
  const Scalar _tmp54 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(-2.71799795);
  const Scalar _tmp56 = Scalar(1.0) / (_tmp55);
  const Scalar _tmp57 = _tmp51 * _tmp56;
  const Scalar _tmp58 = _tmp46 * _tmp47;
  const Scalar _tmp59 = -_tmp48 + _tmp57 * _tmp58;
  const Scalar _tmp60 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp61 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp62 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp63 = _tmp61 + _tmp62;
  const Scalar _tmp64 = _tmp60 + _tmp63;
  const Scalar _tmp65 = _tmp57 * _tmp64;
  const Scalar _tmp66 = -_tmp60;
  const Scalar _tmp67 = _tmp63 + _tmp66;
  const Scalar _tmp68 = _tmp61 - _tmp62;
  const Scalar _tmp69 = _tmp60 + _tmp68;
  const Scalar _tmp70 = _tmp17 + _tmp5;
  const Scalar _tmp71 = _tmp70 + position_vector(1, 0);
  const Scalar _tmp72 = _tmp71 + Scalar(-4.8333311099999996);
  const Scalar _tmp73 = _tmp22 + _tmp52;
  const Scalar _tmp74 = _tmp73 + position_vector(0, 0);
  const Scalar _tmp75 = _tmp74 + Scalar(1.79662371);
  const Scalar _tmp76 = std::pow(Scalar(std::pow(_tmp72, Scalar(2)) + std::pow(_tmp75, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp77 = _tmp72 * _tmp76;
  const Scalar _tmp78 = _tmp75 * _tmp76;
  const Scalar _tmp79 = -_tmp65 * _tmp78 + _tmp69 * _tmp77;
  const Scalar _tmp80 = Scalar(1.0) / (_tmp57 * _tmp78 - _tmp77);
  const Scalar _tmp81 = _tmp59 * _tmp80;
  const Scalar _tmp82 = _tmp48 * _tmp67 - _tmp58 * _tmp65 - _tmp79 * _tmp81;
  const Scalar _tmp83 = Scalar(1.0) * _tmp49;
  const Scalar _tmp84 = -_tmp83;
  const Scalar _tmp85 = Scalar(1.0) / (_tmp70 + _tmp84);
  const Scalar _tmp86 = Scalar(1.0) * _tmp53;
  const Scalar _tmp87 = -_tmp73 + _tmp86;
  const Scalar _tmp88 = _tmp85 * _tmp87;
  const Scalar _tmp89 = _tmp64 * _tmp78 - _tmp69 * _tmp78;
  const Scalar _tmp90 = _tmp58 * _tmp64 - _tmp58 * _tmp67 - _tmp81 * _tmp89 - _tmp82 * _tmp88;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = Scalar(1.0) * _tmp85;
  const Scalar _tmp93 = Scalar(1.0) * _tmp80;
  const Scalar _tmp94 = _tmp79 * _tmp80 * _tmp87 * _tmp92 - _tmp89 * _tmp93;
  const Scalar _tmp95 =
      std::sqrt(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp55, Scalar(2))));
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp56 * _tmp95;
  const Scalar _tmp98 = _tmp97 * (-_tmp49 * _tmp55 * _tmp96 + _tmp51 * _tmp53 * _tmp96);
  const Scalar _tmp99 = _tmp70 * _tmp78 - _tmp73 * _tmp77 + _tmp78 * _tmp98;
  const Scalar _tmp100 = _tmp41 * _tmp58 - _tmp44 * _tmp48 + _tmp58 * _tmp98 - _tmp81 * _tmp99;
  const Scalar _tmp101 = _tmp100 * _tmp91;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp90;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp94 - _tmp93 * _tmp99);
  const Scalar _tmp105 = _tmp91 * (_tmp104 + _tmp94);
  const Scalar _tmp106 = -_tmp105 * _tmp59 + Scalar(1.0);
  const Scalar _tmp107 = _tmp78 * _tmp80;
  const Scalar _tmp108 = _tmp19 + Scalar(8.3196563700000006);
  const Scalar _tmp109 = _tmp30 + Scalar(1.9874742000000001);
  const Scalar _tmp110 =
      std::pow(Scalar(std::pow(_tmp108, Scalar(2)) + std::pow(_tmp109, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp111 = _tmp108 * _tmp110;
  const Scalar _tmp112 = _tmp111 * _tmp97 * (_tmp105 * _tmp58 + _tmp106 * _tmp107);
  const Scalar _tmp113 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp114 = _tmp83 * _tmp88 + _tmp86;
  const Scalar _tmp115 = 0;
  const Scalar _tmp116 = _tmp57 * _tmp80;
  const Scalar _tmp117 = _tmp116 * _tmp79 + _tmp65;
  const Scalar _tmp118 = _tmp116 * _tmp89 - _tmp117 * _tmp88 - _tmp64;
  const Scalar _tmp119 = _tmp103 * (-_tmp101 * _tmp118 + _tmp116 * _tmp99 - _tmp98);
  const Scalar _tmp120 = _tmp91 * (_tmp118 + _tmp119);
  const Scalar _tmp121 = -_tmp120 * _tmp59 - _tmp57;
  const Scalar _tmp122 = _tmp109 * _tmp110;
  const Scalar _tmp123 = _tmp122 * _tmp97 * (_tmp107 * _tmp121 + _tmp120 * _tmp58 + Scalar(1.0));
  const Scalar _tmp124 = _tmp111 * _tmp29 - _tmp122 * _tmp18;
  const Scalar _tmp125 = Scalar(1.0) * _tmp102;
  const Scalar _tmp126 = _tmp102 * _tmp59 * _tmp93;
  const Scalar _tmp127 = _tmp124 * _tmp97 * (_tmp125 * _tmp58 - _tmp126 * _tmp78);
  const Scalar _tmp128 = -_tmp112 * fh1 -
                         _tmp113 * _tmp97 * (_tmp115 * _tmp58 - _tmp115 * _tmp78 * _tmp81) -
                         _tmp123 * fh1 - _tmp127 * fh1;
  const Scalar _tmp129 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp130 = -_tmp112 - _tmp123 - _tmp127;
  const Scalar _tmp131 = _tmp129 * _tmp130;
  const Scalar _tmp132 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp133 = _tmp41 + _tmp84;
  const Scalar _tmp134 = _tmp133 * _tmp88;
  const Scalar _tmp135 = Scalar(1.0) / (-_tmp134 - _tmp44 + _tmp86);
  const Scalar _tmp136 = Scalar(1.0) * _tmp135;
  const Scalar _tmp137 = _tmp133 * _tmp135;
  const Scalar _tmp138 = _tmp104 * _tmp137 - _tmp105 * _tmp82 - _tmp79 * _tmp93;
  const Scalar _tmp139 = Scalar(1.0) * _tmp111 * (_tmp104 * _tmp136 - _tmp138 * _tmp92);
  const Scalar _tmp140 = _tmp117 + _tmp119 * _tmp137 - _tmp120 * _tmp82;
  const Scalar _tmp141 = Scalar(1.0) * _tmp122 * (_tmp119 * _tmp136 - _tmp140 * _tmp92);
  const Scalar _tmp142 = _tmp66 + _tmp68;
  const Scalar _tmp143 = _tmp142 * fh1;
  const Scalar _tmp144 = -_tmp111 * _tmp143 - Scalar(5.1796800000000003) * _tmp15 - _tmp18 * fv1;
  const Scalar _tmp145 = _tmp136 * _tmp88;
  const Scalar _tmp146 = _tmp134 * _tmp136 + Scalar(1.0);
  const Scalar _tmp147 = Scalar(1.0) * _tmp145 - Scalar(1.0) * _tmp146 * _tmp92;
  const Scalar _tmp148 = _tmp103 * _tmp136;
  const Scalar _tmp149 = -_tmp125 * _tmp82 + _tmp133 * _tmp148;
  const Scalar _tmp150 = Scalar(1.0) * _tmp124;
  const Scalar _tmp151 = _tmp150 * (_tmp148 - _tmp149 * _tmp92);
  const Scalar _tmp152 = _tmp122 * _tmp143 + Scalar(5.1796800000000003) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp153 = _tmp133 * _tmp85;
  const Scalar _tmp154 = _tmp136 * _tmp153;
  const Scalar _tmp155 = -Scalar(1.0) * _tmp136 + Scalar(1.0) * _tmp154;
  const Scalar _tmp156 = _tmp114 * _tmp135;
  const Scalar _tmp157 = -_tmp115 * _tmp82 - _tmp133 * _tmp156 + _tmp84;
  const Scalar _tmp158 =
      Scalar(1.0) * _tmp113 * (-_tmp114 * _tmp136 - _tmp157 * _tmp92 + Scalar(1.0)) +
      _tmp139 * fh1 + _tmp141 * fh1 + _tmp144 * _tmp147 + _tmp151 * fh1 + _tmp152 * _tmp155;
  const Scalar _tmp159 = _tmp111 * _tmp142;
  const Scalar _tmp160 = _tmp122 * _tmp142;
  const Scalar _tmp161 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp162 = (-_tmp131 * _tmp158 + _tmp161 * (_tmp139 + _tmp141 - _tmp147 * _tmp159 +
                                                          _tmp151 + _tmp155 * _tmp160)) /
                         std::sqrt(Scalar(_tmp129 * std::pow(_tmp158, Scalar(2)) + 1));
  const Scalar _tmp163 = std::asinh(_tmp158 * _tmp161);
  const Scalar _tmp164 = Scalar(9.6622558468725703) * _tmp163;
  const Scalar _tmp165 = Scalar(0.1034955) * _tmp161;
  const Scalar _tmp166 =
      -_tmp128 * _tmp164 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp50), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp54), Scalar(2))));
  const Scalar _tmp167 = _tmp165 * _tmp166;
  const Scalar _tmp168 = Scalar(1.0) * _tmp163;
  const Scalar _tmp169 = _tmp122 * _tmp140 * _tmp85;
  const Scalar _tmp170 = _tmp124 * _tmp149 * _tmp85;
  const Scalar _tmp171 = _tmp136 * _tmp152;
  const Scalar _tmp172 = _tmp111 * _tmp138 * _tmp85;
  const Scalar _tmp173 = _tmp146 * _tmp85;
  const Scalar _tmp174 = _tmp113 * _tmp157 * _tmp85 + _tmp144 * _tmp173 - _tmp153 * _tmp171 +
                         _tmp169 * fh1 + _tmp170 * fh1 + _tmp172 * fh1;
  const Scalar _tmp175 = _tmp121 * _tmp122 * _tmp80;
  const Scalar _tmp176 = _tmp113 * _tmp115;
  const Scalar _tmp177 = _tmp124 * _tmp126;
  const Scalar _tmp178 = _tmp106 * _tmp111 * _tmp80;
  const Scalar _tmp179 = _tmp175 * fh1 - _tmp176 * _tmp81 - _tmp177 * fh1 + _tmp178 * fh1;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = std::asinh(_tmp174 * _tmp180);
  const Scalar _tmp182 = Scalar(1.0) * _tmp181;
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp184 =
      -_tmp181 * _tmp183 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp71), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp74 - 1), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = _tmp175 - _tmp177 + _tmp178;
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp187;
  const Scalar _tmp189 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp190 = _tmp187 * _tmp189;
  const Scalar _tmp191 = (-_tmp174 * _tmp190 + _tmp180 * (-_tmp154 * _tmp160 - _tmp159 * _tmp173 +
                                                          _tmp169 + _tmp170 + _tmp172)) /
                         std::sqrt(Scalar(std::pow(_tmp174, Scalar(2)) * _tmp189 + 1));
  const Scalar _tmp192 = _tmp124 * _tmp148;
  const Scalar _tmp193 = _tmp104 * _tmp111 * _tmp135;
  const Scalar _tmp194 = _tmp119 * _tmp122 * _tmp135;
  const Scalar _tmp195 = _tmp113 * _tmp156 - _tmp144 * _tmp145 + _tmp171 - _tmp192 * fh1 -
                         _tmp193 * fh1 - _tmp194 * fh1;
  const Scalar _tmp196 = _tmp102 * _tmp150;
  const Scalar _tmp197 = _tmp105 * _tmp111;
  const Scalar _tmp198 = _tmp120 * _tmp122;
  const Scalar _tmp199 = _tmp176 + _tmp196 * fh1 + _tmp197 * fh1 + _tmp198 * fh1;
  const Scalar _tmp200 = Scalar(1.0) / (_tmp199);
  const Scalar _tmp201 = std::asinh(_tmp195 * _tmp200);
  const Scalar _tmp202 = Scalar(1.0) * _tmp201;
  const Scalar _tmp203 = std::pow(_tmp199, Scalar(-2));
  const Scalar _tmp204 = _tmp196 + _tmp197 + _tmp198;
  const Scalar _tmp205 = _tmp203 * _tmp204;
  const Scalar _tmp206 = (-_tmp195 * _tmp205 + _tmp200 * (_tmp136 * _tmp160 + _tmp145 * _tmp159 -
                                                          _tmp192 - _tmp193 - _tmp194)) /
                         std::sqrt(Scalar(std::pow(_tmp195, Scalar(2)) * _tmp203 + 1));
  const Scalar _tmp207 = Scalar(9.6622558468725703) * _tmp199;
  const Scalar _tmp208 =
      -_tmp201 * _tmp207 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp45), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp42 - 1), Scalar(2))));
  const Scalar _tmp209 = Scalar(0.1034955) * _tmp200;
  const Scalar _tmp210 = _tmp208 * _tmp209;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp204;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = -Scalar(8.4718465800000011) * _tmp31 -
               Scalar(9.6622558468725703) * fh1 *
                   (-Scalar(1.0) * _tmp0 * _tmp38 * fv1 * std::sinh(_tmp39) -
                    Scalar(0.87679799772039002) * _tmp0 -
                    (-Scalar(0.1034955) * _tmp0 * _tmp35 +
                     _tmp36 * (Scalar(9.6622558468725703) * _tmp32 * _tmp38 - _tmp34)) *
                        std::sinh(_tmp37)) +
               Scalar(9.6622558468725703) * std::cosh(_tmp37) -
               Scalar(9.6622558468725703) * std::cosh(_tmp39);
  _res(1, 0) =
      -Scalar(9.6622558468725703) * _tmp130 *
          (Scalar(0.86565325453551001) * _tmp161 - std::cosh(_tmp167) + std::cosh(_tmp168)) -
      _tmp132 *
          (-Scalar(0.86565325453551001) * _tmp131 + Scalar(1.0) * _tmp162 * std::sinh(_tmp168) -
           (-Scalar(0.1034955) * _tmp131 * _tmp166 +
            _tmp165 * (-_tmp130 * _tmp164 - _tmp132 * _tmp162)) *
               std::sinh(_tmp167));
  _res(2, 0) =
      -_tmp183 *
          (-Scalar(0.86625939559540499) * _tmp190 + Scalar(1.0) * _tmp191 * std::sinh(_tmp182) -
           (-Scalar(0.1034955) * _tmp184 * _tmp190 +
            _tmp185 * (-_tmp181 * _tmp188 - _tmp183 * _tmp191)) *
               std::sinh(_tmp186)) -
      _tmp188 * (Scalar(0.86625939559540499) * _tmp180 + std::cosh(_tmp182) - std::cosh(_tmp186));
  _res(3, 0) =
      -_tmp207 *
          (-Scalar(0.87653584775870996) * _tmp205 + Scalar(1.0) * _tmp206 * std::sinh(_tmp202) -
           (-Scalar(0.1034955) * _tmp205 * _tmp208 +
            _tmp209 * (-_tmp201 * _tmp211 - _tmp206 * _tmp207)) *
               std::sinh(_tmp210)) -
      _tmp211 * (Scalar(0.87653584775870996) * _tmp200 + std::cosh(_tmp202) - std::cosh(_tmp210));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
