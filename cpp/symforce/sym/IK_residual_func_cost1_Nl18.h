// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_Nl18
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1Nl18(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 505

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (156)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = 2 * _tmp3;
  const Scalar _tmp5 = _tmp2 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp2 * _tmp6;
  const Scalar _tmp11 = _tmp3 * _tmp7;
  const Scalar _tmp12 = _tmp10 + _tmp11;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp15 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 - _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp9;
  const Scalar _tmp19 = _tmp18 + position_vector(0, 0);
  const Scalar _tmp20 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp14 +
                        Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999);
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp23 = _tmp4 * _tmp6;
  const Scalar _tmp24 = _tmp2 * _tmp7;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(1, 0);
  const Scalar _tmp30 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp31 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp32 =
      -Scalar(0.010999999999999999) * _tmp15 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp34 = _tmp32 - _tmp33;
  const Scalar _tmp35 = _tmp31 + _tmp34;
  const Scalar _tmp36 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp37 = -_tmp21;
  const Scalar _tmp38 = _tmp27 + _tmp37;
  const Scalar _tmp39 = _tmp38 + position_vector(1, 0);
  const Scalar _tmp40 = _tmp39 + Scalar(8.3196563700000006);
  const Scalar _tmp41 = -_tmp9;
  const Scalar _tmp42 = _tmp17 + _tmp41;
  const Scalar _tmp43 = _tmp42 + position_vector(0, 0);
  const Scalar _tmp44 = _tmp43 + Scalar(1.9874742000000001);
  const Scalar _tmp45 = Scalar(1.0) / (_tmp44);
  const Scalar _tmp46 = _tmp40 * _tmp45;
  const Scalar _tmp47 = _tmp22 + _tmp26;
  const Scalar _tmp48 = _tmp37 + _tmp47;
  const Scalar _tmp49 = _tmp48 + position_vector(1, 0);
  const Scalar _tmp50 = _tmp49 + Scalar(8.3888750099999996);
  const Scalar _tmp51 = _tmp13 + _tmp16;
  const Scalar _tmp52 = _tmp41 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(-2.5202214700000001);
  const Scalar _tmp55 = std::pow(Scalar(std::pow(_tmp50, Scalar(2)) + std::pow(_tmp54, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp56 = _tmp54 * _tmp55;
  const Scalar _tmp57 = _tmp50 * _tmp55;
  const Scalar _tmp58 = Scalar(1.0) / (_tmp46 * _tmp56 - _tmp57);
  const Scalar _tmp59 = _tmp56 * _tmp58;
  const Scalar _tmp60 = Scalar(1.0) * _tmp38;
  const Scalar _tmp61 = -_tmp60;
  const Scalar _tmp62 = Scalar(1.0) / (_tmp48 + _tmp61);
  const Scalar _tmp63 = Scalar(1.0) * _tmp42;
  const Scalar _tmp64 = _tmp62 * (-_tmp52 + _tmp63);
  const Scalar _tmp65 = _tmp60 * _tmp64 + _tmp63;
  const Scalar _tmp66 = 0;
  const Scalar _tmp67 = -_tmp31;
  const Scalar _tmp68 = _tmp34 + _tmp67;
  const Scalar _tmp69 = _tmp32 + _tmp33;
  const Scalar _tmp70 = _tmp67 + _tmp69;
  const Scalar _tmp71 = _tmp56 * _tmp68 - _tmp56 * _tmp70;
  const Scalar _tmp72 = _tmp21 + _tmp47;
  const Scalar _tmp73 = _tmp72 + position_vector(1, 0);
  const Scalar _tmp74 = _tmp73 + Scalar(-4.7752063900000001);
  const Scalar _tmp75 = _tmp51 + _tmp9;
  const Scalar _tmp76 = _tmp75 + position_vector(0, 0);
  const Scalar _tmp77 = _tmp76 + Scalar(-2.71799795);
  const Scalar _tmp78 = std::pow(Scalar(std::pow(_tmp74, Scalar(2)) + std::pow(_tmp77, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp79 = _tmp74 * _tmp78;
  const Scalar _tmp80 = _tmp77 * _tmp78;
  const Scalar _tmp81 = _tmp46 * _tmp80 - _tmp79;
  const Scalar _tmp82 = _tmp58 * _tmp81;
  const Scalar _tmp83 = _tmp31 + _tmp69;
  const Scalar _tmp84 = _tmp46 * _tmp68;
  const Scalar _tmp85 = -_tmp56 * _tmp84 + _tmp57 * _tmp70;
  const Scalar _tmp86 = _tmp79 * _tmp83 - _tmp80 * _tmp84 - _tmp82 * _tmp85;
  const Scalar _tmp87 = -_tmp64 * _tmp86 + _tmp68 * _tmp80 - _tmp71 * _tmp82 - _tmp80 * _tmp83;
  const Scalar _tmp88 = Scalar(1.0) / (_tmp87);
  const Scalar _tmp89 = _tmp81 * _tmp88;
  const Scalar _tmp90 = _tmp66 * _tmp89;
  const Scalar _tmp91 = _tmp66 * _tmp88;
  const Scalar _tmp92 =
      std::sqrt(Scalar(std::pow(_tmp40, Scalar(2)) + std::pow(_tmp44, Scalar(2))));
  const Scalar _tmp93 = _tmp45 * _tmp92;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp95 = _tmp93 * (-_tmp38 * _tmp44 * _tmp94 + _tmp40 * _tmp42 * _tmp94);
  const Scalar _tmp96 = _tmp48 * _tmp56 - _tmp52 * _tmp57 + _tmp56 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) * _tmp58;
  const Scalar _tmp98 = _tmp85 * _tmp97;
  const Scalar _tmp99 = _tmp64 * _tmp98 - _tmp71 * _tmp97;
  const Scalar _tmp100 = _tmp72 * _tmp80 - _tmp75 * _tmp79 + _tmp80 * _tmp95 - _tmp82 * _tmp96;
  const Scalar _tmp101 = _tmp100 * _tmp88;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp87;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp99 - _tmp96 * _tmp97);
  const Scalar _tmp105 = _tmp104 + _tmp99;
  const Scalar _tmp106 = -_tmp105 * _tmp89 + Scalar(1.0);
  const Scalar _tmp107 = _tmp80 * _tmp88;
  const Scalar _tmp108 = _tmp29 + Scalar(-4.8333311099999996);
  const Scalar _tmp109 = _tmp19 + Scalar(1.79662371);
  const Scalar _tmp110 =
      std::pow(Scalar(std::pow(_tmp108, Scalar(2)) + std::pow(_tmp109, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp111 = _tmp108 * _tmp110;
  const Scalar _tmp112 = _tmp111 * fh1;
  const Scalar _tmp113 = _tmp46 * _tmp58;
  const Scalar _tmp114 = _tmp113 * _tmp85 + _tmp84;
  const Scalar _tmp115 = _tmp113 * _tmp71 - _tmp114 * _tmp64 - _tmp68;
  const Scalar _tmp116 = _tmp103 * (-_tmp101 * _tmp115 + _tmp113 * _tmp96 - _tmp95);
  const Scalar _tmp117 = _tmp115 + _tmp116;
  const Scalar _tmp118 = -_tmp117 * _tmp89 - _tmp46;
  const Scalar _tmp119 = _tmp109 * _tmp110;
  const Scalar _tmp120 = _tmp119 * fh1;
  const Scalar _tmp121 = Scalar(1.0) * _tmp102;
  const Scalar _tmp122 = _tmp102 * _tmp81 * _tmp97;
  const Scalar _tmp123 = fh1 * (_tmp111 * _tmp18 - _tmp119 * _tmp28);
  const Scalar _tmp124 = -_tmp112 * _tmp93 * (_tmp105 * _tmp107 + _tmp106 * _tmp59) -
                         _tmp120 * _tmp93 * (_tmp107 * _tmp117 + _tmp118 * _tmp59 + Scalar(1.0)) -
                         _tmp123 * _tmp93 * (_tmp121 * _tmp80 - _tmp122 * _tmp56) -
                         _tmp36 * _tmp93 * (-_tmp59 * _tmp90 + _tmp80 * _tmp91);
  const Scalar _tmp125 = Scalar(1.0) / (_tmp124);
  const Scalar _tmp126 = _tmp61 + _tmp72;
  const Scalar _tmp127 = _tmp126 * _tmp64;
  const Scalar _tmp128 = Scalar(1.0) / (-_tmp127 + _tmp63 - _tmp75);
  const Scalar _tmp129 = Scalar(1.0) * _tmp128;
  const Scalar _tmp130 = _tmp103 * _tmp129;
  const Scalar _tmp131 = -_tmp121 * _tmp86 + _tmp126 * _tmp130;
  const Scalar _tmp132 = Scalar(1.0) * _tmp62;
  const Scalar _tmp133 = _tmp126 * _tmp128;
  const Scalar _tmp134 = _tmp86 * _tmp88;
  const Scalar _tmp135 = _tmp114 + _tmp116 * _tmp133 - _tmp117 * _tmp134;
  const Scalar _tmp136 = _tmp128 * _tmp65;
  const Scalar _tmp137 = -_tmp126 * _tmp136 - _tmp134 * _tmp66 + _tmp61;
  const Scalar _tmp138 = _tmp104 * _tmp133 - _tmp105 * _tmp134 - _tmp98;
  const Scalar _tmp139 = _tmp62 * (_tmp127 * _tmp129 + Scalar(1.0));
  const Scalar _tmp140 = _tmp129 * _tmp64;
  const Scalar _tmp141 = _tmp35 * fh1;
  const Scalar _tmp142 = -_tmp111 * _tmp141 - Scalar(5.1796800000000003) * _tmp25 - _tmp28 * fv1;
  const Scalar _tmp143 = _tmp126 * _tmp62;
  const Scalar _tmp144 = _tmp119 * _tmp141 + Scalar(5.1796800000000003) * _tmp12 + _tmp18 * fv1;
  const Scalar _tmp145 = std::asinh(
      _tmp125 * (Scalar(1.0) * _tmp112 * (_tmp104 * _tmp129 - _tmp132 * _tmp138) +
                 Scalar(1.0) * _tmp120 * (_tmp116 * _tmp129 - _tmp132 * _tmp135) +
                 Scalar(1.0) * _tmp123 * (_tmp130 - _tmp131 * _tmp132) +
                 Scalar(1.0) * _tmp142 * (-Scalar(1.0) * _tmp139 + _tmp140) +
                 Scalar(1.0) * _tmp144 * (_tmp129 * _tmp143 - _tmp129) +
                 Scalar(1.0) * _tmp36 * (-_tmp129 * _tmp65 - _tmp132 * _tmp137 + Scalar(1.0))));
  const Scalar _tmp146 = Scalar(9.6622558468725703) * _tmp124;
  const Scalar _tmp147 = _tmp129 * _tmp144;
  const Scalar _tmp148 = _tmp106 * _tmp112 * _tmp58 + _tmp118 * _tmp120 * _tmp58 -
                         _tmp122 * _tmp123 - _tmp36 * _tmp58 * _tmp90;
  const Scalar _tmp149 = Scalar(1.0) / (_tmp148);
  const Scalar _tmp150 =
      std::asinh(_tmp149 * (_tmp112 * _tmp138 * _tmp62 + _tmp120 * _tmp135 * _tmp62 +
                            _tmp123 * _tmp131 * _tmp62 + _tmp137 * _tmp36 * _tmp62 +
                            _tmp139 * _tmp142 - _tmp143 * _tmp147));
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp148;
  const Scalar _tmp152 =
      _tmp105 * _tmp112 * _tmp88 + _tmp117 * _tmp120 * _tmp88 + _tmp121 * _tmp123 + _tmp36 * _tmp91;
  const Scalar _tmp153 = Scalar(1.0) / (_tmp152);
  const Scalar _tmp154 =
      std::asinh(_tmp153 * (-_tmp104 * _tmp112 * _tmp128 - _tmp116 * _tmp120 * _tmp128 -
                            _tmp123 * _tmp130 + _tmp136 * _tmp36 - _tmp140 * _tmp142 + _tmp147));
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp152;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp30 *
          (Scalar(0.86625939559540499) * _tmp0 + std::cosh(Scalar(1.0) * _tmp1) -
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp30 -
                Scalar(4.8333311099999996) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp29), Scalar(2)) +
                        Scalar(0.13817235445745474) *
                            std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp19 - 1),
                                     Scalar(2))))))) +
      _tmp35 + position_vector(2, 0);
  _res(1, 0) =
      -_tmp146 *
          (Scalar(0.87679799772039002) * _tmp125 + std::cosh(Scalar(1.0) * _tmp145) -
           std::cosh(
               Scalar(0.1034955) * _tmp125 *
               (-_tmp145 * _tmp146 -
                Scalar(8.3196563700000006) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp39 - 1), Scalar(2)) +
                        Scalar(0.057067943376852184) *
                            std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp43 - 1),
                                     Scalar(2))))))) +
      _tmp68 + position_vector(2, 0);
  _res(2, 0) =
      -_tmp151 *
          (Scalar(0.87653584775870996) * _tmp149 + std::cosh(Scalar(1.0) * _tmp150) -
           std::cosh(
               Scalar(0.1034955) * _tmp149 *
               (-_tmp150 * _tmp151 -
                Scalar(8.3888750099999996) *
                    std::sqrt(Scalar(
                        Scalar(0.090254729040973036) *
                            std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp53), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp49 - 1),
                                 Scalar(2))))))) +
      _tmp70 + position_vector(2, 0);
  _res(3, 0) =
      -_tmp155 *
          (Scalar(0.86565325453551001) * _tmp153 + std::cosh(Scalar(1.0) * _tmp154) -
           std::cosh(
               Scalar(0.1034955) * _tmp153 *
               (-_tmp154 * _tmp155 -
                Scalar(4.7752063900000001) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp73), Scalar(2)) +
                        Scalar(0.32397683292140877) *
                            std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp76),
                                     Scalar(2))))))) +
      _tmp83 + position_vector(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
