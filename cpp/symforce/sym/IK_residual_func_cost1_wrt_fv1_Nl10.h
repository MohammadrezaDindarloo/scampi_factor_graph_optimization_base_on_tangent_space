// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl10
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl10(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 610

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (194)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp10 = 2 * _tmp3 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp5;
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp5;
  const Scalar _tmp15 = _tmp14 * _tmp9;
  const Scalar _tmp16 = _tmp11 * _tmp3;
  const Scalar _tmp17 = _tmp15 - _tmp16;
  const Scalar _tmp18 = Scalar(0.010999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = _tmp13 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _tmp21 + position_vector(1, 0);
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp24 = -_tmp23;
  const Scalar _tmp25 = _tmp14 * _tmp3;
  const Scalar _tmp26 = _tmp11 * _tmp9;
  const Scalar _tmp27 = _tmp25 + _tmp26;
  const Scalar _tmp28 = -Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = 1 - 2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp29 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp31 = _tmp28 + _tmp30;
  const Scalar _tmp32 = _tmp24 + _tmp31;
  const Scalar _tmp33 = _tmp32 + position_vector(0, 0);
  const Scalar _tmp34 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp35 = _tmp28 - _tmp30;
  const Scalar _tmp36 = _tmp24 + _tmp35;
  const Scalar _tmp37 = -_tmp13;
  const Scalar _tmp38 = _tmp19 + _tmp37 + _tmp8;
  const Scalar _tmp39 = _tmp38 + position_vector(1, 0);
  const Scalar _tmp40 = _tmp39 + Scalar(8.3196563700000006);
  const Scalar _tmp41 = _tmp36 + position_vector(0, 0);
  const Scalar _tmp42 = _tmp41 + Scalar(1.9874742000000001);
  const Scalar _tmp43 = std::pow(Scalar(std::pow(_tmp40, Scalar(2)) + std::pow(_tmp42, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp44 = _tmp40 * _tmp43;
  const Scalar _tmp45 = _tmp42 * _tmp43;
  const Scalar _tmp46 = _tmp37 + _tmp7;
  const Scalar _tmp47 = _tmp19 + _tmp46;
  const Scalar _tmp48 = _tmp47 + position_vector(1, 0);
  const Scalar _tmp49 = _tmp48 + Scalar(-4.8333311099999996);
  const Scalar _tmp50 = _tmp23 + _tmp35;
  const Scalar _tmp51 = _tmp50 + position_vector(0, 0);
  const Scalar _tmp52 = _tmp51 + Scalar(1.79662371);
  const Scalar _tmp53 =
      std::sqrt(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp52, Scalar(2))));
  const Scalar _tmp54 = Scalar(1.0) / (_tmp53);
  const Scalar _tmp55 = Scalar(1.0) / (_tmp52);
  const Scalar _tmp56 = _tmp53 * _tmp55;
  const Scalar _tmp57 = _tmp56 * (-_tmp47 * _tmp52 * _tmp54 + _tmp49 * _tmp50 * _tmp54);
  const Scalar _tmp58 = -_tmp36 * _tmp44 + _tmp38 * _tmp45 + _tmp45 * _tmp57;
  const Scalar _tmp59 = _tmp49 * _tmp55;
  const Scalar _tmp60 = Scalar(1.0) / (-_tmp44 + _tmp45 * _tmp59);
  const Scalar _tmp61 = Scalar(1.0) * _tmp60;
  const Scalar _tmp62 = Scalar(1.0) * _tmp50;
  const Scalar _tmp63 = -_tmp36 + _tmp62;
  const Scalar _tmp64 = Scalar(0.20999999999999999) * _tmp25 - Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp65 = -_tmp64;
  const Scalar _tmp66 =
      -Scalar(0.010999999999999999) * _tmp29 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp67 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp68 = _tmp66 - _tmp67;
  const Scalar _tmp69 = _tmp65 + _tmp68;
  const Scalar _tmp70 = _tmp66 + _tmp67;
  const Scalar _tmp71 = _tmp65 + _tmp70;
  const Scalar _tmp72 = _tmp59 * _tmp71;
  const Scalar _tmp73 = _tmp44 * _tmp69 - _tmp45 * _tmp72;
  const Scalar _tmp74 = Scalar(1.0) * _tmp47;
  const Scalar _tmp75 = -_tmp74;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp38 + _tmp75);
  const Scalar _tmp77 = Scalar(1.0) * _tmp76;
  const Scalar _tmp78 = -_tmp45 * _tmp69 + _tmp45 * _tmp71;
  const Scalar _tmp79 = _tmp60 * _tmp63 * _tmp73 * _tmp77 - _tmp61 * _tmp78;
  const Scalar _tmp80 = _tmp20 + _tmp7;
  const Scalar _tmp81 = _tmp80 + position_vector(1, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(-4.7752063900000001);
  const Scalar _tmp83 = _tmp23 + _tmp31;
  const Scalar _tmp84 = _tmp83 + position_vector(0, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(-2.71799795);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp85 * _tmp86;
  const Scalar _tmp88 = _tmp71 * _tmp87;
  const Scalar _tmp89 = _tmp82 * _tmp86;
  const Scalar _tmp90 = _tmp59 * _tmp87 - _tmp89;
  const Scalar _tmp91 = _tmp60 * _tmp90;
  const Scalar _tmp92 = _tmp64 + _tmp70;
  const Scalar _tmp93 = -_tmp59 * _tmp88 - _tmp73 * _tmp91 + _tmp89 * _tmp92;
  const Scalar _tmp94 = _tmp63 * _tmp76;
  const Scalar _tmp95 = -_tmp78 * _tmp91 - _tmp87 * _tmp92 + _tmp88 - _tmp93 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp57 * _tmp87 - _tmp58 * _tmp91 + _tmp80 * _tmp87 - _tmp83 * _tmp89;
  const Scalar _tmp98 = _tmp96 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp97);
  const Scalar _tmp100 = _tmp95 * _tmp99;
  const Scalar _tmp101 = _tmp100 * (-_tmp58 * _tmp61 - _tmp79 * _tmp98);
  const Scalar _tmp102 = _tmp101 + _tmp79;
  const Scalar _tmp103 = _tmp90 * _tmp96;
  const Scalar _tmp104 = -_tmp102 * _tmp103 + Scalar(1.0);
  const Scalar _tmp105 = _tmp45 * _tmp60;
  const Scalar _tmp106 = _tmp87 * _tmp96;
  const Scalar _tmp107 = _tmp22 + Scalar(8.3888750099999996);
  const Scalar _tmp108 = _tmp33 + Scalar(-2.5202214700000001);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp107 * _tmp109;
  const Scalar _tmp111 = _tmp110 * fh1;
  const Scalar _tmp112 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp113 = _tmp62 + _tmp74 * _tmp94;
  const Scalar _tmp114 = 0;
  const Scalar _tmp115 = _tmp114 * _tmp96;
  const Scalar _tmp116 = _tmp115 * _tmp91;
  const Scalar _tmp117 = _tmp56 * (_tmp115 * _tmp87 - _tmp116 * _tmp45);
  const Scalar _tmp118 = Scalar(1.0) * _tmp99;
  const Scalar _tmp119 = _tmp108 * _tmp109;
  const Scalar _tmp120 = fh1 * (_tmp110 * _tmp32 - _tmp119 * _tmp21);
  const Scalar _tmp121 = _tmp59 * _tmp60;
  const Scalar _tmp122 = _tmp121 * _tmp73 + _tmp72;
  const Scalar _tmp123 = _tmp121 * _tmp78 - _tmp122 * _tmp94 - _tmp71;
  const Scalar _tmp124 = _tmp100 * (_tmp121 * _tmp58 - _tmp123 * _tmp98 - _tmp57);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = -_tmp103 * _tmp125 - _tmp59;
  const Scalar _tmp127 = _tmp119 * fh1;
  const Scalar _tmp128 = -_tmp111 * _tmp56 * (_tmp102 * _tmp106 + _tmp104 * _tmp105) -
                         _tmp112 * _tmp117 -
                         _tmp120 * _tmp56 * (-_tmp118 * _tmp45 * _tmp91 + _tmp118 * _tmp87) -
                         _tmp127 * _tmp56 * (_tmp105 * _tmp126 + _tmp106 * _tmp125 + Scalar(1.0));
  const Scalar _tmp129 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp130 = fh1 * (_tmp64 + _tmp68);
  const Scalar _tmp131 = _tmp119 * _tmp130 + Scalar(5.1796800000000003) * _tmp27 + _tmp32 * fv1;
  const Scalar _tmp132 = _tmp75 + _tmp80;
  const Scalar _tmp133 = _tmp132 * _tmp94;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 + _tmp62 - _tmp83);
  const Scalar _tmp135 = Scalar(1.0) * _tmp134;
  const Scalar _tmp136 = _tmp132 * _tmp76;
  const Scalar _tmp137 = Scalar(1.0) * _tmp135 * _tmp136 - Scalar(1.0) * _tmp135;
  const Scalar _tmp138 = _tmp132 * _tmp134;
  const Scalar _tmp139 = _tmp93 * _tmp96;
  const Scalar _tmp140 = _tmp101 * _tmp138 - _tmp102 * _tmp139 - _tmp61 * _tmp73;
  const Scalar _tmp141 = _tmp113 * _tmp134;
  const Scalar _tmp142 = _tmp76 * (-_tmp114 * _tmp139 - _tmp132 * _tmp141 + _tmp75);
  const Scalar _tmp143 = -Scalar(1.0) * _tmp113 * _tmp135 - Scalar(1.0) * _tmp142 + Scalar(1.0);
  const Scalar _tmp144 = _tmp100 * _tmp135;
  const Scalar _tmp145 = -_tmp118 * _tmp93 + _tmp132 * _tmp144;
  const Scalar _tmp146 = _tmp122 + _tmp124 * _tmp138 - _tmp125 * _tmp139;
  const Scalar _tmp147 = -_tmp110 * _tmp130 - Scalar(5.1796800000000003) * _tmp17 - _tmp21 * fv1;
  const Scalar _tmp148 = _tmp135 * _tmp94;
  const Scalar _tmp149 = _tmp133 * _tmp135 + Scalar(1.0);
  const Scalar _tmp150 = Scalar(1.0) * _tmp148 - Scalar(1.0) * _tmp149 * _tmp77;
  const Scalar _tmp151 = Scalar(1.0) * _tmp111 * (_tmp101 * _tmp135 - _tmp140 * _tmp77) +
                         _tmp112 * _tmp143 + Scalar(1.0) * _tmp120 * (_tmp144 - _tmp145 * _tmp77) +
                         Scalar(1.0) * _tmp127 * (_tmp124 * _tmp135 - _tmp146 * _tmp77) +
                         _tmp131 * _tmp137 + _tmp147 * _tmp150;
  const Scalar _tmp152 = std::asinh(_tmp129 * _tmp151);
  const Scalar _tmp153 = Scalar(1.0) * _tmp152;
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp155 =
      -_tmp152 * _tmp154 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp48), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp51 - 1), Scalar(2))));
  const Scalar _tmp156 = Scalar(0.1034955) * _tmp129;
  const Scalar _tmp157 = _tmp155 * _tmp156;
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp117;
  const Scalar _tmp159 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp160 = _tmp117 * _tmp159;
  const Scalar _tmp161 = _tmp18 + _tmp46;
  const Scalar _tmp162 =
      (_tmp129 * (_tmp137 * _tmp32 - _tmp143 + _tmp150 * _tmp161) - _tmp151 * _tmp160) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp159 + 1));
  const Scalar _tmp163 = _tmp149 * _tmp76;
  const Scalar _tmp164 = _tmp131 * _tmp135;
  const Scalar _tmp165 = _tmp111 * _tmp140 * _tmp76 + _tmp112 * _tmp142 +
                         _tmp120 * _tmp145 * _tmp76 + _tmp127 * _tmp146 * _tmp76 -
                         _tmp136 * _tmp164 + _tmp147 * _tmp163;
  const Scalar _tmp166 = _tmp112 * _tmp115;
  const Scalar _tmp167 = _tmp118 * _tmp120;
  const Scalar _tmp168 =
      _tmp104 * _tmp111 * _tmp60 + _tmp126 * _tmp127 * _tmp60 - _tmp166 * _tmp91 - _tmp167 * _tmp91;
  const Scalar _tmp169 = Scalar(1.0) / (_tmp168);
  const Scalar _tmp170 = std::asinh(_tmp165 * _tmp169);
  const Scalar _tmp171 = Scalar(9.6622558468725703) * _tmp168;
  const Scalar _tmp172 =
      -_tmp170 * _tmp171 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp39 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp41 - 1), Scalar(2))));
  const Scalar _tmp173 = std::pow(_tmp168, Scalar(-2));
  const Scalar _tmp174 = _tmp116 * _tmp173;
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp115;
  const Scalar _tmp176 = _tmp175 * _tmp91;
  const Scalar _tmp177 = _tmp135 * _tmp32;
  const Scalar _tmp178 =
      (-_tmp165 * _tmp174 + _tmp169 * (-_tmp136 * _tmp177 - _tmp142 + _tmp161 * _tmp163)) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp173 + 1));
  const Scalar _tmp179 = Scalar(0.1034955) * _tmp169;
  const Scalar _tmp180 = _tmp172 * _tmp179;
  const Scalar _tmp181 = Scalar(1.0) * _tmp170;
  const Scalar _tmp182 =
      _tmp102 * _tmp111 * _tmp96 + _tmp125 * _tmp127 * _tmp96 + _tmp166 + _tmp167;
  const Scalar _tmp183 = std::pow(_tmp182, Scalar(-2));
  const Scalar _tmp184 = _tmp115 * _tmp183;
  const Scalar _tmp185 = -_tmp101 * _tmp111 * _tmp134 + _tmp112 * _tmp141 - _tmp120 * _tmp144 -
                         _tmp124 * _tmp127 * _tmp134 - _tmp147 * _tmp148 + _tmp164;
  const Scalar _tmp186 = Scalar(1.0) / (_tmp182);
  const Scalar _tmp187 = std::asinh(_tmp185 * _tmp186);
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp182;
  const Scalar _tmp189 =
      -_tmp187 * _tmp188 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp81), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp84), Scalar(2))));
  const Scalar _tmp190 = (_tmp184 * _tmp185 + _tmp186 * (-_tmp141 - _tmp148 * _tmp161 + _tmp177)) /
                         std::sqrt(Scalar(_tmp183 * std::pow(_tmp185, Scalar(2)) + 1));
  const Scalar _tmp191 = Scalar(0.1034955) * _tmp186;
  const Scalar _tmp192 = _tmp189 * _tmp191;
  const Scalar _tmp193 = Scalar(1.0) * _tmp187;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp34 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 *
           std::sinh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp34 -
                Scalar(8.3888750099999996) *
                    std::sqrt(Scalar(
                        Scalar(0.090254729040973036) *
                            std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp33), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp22 - 1), Scalar(2)))))));
  _res(1, 0) =
      -_tmp154 *
          (-Scalar(0.86625939559540499) * _tmp160 + Scalar(1.0) * _tmp162 * std::sinh(_tmp153) -
           (-Scalar(0.1034955) * _tmp155 * _tmp160 +
            _tmp156 * (-_tmp152 * _tmp158 - _tmp154 * _tmp162)) *
               std::sinh(_tmp157)) -
      _tmp158 * (Scalar(0.86625939559540499) * _tmp129 + std::cosh(_tmp153) - std::cosh(_tmp157));
  _res(2, 0) =
      -_tmp171 *
          (-Scalar(0.87679799772039002) * _tmp174 + Scalar(1.0) * _tmp178 * std::sinh(_tmp181) -
           (-Scalar(0.1034955) * _tmp172 * _tmp174 +
            _tmp179 * (-_tmp170 * _tmp176 - _tmp171 * _tmp178)) *
               std::sinh(_tmp180)) -
      _tmp176 * (Scalar(0.87679799772039002) * _tmp169 - std::cosh(_tmp180) + std::cosh(_tmp181));
  _res(3, 0) =
      _tmp175 * (Scalar(0.86565325453551001) * _tmp186 - std::cosh(_tmp192) + std::cosh(_tmp193)) -
      _tmp188 *
          (Scalar(0.86565325453551001) * _tmp184 + Scalar(1.0) * _tmp190 * std::sinh(_tmp193) -
           (Scalar(0.1034955) * _tmp184 * _tmp189 +
            _tmp191 * (_tmp175 * _tmp187 - _tmp188 * _tmp190)) *
               std::sinh(_tmp192));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
