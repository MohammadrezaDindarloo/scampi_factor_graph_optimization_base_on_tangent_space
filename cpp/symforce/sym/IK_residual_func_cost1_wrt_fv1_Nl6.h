// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl6
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl6(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 612

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp4;
  const Scalar _tmp6 = _tmp3 * _tmp5;
  const Scalar _tmp7 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp8 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp9 = _tmp7 * _tmp8;
  const Scalar _tmp10 = Scalar(0.20999999999999999) * _tmp6 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp11 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp12 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp11 +
                        Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999);
  const Scalar _tmp14 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp15 = _tmp4 * _tmp8;
  const Scalar _tmp16 = _tmp14 - _tmp15;
  const Scalar _tmp17 = Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp17;
  const Scalar _tmp19 = -_tmp13 + _tmp18;
  const Scalar _tmp20 = _tmp10 + _tmp19;
  const Scalar _tmp21 = _tmp20 + position_vector(1, 0);
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp6 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = _tmp5 * _tmp7;
  const Scalar _tmp25 = _tmp3 * _tmp8;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp30 = _tmp27 + _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(0, 0);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = -_tmp10;
  const Scalar _tmp35 = _tmp19 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = _tmp36 + Scalar(8.3196563700000006);
  const Scalar _tmp38 = _tmp27 - _tmp29;
  const Scalar _tmp39 = _tmp23 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 + Scalar(1.9874742000000001);
  const Scalar _tmp42 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp43 = _tmp37 * _tmp42;
  const Scalar _tmp44 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp45 = -_tmp44;
  const Scalar _tmp46 =
      -Scalar(0.010999999999999999) * _tmp11 - Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp24 - Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp48 = _tmp46 - _tmp47;
  const Scalar _tmp49 = _tmp45 + _tmp48;
  const Scalar _tmp50 = _tmp10 + _tmp13 + _tmp18;
  const Scalar _tmp51 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp52 = _tmp51 + Scalar(-4.7752063900000001);
  const Scalar _tmp53 = _tmp22 + _tmp30;
  const Scalar _tmp54 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(-2.71799795);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp55 * _tmp56;
  const Scalar _tmp58 = _tmp49 * _tmp57;
  const Scalar _tmp59 = _tmp46 + _tmp47;
  const Scalar _tmp60 = _tmp44 + _tmp59;
  const Scalar _tmp61 = _tmp52 * _tmp56;
  const Scalar _tmp62 = Scalar(1.0) / (_tmp43 * _tmp57 - _tmp61);
  const Scalar _tmp63 = _tmp62 * (-_tmp57 * _tmp60 + _tmp58);
  const Scalar _tmp64 = -_tmp43 * _tmp58 + _tmp60 * _tmp61;
  const Scalar _tmp65 = _tmp43 * _tmp62;
  const Scalar _tmp66 = _tmp43 * _tmp49;
  const Scalar _tmp67 = _tmp64 * _tmp65 + _tmp66;
  const Scalar _tmp68 = Scalar(1.0) * _tmp35;
  const Scalar _tmp69 = -_tmp68;
  const Scalar _tmp70 = Scalar(1.0) / (_tmp50 + _tmp69);
  const Scalar _tmp71 = Scalar(1.0) * _tmp39;
  const Scalar _tmp72 = -_tmp53 + _tmp71;
  const Scalar _tmp73 = _tmp70 * _tmp72;
  const Scalar _tmp74 = _tmp43 * _tmp63 - _tmp49 - _tmp67 * _tmp73;
  const Scalar _tmp75 = _tmp44 + _tmp48;
  const Scalar _tmp76 = _tmp13 + _tmp34;
  const Scalar _tmp77 = _tmp18 + _tmp76;
  const Scalar _tmp78 = _tmp77 + position_vector(1, 0);
  const Scalar _tmp79 = _tmp78 + Scalar(-4.8333311099999996);
  const Scalar _tmp80 = _tmp22 + _tmp38;
  const Scalar _tmp81 = _tmp80 + position_vector(0, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(1.79662371);
  const Scalar _tmp83 = std::pow(Scalar(std::pow(_tmp79, Scalar(2)) + std::pow(_tmp82, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp84 = _tmp79 * _tmp83;
  const Scalar _tmp85 = _tmp82 * _tmp83;
  const Scalar _tmp86 = _tmp43 * _tmp85 - _tmp84;
  const Scalar _tmp87 = _tmp62 * _tmp86;
  const Scalar _tmp88 = -_tmp64 * _tmp87 - _tmp66 * _tmp85 + _tmp75 * _tmp84;
  const Scalar _tmp89 = _tmp49 * _tmp85 - _tmp63 * _tmp86 - _tmp73 * _tmp88 - _tmp75 * _tmp85;
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 =
      std::sqrt(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp41, Scalar(2))));
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp42 * _tmp91;
  const Scalar _tmp94 = _tmp93 * (-_tmp35 * _tmp41 * _tmp92 + _tmp37 * _tmp39 * _tmp92);
  const Scalar _tmp95 = _tmp50 * _tmp57 - _tmp53 * _tmp61 + _tmp57 * _tmp94;
  const Scalar _tmp96 = _tmp77 * _tmp85 - _tmp80 * _tmp84 + _tmp85 * _tmp94 - _tmp87 * _tmp95;
  const Scalar _tmp97 = _tmp90 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp99 = _tmp89 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (_tmp65 * _tmp95 - _tmp74 * _tmp97 - _tmp94);
  const Scalar _tmp101 = _tmp100 + _tmp74;
  const Scalar _tmp102 = _tmp85 * _tmp90;
  const Scalar _tmp103 = _tmp86 * _tmp90;
  const Scalar _tmp104 = -_tmp101 * _tmp103 - _tmp43;
  const Scalar _tmp105 = _tmp57 * _tmp62;
  const Scalar _tmp106 = _tmp32 + Scalar(-2.5202214700000001);
  const Scalar _tmp107 = _tmp21 + Scalar(8.3888750099999996);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = Scalar(1.0) * _tmp98;
  const Scalar _tmp112 = Scalar(1.0) * _tmp62;
  const Scalar _tmp113 = _tmp112 * _tmp86 * _tmp98;
  const Scalar _tmp114 = _tmp107 * _tmp108;
  const Scalar _tmp115 = fh1 * (-_tmp109 * _tmp20 + _tmp114 * _tmp31);
  const Scalar _tmp116 = Scalar(1.0) * _tmp70;
  const Scalar _tmp117 = _tmp116 * _tmp62 * _tmp64 * _tmp72 - Scalar(1.0) * _tmp63;
  const Scalar _tmp118 = _tmp99 * (-_tmp112 * _tmp95 - _tmp117 * _tmp97);
  const Scalar _tmp119 = _tmp117 + _tmp118;
  const Scalar _tmp120 = -_tmp103 * _tmp119 + Scalar(1.0);
  const Scalar _tmp121 = _tmp114 * fh1;
  const Scalar _tmp122 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp123 = _tmp68 * _tmp73 + _tmp71;
  const Scalar _tmp124 = 0;
  const Scalar _tmp125 = _tmp103 * _tmp124;
  const Scalar _tmp126 = _tmp124 * _tmp90;
  const Scalar _tmp127 = _tmp93 * (-_tmp105 * _tmp125 + _tmp126 * _tmp85);
  const Scalar _tmp128 = -_tmp110 * _tmp93 * (_tmp101 * _tmp102 + _tmp104 * _tmp105 + Scalar(1.0)) -
                         _tmp115 * _tmp93 * (_tmp111 * _tmp85 - _tmp113 * _tmp57) -
                         _tmp121 * _tmp93 * (_tmp102 * _tmp119 + _tmp105 * _tmp120) -
                         _tmp122 * _tmp127;
  const Scalar _tmp129 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp130 = fh1 * (_tmp45 + _tmp59);
  const Scalar _tmp131 = _tmp109 * _tmp130 + Scalar(5.1796800000000003) * _tmp26 + _tmp31 * fv1;
  const Scalar _tmp132 = _tmp69 + _tmp77;
  const Scalar _tmp133 = _tmp132 * _tmp73;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 + _tmp71 - _tmp80);
  const Scalar _tmp135 = Scalar(1.0) * _tmp134;
  const Scalar _tmp136 = _tmp132 * _tmp70;
  const Scalar _tmp137 = Scalar(1.0) * _tmp135 * _tmp136 - Scalar(1.0) * _tmp135;
  const Scalar _tmp138 = -_tmp114 * _tmp130 - Scalar(5.1796800000000003) * _tmp16 - _tmp20 * fv1;
  const Scalar _tmp139 = _tmp70 * (_tmp133 * _tmp135 + Scalar(1.0));
  const Scalar _tmp140 = _tmp135 * _tmp73;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp139 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = _tmp88 * _tmp90;
  const Scalar _tmp143 = _tmp132 * _tmp134;
  const Scalar _tmp144 = _tmp100 * _tmp143 - _tmp101 * _tmp142 + _tmp67;
  const Scalar _tmp145 = Scalar(1.0) * fh1;
  const Scalar _tmp146 = _tmp135 * _tmp99;
  const Scalar _tmp147 = -_tmp111 * _tmp88 + _tmp132 * _tmp146;
  const Scalar _tmp148 = _tmp123 * _tmp134;
  const Scalar _tmp149 = _tmp70 * (-_tmp124 * _tmp142 - _tmp132 * _tmp148 + _tmp69);
  const Scalar _tmp150 = -Scalar(1.0) * _tmp123 * _tmp135 - Scalar(1.0) * _tmp149 + Scalar(1.0);
  const Scalar _tmp151 = -_tmp112 * _tmp64 + _tmp118 * _tmp143 - _tmp119 * _tmp142;
  const Scalar _tmp152 = _tmp109 * _tmp145 * (_tmp100 * _tmp135 - _tmp116 * _tmp144) +
                         _tmp114 * _tmp145 * (-_tmp116 * _tmp151 + _tmp118 * _tmp135) +
                         Scalar(1.0) * _tmp115 * (-_tmp116 * _tmp147 + _tmp146) +
                         _tmp122 * _tmp150 + _tmp131 * _tmp137 + _tmp138 * _tmp141;
  const Scalar _tmp153 = std::asinh(_tmp129 * _tmp152);
  const Scalar _tmp154 = Scalar(1.0) * _tmp153;
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp156 =
      -_tmp153 * _tmp155 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp36 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp40 - 1), Scalar(2))));
  const Scalar _tmp157 = Scalar(0.1034955) * _tmp129;
  const Scalar _tmp158 = _tmp156 * _tmp157;
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp160 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp161 = _tmp127 * _tmp160;
  const Scalar _tmp162 = _tmp17 + _tmp76;
  const Scalar _tmp163 =
      (_tmp129 * (_tmp137 * _tmp31 + _tmp141 * _tmp162 - _tmp150) - _tmp152 * _tmp161) /
      std::sqrt(Scalar(std::pow(_tmp152, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp164 = _tmp125 * _tmp62;
  const Scalar _tmp165 = _tmp104 * _tmp110 * _tmp62 - _tmp113 * _tmp115 +
                         _tmp120 * _tmp121 * _tmp62 - _tmp122 * _tmp164;
  const Scalar _tmp166 = Scalar(1.0) / (_tmp165);
  const Scalar _tmp167 = _tmp131 * _tmp135;
  const Scalar _tmp168 = _tmp110 * _tmp144 * _tmp70 + _tmp115 * _tmp147 * _tmp70 +
                         _tmp121 * _tmp151 * _tmp70 + _tmp122 * _tmp149 - _tmp136 * _tmp167 +
                         _tmp138 * _tmp139;
  const Scalar _tmp169 = std::asinh(_tmp166 * _tmp168);
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp171 =
      -_tmp169 * _tmp170 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp51), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp54), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp166;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = Scalar(1.0) * _tmp169;
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp176 = std::pow(_tmp165, Scalar(-2));
  const Scalar _tmp177 = _tmp164 * _tmp176;
  const Scalar _tmp178 = _tmp135 * _tmp31;
  const Scalar _tmp179 =
      (_tmp166 * (-_tmp136 * _tmp178 + _tmp139 * _tmp162 - _tmp149) - _tmp168 * _tmp177) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp176 + 1));
  const Scalar _tmp180 = _tmp101 * _tmp110 * _tmp90 + _tmp111 * _tmp115 +
                         _tmp119 * _tmp121 * _tmp90 + _tmp122 * _tmp126;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp180);
  const Scalar _tmp182 = -_tmp100 * _tmp110 * _tmp134 - _tmp115 * _tmp146 -
                         _tmp118 * _tmp121 * _tmp134 + _tmp122 * _tmp148 - _tmp138 * _tmp140 +
                         _tmp167;
  const Scalar _tmp183 = std::asinh(_tmp181 * _tmp182);
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp185 =
      -_tmp183 * _tmp184 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp78), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp81 - 1), Scalar(2))));
  const Scalar _tmp186 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = Scalar(1.0) * _tmp183;
  const Scalar _tmp189 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp190 = std::pow(_tmp180, Scalar(-2));
  const Scalar _tmp191 = _tmp126 * _tmp190;
  const Scalar _tmp192 = (_tmp181 * (-_tmp140 * _tmp162 - _tmp148 + _tmp178) + _tmp182 * _tmp191) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp190 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 *
           std::sinh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp33 -
                Scalar(8.3888750099999996) *
                    std::sqrt(Scalar(
                        Scalar(0.090254729040973036) *
                            std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp32), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp21 - 1), Scalar(2)))))));
  _res(1, 0) =
      -_tmp155 *
          (-Scalar(0.87679799772039002) * _tmp161 + Scalar(1.0) * _tmp163 * std::sinh(_tmp154) -
           (-Scalar(0.1034955) * _tmp156 * _tmp161 +
            _tmp157 * (-_tmp153 * _tmp159 - _tmp155 * _tmp163)) *
               std::sinh(_tmp158)) -
      _tmp159 * (Scalar(0.87679799772039002) * _tmp129 + std::cosh(_tmp154) - std::cosh(_tmp158));
  _res(2, 0) =
      -_tmp170 *
          (-Scalar(0.86565325453551001) * _tmp177 + Scalar(1.0) * _tmp179 * std::sinh(_tmp174) -
           (-Scalar(0.1034955) * _tmp171 * _tmp177 +
            _tmp172 * (-_tmp169 * _tmp175 - _tmp170 * _tmp179)) *
               std::sinh(_tmp173)) -
      _tmp175 * (Scalar(0.86565325453551001) * _tmp166 - std::cosh(_tmp173) + std::cosh(_tmp174));
  _res(3, 0) =
      -_tmp184 *
          (Scalar(0.86625939559540499) * _tmp191 + Scalar(1.0) * _tmp192 * std::sinh(_tmp188) -
           (Scalar(0.1034955) * _tmp185 * _tmp191 +
            _tmp186 * (_tmp183 * _tmp189 - _tmp184 * _tmp192)) *
               std::sinh(_tmp187)) +
      _tmp189 * (Scalar(0.86625939559540499) * _tmp181 - std::cosh(_tmp187) + std::cosh(_tmp188));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
