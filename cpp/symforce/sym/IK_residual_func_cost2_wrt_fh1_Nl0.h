// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl0
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl0(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 641

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (214)
  const Scalar _tmp0 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp1 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp2 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp3 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp3 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp5 = -_tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp7 = 2 * _tmp6;
  const Scalar _tmp8 = _tmp2 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = _tmp0 * _tmp9;
  const Scalar _tmp11 = -_tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = _tmp0 * _tmp7;
  const Scalar _tmp14 = _tmp2 * _tmp9;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(1, 0);
  const Scalar _tmp19 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp3;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = 2 * _tmp0 * _tmp2;
  const Scalar _tmp23 = _tmp6 * _tmp9;
  const Scalar _tmp24 = _tmp22 + _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 = Scalar(1.0) / (fh1);
  const Scalar _tmp31 = _tmp30 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -_tmp33 * fh1 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp18 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp29 - 1), Scalar(2))));
  const Scalar _tmp35 = Scalar(0.1034955) * _tmp30;
  const Scalar _tmp36 = _tmp34 * _tmp35;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(1.0) * _tmp32;
  const Scalar _tmp40 = _tmp12 + _tmp15;
  const Scalar _tmp41 = _tmp40 + _tmp5;
  const Scalar _tmp42 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp43 = _tmp20 + _tmp27;
  const Scalar _tmp44 = _tmp43 + position_vector(0, 0);
  const Scalar _tmp45 = Scalar(0.20999999999999999) * _tmp22 - Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp46 =
      -Scalar(0.010999999999999999) * _tmp1 - Scalar(0.010999999999999999) * _tmp19;
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp48 = _tmp46 - _tmp47;
  const Scalar _tmp49 = _tmp45 + _tmp48;
  const Scalar _tmp50 = _tmp4 + _tmp40;
  const Scalar _tmp51 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp52 = _tmp51 + Scalar(-4.7752063900000001);
  const Scalar _tmp53 = _tmp25 + _tmp26;
  const Scalar _tmp54 = _tmp20 + _tmp53;
  const Scalar _tmp55 = _tmp54 + position_vector(0, 0);
  const Scalar _tmp56 = _tmp55 + Scalar(-2.71799795);
  const Scalar _tmp57 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp56, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp58 = _tmp56 * _tmp57;
  const Scalar _tmp59 = _tmp46 + _tmp47;
  const Scalar _tmp60 = _tmp45 + _tmp59;
  const Scalar _tmp61 = _tmp49 * _tmp58 - _tmp58 * _tmp60;
  const Scalar _tmp62 = _tmp44 + Scalar(-2.5202214700000001);
  const Scalar _tmp63 = Scalar(1.0) / (_tmp62);
  const Scalar _tmp64 = _tmp42 + Scalar(8.3888750099999996);
  const Scalar _tmp65 = _tmp63 * _tmp64;
  const Scalar _tmp66 = _tmp52 * _tmp57;
  const Scalar _tmp67 = Scalar(1.0) / (_tmp58 * _tmp65 - _tmp66);
  const Scalar _tmp68 = _tmp65 * _tmp67;
  const Scalar _tmp69 = _tmp49 * _tmp65;
  const Scalar _tmp70 = -_tmp58 * _tmp69 + _tmp60 * _tmp66;
  const Scalar _tmp71 = _tmp68 * _tmp70 + _tmp69;
  const Scalar _tmp72 = Scalar(1.0) * _tmp41;
  const Scalar _tmp73 = -_tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp50 + _tmp73);
  const Scalar _tmp75 = Scalar(1.0) * _tmp43;
  const Scalar _tmp76 = _tmp74 * (-_tmp54 + _tmp75);
  const Scalar _tmp77 = -_tmp49 + _tmp61 * _tmp68 - _tmp71 * _tmp76;
  const Scalar _tmp78 = -_tmp45;
  const Scalar _tmp79 = _tmp59 + _tmp78;
  const Scalar _tmp80 = _tmp21 + _tmp53;
  const Scalar _tmp81 = _tmp80 + position_vector(0, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(1.79662371);
  const Scalar _tmp83 = _tmp16 + _tmp4;
  const Scalar _tmp84 = _tmp83 + position_vector(1, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(-4.8333311099999996);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp82 * _tmp86;
  const Scalar _tmp88 = _tmp85 * _tmp86;
  const Scalar _tmp89 = _tmp65 * _tmp87 - _tmp88;
  const Scalar _tmp90 = _tmp67 * _tmp89;
  const Scalar _tmp91 = -_tmp69 * _tmp87 - _tmp70 * _tmp90 + _tmp79 * _tmp88;
  const Scalar _tmp92 = _tmp49 * _tmp87 - _tmp61 * _tmp90 - _tmp76 * _tmp91 - _tmp79 * _tmp87;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp94 =
      std::sqrt(Scalar(std::pow(_tmp62, Scalar(2)) + std::pow(_tmp64, Scalar(2))));
  const Scalar _tmp95 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp96 = _tmp63 * _tmp94;
  const Scalar _tmp97 = _tmp96 * (-_tmp41 * _tmp62 * _tmp95 + _tmp43 * _tmp64 * _tmp95);
  const Scalar _tmp98 = _tmp50 * _tmp58 - _tmp54 * _tmp66 + _tmp58 * _tmp97;
  const Scalar _tmp99 = -_tmp80 * _tmp88 + _tmp83 * _tmp87 + _tmp87 * _tmp97 - _tmp90 * _tmp98;
  const Scalar _tmp100 = _tmp93 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp102 = _tmp101 * _tmp92;
  const Scalar _tmp103 = _tmp102 * (-_tmp100 * _tmp77 + _tmp68 * _tmp98 - _tmp97);
  const Scalar _tmp104 = _tmp103 + _tmp77;
  const Scalar _tmp105 = _tmp87 * _tmp93;
  const Scalar _tmp106 = _tmp89 * _tmp93;
  const Scalar _tmp107 = _tmp67 * (-_tmp104 * _tmp106 - _tmp65);
  const Scalar _tmp108 = _tmp29 + Scalar(1.9874742000000001);
  const Scalar _tmp109 = _tmp18 + Scalar(8.3196563700000006);
  const Scalar _tmp110 =
      std::pow(Scalar(std::pow(_tmp108, Scalar(2)) + std::pow(_tmp109, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp111 = _tmp108 * _tmp110;
  const Scalar _tmp112 = _tmp111 * _tmp96 * (_tmp104 * _tmp105 + _tmp107 * _tmp58 + Scalar(1.0));
  const Scalar _tmp113 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp114 = _tmp72 * _tmp76 + _tmp75;
  const Scalar _tmp115 = 0;
  const Scalar _tmp116 = _tmp115 * _tmp93;
  const Scalar _tmp117 = _tmp58 * _tmp90;
  const Scalar _tmp118 = Scalar(1.0) * _tmp67;
  const Scalar _tmp119 = _tmp118 * _tmp70;
  const Scalar _tmp120 = -_tmp118 * _tmp61 + _tmp119 * _tmp76;
  const Scalar _tmp121 = _tmp102 * (-_tmp100 * _tmp120 - _tmp118 * _tmp98);
  const Scalar _tmp122 = _tmp120 + _tmp121;
  const Scalar _tmp123 = _tmp67 * (-_tmp106 * _tmp122 + Scalar(1.0));
  const Scalar _tmp124 = _tmp109 * _tmp110;
  const Scalar _tmp125 = _tmp124 * _tmp96 * (_tmp105 * _tmp122 + _tmp123 * _tmp58);
  const Scalar _tmp126 = -_tmp111 * _tmp17 + _tmp124 * _tmp28;
  const Scalar _tmp127 = Scalar(1.0) * _tmp101;
  const Scalar _tmp128 = _tmp126 * _tmp96 * (-_tmp117 * _tmp127 + _tmp127 * _tmp87);
  const Scalar _tmp129 = -_tmp112 * fh1 -
                         _tmp113 * _tmp96 * (-_tmp116 * _tmp117 + _tmp116 * _tmp87) -
                         _tmp125 * fh1 - _tmp128 * fh1;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp131 = _tmp91 * _tmp93;
  const Scalar _tmp132 = _tmp73 + _tmp83;
  const Scalar _tmp133 = _tmp132 * _tmp76;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 + _tmp75 - _tmp80);
  const Scalar _tmp135 = _tmp132 * _tmp134;
  const Scalar _tmp136 = -_tmp119 + _tmp121 * _tmp135 - _tmp122 * _tmp131;
  const Scalar _tmp137 = Scalar(1.0) * _tmp74;
  const Scalar _tmp138 = Scalar(1.0) * _tmp134;
  const Scalar _tmp139 = Scalar(1.0) * _tmp124 * (_tmp121 * _tmp138 - _tmp136 * _tmp137);
  const Scalar _tmp140 = _tmp114 * _tmp134;
  const Scalar _tmp141 = -_tmp115 * _tmp131 - _tmp132 * _tmp140 + _tmp73;
  const Scalar _tmp142 = _tmp103 * _tmp135 - _tmp104 * _tmp131 + _tmp71;
  const Scalar _tmp143 = Scalar(1.0) * _tmp111 * (_tmp103 * _tmp138 - _tmp137 * _tmp142);
  const Scalar _tmp144 = _tmp132 * _tmp138;
  const Scalar _tmp145 = _tmp74 * (_tmp102 * _tmp144 - _tmp127 * _tmp91);
  const Scalar _tmp146 = _tmp102 * _tmp138;
  const Scalar _tmp147 = Scalar(1.0) * _tmp126 * (-Scalar(1.0) * _tmp145 + _tmp146);
  const Scalar _tmp148 = _tmp48 + _tmp78;
  const Scalar _tmp149 = _tmp148 * fh1;
  const Scalar _tmp150 = -Scalar(5.1796800000000003) * _tmp11 - _tmp124 * _tmp149 - _tmp17 * fv1;
  const Scalar _tmp151 = _tmp133 * _tmp138 + Scalar(1.0);
  const Scalar _tmp152 = _tmp138 * _tmp76;
  const Scalar _tmp153 = -Scalar(1.0) * _tmp137 * _tmp151 + Scalar(1.0) * _tmp152;
  const Scalar _tmp154 = _tmp111 * _tmp149 + Scalar(5.1796800000000003) * _tmp24 + _tmp28 * fv1;
  const Scalar _tmp155 = _tmp144 * _tmp74;
  const Scalar _tmp156 = -Scalar(1.0) * _tmp138 + Scalar(1.0) * _tmp155;
  const Scalar _tmp157 =
      Scalar(1.0) * _tmp113 * (-_tmp114 * _tmp138 - _tmp137 * _tmp141 + Scalar(1.0)) +
      _tmp139 * fh1 + _tmp143 * fh1 + _tmp147 * fh1 + _tmp150 * _tmp153 + _tmp154 * _tmp156;
  const Scalar _tmp158 = std::asinh(_tmp130 * _tmp157);
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp160 =
      -_tmp158 * _tmp159 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp44), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp42 - 1), Scalar(2))));
  const Scalar _tmp161 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp162 = _tmp160 * _tmp161;
  const Scalar _tmp163 = Scalar(1.0) * _tmp158;
  const Scalar _tmp164 = -_tmp112 - _tmp125 - _tmp128;
  const Scalar _tmp165 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp166 = _tmp111 * _tmp148;
  const Scalar _tmp167 = _tmp124 * _tmp148;
  const Scalar _tmp168 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp169 = _tmp164 * _tmp168;
  const Scalar _tmp170 =
      (_tmp130 * (_tmp139 + _tmp143 + _tmp147 - _tmp153 * _tmp167 + _tmp156 * _tmp166) -
       _tmp157 * _tmp169) /
      std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp168 + 1));
  const Scalar _tmp171 = _tmp126 * _tmp145;
  const Scalar _tmp172 = _tmp124 * _tmp136 * _tmp74;
  const Scalar _tmp173 = _tmp111 * _tmp142 * _tmp74;
  const Scalar _tmp174 = _tmp138 * _tmp154;
  const Scalar _tmp175 = _tmp151 * _tmp74;
  const Scalar _tmp176 = _tmp113 * _tmp141 * _tmp74 - _tmp132 * _tmp174 * _tmp74 +
                         _tmp150 * _tmp175 + _tmp171 * fh1 + _tmp172 * fh1 + _tmp173 * fh1;
  const Scalar _tmp177 = _tmp126 * _tmp127;
  const Scalar _tmp178 = _tmp177 * fh1;
  const Scalar _tmp179 = _tmp113 * _tmp116;
  const Scalar _tmp180 = _tmp107 * _tmp111;
  const Scalar _tmp181 = _tmp123 * _tmp124;
  const Scalar _tmp182 = -_tmp178 * _tmp90 - _tmp179 * _tmp90 + _tmp180 * fh1 + _tmp181 * fh1;
  const Scalar _tmp183 = Scalar(1.0) / (_tmp182);
  const Scalar _tmp184 = std::asinh(_tmp176 * _tmp183);
  const Scalar _tmp185 = Scalar(1.0) * _tmp184;
  const Scalar _tmp186 = std::pow(_tmp182, Scalar(-2));
  const Scalar _tmp187 = -_tmp177 * _tmp90 + _tmp180 + _tmp181;
  const Scalar _tmp188 = _tmp186 * _tmp187;
  const Scalar _tmp189 = (-_tmp176 * _tmp188 + _tmp183 * (-_tmp155 * _tmp166 - _tmp167 * _tmp175 +
                                                          _tmp171 + _tmp172 + _tmp173)) /
                         std::sqrt(Scalar(std::pow(_tmp176, Scalar(2)) * _tmp186 + 1));
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp182;
  const Scalar _tmp191 = Scalar(9.6622558468725703) * _tmp187;
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp183;
  const Scalar _tmp193 =
      -_tmp184 * _tmp190 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp51), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp55), Scalar(2))));
  const Scalar _tmp194 = _tmp192 * _tmp193;
  const Scalar _tmp195 = _tmp126 * _tmp146;
  const Scalar _tmp196 = _tmp121 * _tmp124 * _tmp134;
  const Scalar _tmp197 = _tmp103 * _tmp111 * _tmp134;
  const Scalar _tmp198 = _tmp113 * _tmp140 - _tmp150 * _tmp152 + _tmp174 - _tmp195 * fh1 -
                         _tmp196 * fh1 - _tmp197 * fh1;
  const Scalar _tmp199 = _tmp122 * _tmp124 * _tmp93;
  const Scalar _tmp200 = _tmp104 * _tmp111 * _tmp93;
  const Scalar _tmp201 = _tmp178 + _tmp179 + _tmp199 * fh1 + _tmp200 * fh1;
  const Scalar _tmp202 = Scalar(1.0) / (_tmp201);
  const Scalar _tmp203 = std::asinh(_tmp198 * _tmp202);
  const Scalar _tmp204 = Scalar(9.6622558468725703) * _tmp201;
  const Scalar _tmp205 =
      -_tmp203 * _tmp204 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp84), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp81 - 1), Scalar(2))));
  const Scalar _tmp206 = Scalar(0.1034955) * _tmp202;
  const Scalar _tmp207 = _tmp205 * _tmp206;
  const Scalar _tmp208 = Scalar(1.0) * _tmp203;
  const Scalar _tmp209 = _tmp177 + _tmp199 + _tmp200;
  const Scalar _tmp210 = Scalar(9.6622558468725703) * _tmp209;
  const Scalar _tmp211 = std::pow(_tmp201, Scalar(-2));
  const Scalar _tmp212 = _tmp209 * _tmp211;
  const Scalar _tmp213 = (-_tmp198 * _tmp212 + _tmp202 * (_tmp138 * _tmp166 + _tmp152 * _tmp167 -
                                                          _tmp195 - _tmp196 - _tmp197)) /
                         std::sqrt(Scalar(std::pow(_tmp198, Scalar(2)) * _tmp211 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::cosh(_tmp39) -
                    (-Scalar(0.1034955) * _tmp34 * _tmp37 +
                     _tmp35 * (Scalar(9.6622558468725703) * _tmp31 * _tmp38 - _tmp33)) *
                        std::cosh(_tmp36)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp36) -
               Scalar(9.6622558468725703) * std::sinh(_tmp39);
  _res(1, 0) = _tmp159 * (-Scalar(1.0) * _tmp170 * std::cosh(_tmp163) -
                          (-Scalar(0.1034955) * _tmp160 * _tmp169 +
                           _tmp161 * (-_tmp158 * _tmp165 - _tmp159 * _tmp170)) *
                              std::cosh(_tmp162)) +
               _tmp165 * (-std::sinh(_tmp162) - std::sinh(_tmp163));
  _res(2, 0) = _tmp190 * (-Scalar(1.0) * _tmp189 * std::cosh(_tmp185) -
                          (-Scalar(0.1034955) * _tmp188 * _tmp193 +
                           _tmp192 * (-_tmp184 * _tmp191 - _tmp189 * _tmp190)) *
                              std::cosh(_tmp194)) +
               _tmp191 * (-std::sinh(_tmp185) - std::sinh(_tmp194));
  _res(3, 0) = _tmp204 * (-Scalar(1.0) * _tmp213 * std::cosh(_tmp208) -
                          (-Scalar(0.1034955) * _tmp205 * _tmp212 +
                           _tmp206 * (-_tmp203 * _tmp210 - _tmp204 * _tmp213)) *
                              std::cosh(_tmp207)) +
               _tmp210 * (-std::sinh(_tmp207) - std::sinh(_tmp208));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
