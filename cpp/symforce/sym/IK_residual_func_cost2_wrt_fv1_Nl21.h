// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl21
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl21(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 600

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (195)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp10 = 2 * _tmp5;
  const Scalar _tmp11 = _tmp10 * _tmp9;
  const Scalar _tmp12 = -_DeltaRot[0] * _Rot_init[0] - _DeltaRot[1] * _Rot_init[1] -
                        _DeltaRot[2] * _Rot_init[2] + _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp13 = 2 * _tmp3;
  const Scalar _tmp14 = _tmp12 * _tmp13;
  const Scalar _tmp15 = _tmp11 + _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 * _tmp9;
  const Scalar _tmp18 = _tmp10 * _tmp12;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp17 - Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp16 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _tmp21 + position_vector(0, 0);
  const Scalar _tmp23 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp26 = _tmp10 * _tmp3;
  const Scalar _tmp27 = 2 * _tmp12 * _tmp9;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp30 = -_tmp29;
  const Scalar _tmp31 = -_tmp25 + _tmp30;
  const Scalar _tmp32 = _tmp24 + _tmp31;
  const Scalar _tmp33 = _tmp32 + position_vector(1, 0);
  const Scalar _tmp34 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp35 = -_tmp24;
  const Scalar _tmp36 = _tmp25 + _tmp35;
  const Scalar _tmp37 = _tmp30 + _tmp36;
  const Scalar _tmp38 = _tmp37 + position_vector(1, 0);
  const Scalar _tmp39 = _tmp16 - _tmp19;
  const Scalar _tmp40 = _tmp39 + _tmp7;
  const Scalar _tmp41 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp42 = Scalar(1.0) * _tmp40;
  const Scalar _tmp43 = Scalar(1.0) * _tmp37;
  const Scalar _tmp44 = -_tmp43;
  const Scalar _tmp45 = _tmp31 + _tmp35;
  const Scalar _tmp46 = _tmp44 + _tmp45;
  const Scalar _tmp47 = _tmp24 + _tmp25 + _tmp30;
  const Scalar _tmp48 = Scalar(1.0) / (_tmp44 + _tmp47);
  const Scalar _tmp49 = _tmp20 + _tmp7;
  const Scalar _tmp50 = _tmp42 - _tmp49;
  const Scalar _tmp51 = _tmp48 * _tmp50;
  const Scalar _tmp52 = _tmp46 * _tmp51;
  const Scalar _tmp53 = _tmp39 + _tmp8;
  const Scalar _tmp54 = Scalar(1.0) / (_tmp42 - _tmp52 - _tmp53);
  const Scalar _tmp55 = Scalar(1.0) * _tmp54;
  const Scalar _tmp56 = Scalar(0.20999999999999999) * _tmp26 + Scalar(0.20999999999999999) * _tmp27;
  const Scalar _tmp57 = -_tmp56;
  const Scalar _tmp58 = -Scalar(0.010999999999999999) * _tmp23 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp59 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp60 = _tmp58 + _tmp59;
  const Scalar _tmp61 = _tmp57 + _tmp60;
  const Scalar _tmp62 = _tmp41 + Scalar(-125.0);
  const Scalar _tmp63 = Scalar(1.0) / (_tmp62);
  const Scalar _tmp64 = _tmp38 + Scalar(110.0);
  const Scalar _tmp65 = _tmp63 * _tmp64;
  const Scalar _tmp66 = _tmp47 + position_vector(1, 0);
  const Scalar _tmp67 = _tmp66 + Scalar(-110.0);
  const Scalar _tmp68 = _tmp49 + position_vector(0, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(-125.0);
  const Scalar _tmp70 = std::pow(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp69, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp71 = _tmp69 * _tmp70;
  const Scalar _tmp72 = _tmp61 * _tmp71;
  const Scalar _tmp73 = _tmp56 + _tmp60;
  const Scalar _tmp74 = _tmp67 * _tmp70;
  const Scalar _tmp75 = -_tmp65 * _tmp72 + _tmp73 * _tmp74;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp65 * _tmp71 - _tmp74);
  const Scalar _tmp77 = _tmp65 * _tmp76;
  const Scalar _tmp78 = _tmp61 * _tmp65 + _tmp75 * _tmp77;
  const Scalar _tmp79 = -_tmp71 * _tmp73 + _tmp72;
  const Scalar _tmp80 = -_tmp51 * _tmp78 - _tmp61 + _tmp77 * _tmp79;
  const Scalar _tmp81 = _tmp45 + position_vector(1, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(110.0);
  const Scalar _tmp83 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp84 = _tmp83 + Scalar(125.0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp84 * _tmp85;
  const Scalar _tmp87 = _tmp61 * _tmp86;
  const Scalar _tmp88 = _tmp58 - _tmp59;
  const Scalar _tmp89 = _tmp57 + _tmp88;
  const Scalar _tmp90 = _tmp82 * _tmp85;
  const Scalar _tmp91 = _tmp65 * _tmp86 - _tmp90;
  const Scalar _tmp92 = _tmp76 * _tmp91;
  const Scalar _tmp93 = -_tmp65 * _tmp87 - _tmp75 * _tmp92 + _tmp89 * _tmp90;
  const Scalar _tmp94 = -_tmp51 * _tmp93 - _tmp79 * _tmp92 - _tmp86 * _tmp89 + _tmp87;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp96 =
      std::sqrt(Scalar(std::pow(_tmp62, Scalar(2)) + std::pow(_tmp64, Scalar(2))));
  const Scalar _tmp97 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp98 = _tmp63 * _tmp96;
  const Scalar _tmp99 = _tmp98 * (-_tmp37 * _tmp62 * _tmp97 + _tmp40 * _tmp64 * _tmp97);
  const Scalar _tmp100 = _tmp47 * _tmp71 - _tmp49 * _tmp74 + _tmp71 * _tmp99;
  const Scalar _tmp101 = -_tmp100 * _tmp92 + _tmp45 * _tmp86 - _tmp53 * _tmp90 + _tmp86 * _tmp99;
  const Scalar _tmp102 = _tmp101 * _tmp95;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp101);
  const Scalar _tmp104 = _tmp103 * _tmp94;
  const Scalar _tmp105 = _tmp104 * (_tmp100 * _tmp77 - _tmp102 * _tmp80 - _tmp99);
  const Scalar _tmp106 = _tmp105 + _tmp80;
  const Scalar _tmp107 = _tmp93 * _tmp95;
  const Scalar _tmp108 = _tmp46 * _tmp54;
  const Scalar _tmp109 = _tmp105 * _tmp108 - _tmp106 * _tmp107 + _tmp78;
  const Scalar _tmp110 = Scalar(1.0) * _tmp48;
  const Scalar _tmp111 = _tmp22 + Scalar(125.0);
  const Scalar _tmp112 = _tmp33 + Scalar(-110.0);
  const Scalar _tmp113 =
      std::pow(Scalar(std::pow(_tmp111, Scalar(2)) + std::pow(_tmp112, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp114 = _tmp111 * _tmp113;
  const Scalar _tmp115 = _tmp114 * fh1;
  const Scalar _tmp116 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp117 = _tmp42 + _tmp43 * _tmp51;
  const Scalar _tmp118 = _tmp117 * _tmp54;
  const Scalar _tmp119 = 0;
  const Scalar _tmp120 = _tmp48 * (-_tmp107 * _tmp119 - _tmp118 * _tmp46 + _tmp44);
  const Scalar _tmp121 = -Scalar(1.0) * _tmp117 * _tmp55 - Scalar(1.0) * _tmp120 + Scalar(1.0);
  const Scalar _tmp122 = Scalar(1.0) * _tmp76;
  const Scalar _tmp123 = _tmp110 * _tmp50 * _tmp75 * _tmp76 - _tmp122 * _tmp79;
  const Scalar _tmp124 = _tmp104 * (-_tmp100 * _tmp122 - _tmp102 * _tmp123);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = -_tmp107 * _tmp125 + _tmp108 * _tmp124 - _tmp122 * _tmp75;
  const Scalar _tmp127 = _tmp112 * _tmp113;
  const Scalar _tmp128 = _tmp127 * fh1;
  const Scalar _tmp129 = Scalar(1.0) * _tmp103;
  const Scalar _tmp130 = _tmp46 * _tmp55;
  const Scalar _tmp131 = _tmp104 * _tmp130 - _tmp129 * _tmp93;
  const Scalar _tmp132 = _tmp104 * _tmp55;
  const Scalar _tmp133 = fh1 * (-_tmp114 * _tmp32 + _tmp127 * _tmp21);
  const Scalar _tmp134 = fh1 * (_tmp56 + _tmp88);
  const Scalar _tmp135 = -_tmp127 * _tmp134 - Scalar(40.024799999999999) * _tmp28 - _tmp32 * fv1;
  const Scalar _tmp136 = _tmp51 * _tmp55;
  const Scalar _tmp137 = _tmp52 * _tmp55 + Scalar(1.0);
  const Scalar _tmp138 = -Scalar(1.0) * _tmp110 * _tmp137 + Scalar(1.0) * _tmp136;
  const Scalar _tmp139 = _tmp114 * _tmp134 + Scalar(40.024799999999999) * _tmp15 + _tmp21 * fv1;
  const Scalar _tmp140 = Scalar(1.0) * _tmp130 * _tmp48 - Scalar(1.0) * _tmp55;
  const Scalar _tmp141 = Scalar(1.0) * _tmp115 * (_tmp105 * _tmp55 - _tmp109 * _tmp110) +
                         _tmp116 * _tmp121 +
                         Scalar(1.0) * _tmp128 * (-_tmp110 * _tmp126 + _tmp124 * _tmp55) +
                         Scalar(1.0) * _tmp133 * (-_tmp110 * _tmp131 + _tmp132) +
                         _tmp135 * _tmp138 + _tmp139 * _tmp140;
  const Scalar _tmp142 = _tmp86 * _tmp95;
  const Scalar _tmp143 = _tmp91 * _tmp95;
  const Scalar _tmp144 = -_tmp125 * _tmp143 + Scalar(1.0);
  const Scalar _tmp145 = _tmp71 * _tmp76;
  const Scalar _tmp146 = -_tmp106 * _tmp143 - _tmp65;
  const Scalar _tmp147 = _tmp103 * _tmp122 * _tmp91;
  const Scalar _tmp148 = _tmp119 * _tmp143;
  const Scalar _tmp149 = _tmp119 * _tmp95;
  const Scalar _tmp150 = _tmp98 * (-_tmp145 * _tmp148 + _tmp149 * _tmp86);
  const Scalar _tmp151 = -_tmp115 * _tmp98 * (_tmp106 * _tmp142 + _tmp145 * _tmp146 + Scalar(1.0)) -
                         _tmp116 * _tmp150 -
                         _tmp128 * _tmp98 * (_tmp125 * _tmp142 + _tmp144 * _tmp145) -
                         _tmp133 * _tmp98 * (_tmp129 * _tmp86 - _tmp147 * _tmp71);
  const Scalar _tmp152 = Scalar(1.0) / (_tmp151);
  const Scalar _tmp153 = std::asinh(_tmp141 * _tmp152);
  const Scalar _tmp154 = Scalar(1.4083112389913199) * _tmp151;
  const Scalar _tmp155 =
      -_tmp153 * _tmp154 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp41), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp38 - 1), Scalar(2))));
  const Scalar _tmp156 = Scalar(0.71007031138673404) * _tmp152;
  const Scalar _tmp157 = _tmp155 * _tmp156;
  const Scalar _tmp158 = Scalar(1.0) * _tmp153;
  const Scalar _tmp159 = Scalar(1.4083112389913199) * _tmp150;
  const Scalar _tmp160 = std::pow(_tmp151, Scalar(-2));
  const Scalar _tmp161 = _tmp150 * _tmp160;
  const Scalar _tmp162 = _tmp29 + _tmp36;
  const Scalar _tmp163 =
      (-_tmp141 * _tmp161 + _tmp152 * (-_tmp121 + _tmp138 * _tmp162 + _tmp140 * _tmp21)) /
      std::sqrt(Scalar(std::pow(_tmp141, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp164 = _tmp137 * _tmp48;
  const Scalar _tmp165 = _tmp139 * _tmp55;
  const Scalar _tmp166 = _tmp46 * _tmp48;
  const Scalar _tmp167 = _tmp109 * _tmp115 * _tmp48 + _tmp116 * _tmp120 +
                         _tmp126 * _tmp128 * _tmp48 + _tmp131 * _tmp133 * _tmp48 +
                         _tmp135 * _tmp164 - _tmp165 * _tmp166;
  const Scalar _tmp168 = _tmp148 * _tmp76;
  const Scalar _tmp169 = _tmp115 * _tmp146 * _tmp76 - _tmp116 * _tmp168 +
                         _tmp128 * _tmp144 * _tmp76 - _tmp133 * _tmp147;
  const Scalar _tmp170 = Scalar(1.0) / (_tmp169);
  const Scalar _tmp171 = std::asinh(_tmp167 * _tmp170);
  const Scalar _tmp172 = Scalar(1.0) * _tmp171;
  const Scalar _tmp173 = _tmp21 * _tmp55;
  const Scalar _tmp174 = std::pow(_tmp169, Scalar(-2));
  const Scalar _tmp175 = _tmp168 * _tmp174;
  const Scalar _tmp176 =
      (-_tmp167 * _tmp175 + _tmp170 * (-_tmp120 + _tmp162 * _tmp164 - _tmp166 * _tmp173)) /
      std::sqrt(Scalar(std::pow(_tmp167, Scalar(2)) * _tmp174 + 1));
  const Scalar _tmp177 = Scalar(1.4083112389913199) * _tmp169;
  const Scalar _tmp178 =
      -_tmp171 * _tmp177 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp66), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp68), Scalar(2))));
  const Scalar _tmp179 = Scalar(0.71007031138673404) * _tmp170;
  const Scalar _tmp180 = _tmp178 * _tmp179;
  const Scalar _tmp181 = Scalar(1.4083112389913199) * _tmp168;
  const Scalar _tmp182 = -_tmp105 * _tmp115 * _tmp54 + _tmp116 * _tmp118 -
                         _tmp124 * _tmp128 * _tmp54 - _tmp132 * _tmp133 - _tmp135 * _tmp136 +
                         _tmp165;
  const Scalar _tmp183 = _tmp106 * _tmp115 * _tmp95 + _tmp116 * _tmp149 +
                         _tmp125 * _tmp128 * _tmp95 + _tmp129 * _tmp133;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp182 * _tmp184);
  const Scalar _tmp186 = Scalar(1.4083112389913199) * _tmp149;
  const Scalar _tmp187 = Scalar(1.4083112389913199) * _tmp183;
  const Scalar _tmp188 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp189 = _tmp149 * _tmp188;
  const Scalar _tmp190 = (_tmp182 * _tmp189 + _tmp184 * (-_tmp118 - _tmp136 * _tmp162 + _tmp173)) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp188 + 1));
  const Scalar _tmp191 = Scalar(0.71007031138673404) * _tmp184;
  const Scalar _tmp192 =
      -_tmp185 * _tmp187 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp81 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp83 - 1), Scalar(2))));
  const Scalar _tmp193 = _tmp191 * _tmp192;
  const Scalar _tmp194 = Scalar(1.0) * _tmp185;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp34 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.71007031138673404) * _tmp0 *
                         (-_tmp1 * _tmp34 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp33),
                                               Scalar(2)) +
                                  std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp22 - 1),
                                           Scalar(2)))))));
  _res(1, 0) = _tmp154 * (-Scalar(1.0) * _tmp163 * std::cosh(_tmp158) -
                          (-Scalar(0.71007031138673404) * _tmp155 * _tmp161 +
                           _tmp156 * (-_tmp153 * _tmp159 - _tmp154 * _tmp163)) *
                              std::cosh(_tmp157)) +
               _tmp159 * (-std::sinh(_tmp157) - std::sinh(_tmp158));
  _res(2, 0) = _tmp177 * (-Scalar(1.0) * _tmp176 * std::cosh(_tmp172) -
                          (-Scalar(0.71007031138673404) * _tmp175 * _tmp178 +
                           _tmp179 * (-_tmp171 * _tmp181 - _tmp176 * _tmp177)) *
                              std::cosh(_tmp180)) +
               _tmp181 * (-std::sinh(_tmp172) - std::sinh(_tmp180));
  _res(3, 0) = -_tmp186 * (-std::sinh(_tmp193) - std::sinh(_tmp194)) +
               _tmp187 * (-Scalar(1.0) * _tmp190 * std::cosh(_tmp194) -
                          (Scalar(0.71007031138673404) * _tmp189 * _tmp192 +
                           _tmp191 * (_tmp185 * _tmp186 - _tmp187 * _tmp190)) *
                              std::cosh(_tmp193));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
