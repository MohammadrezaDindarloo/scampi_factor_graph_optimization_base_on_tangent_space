// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl19
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl19(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 528

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (162)
  const Scalar _tmp0 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = 2 * _tmp1;
  const Scalar _tmp3 = _tmp0 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * _Rot_init[0] - _DeltaRot[1] * _Rot_init[1] -
                       _DeltaRot[2] * _Rot_init[2] + _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp5 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp6 = 2 * _tmp5;
  const Scalar _tmp7 = _tmp4 * _tmp6;
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -_tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp11 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp12 =
      -Scalar(0.010999999999999999) * _tmp10 - Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = _tmp0 * _tmp6;
  const Scalar _tmp14 = _tmp2 * _tmp4;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp9;
  const Scalar _tmp18 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = _tmp3 + _tmp7;
  const Scalar _tmp22 = -Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp2 * _tmp5;
  const Scalar _tmp24 = 2 * _tmp0 * _tmp4;
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp26 = _tmp22 + _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(0, 0);
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp10 +
                        Scalar(0.20999999999999999) * _tmp18 + Scalar(0.20999999999999999);
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp31 = _tmp13 - _tmp14;
  const Scalar _tmp32 = -Scalar(0.010999999999999999) * _tmp31;
  const Scalar _tmp33 = -_tmp30 + _tmp32;
  const Scalar _tmp34 = _tmp29 + _tmp33;
  const Scalar _tmp35 = _tmp34 + position_vector(1, 0);
  const Scalar _tmp36 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp35), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp28 - 1), Scalar(2));
  const Scalar _tmp37 = Scalar(1.0) / (fh1);
  const Scalar _tmp38 = std::asinh(_tmp37 * fv1);
  const Scalar _tmp39 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp40 = _tmp12 - _tmp15;
  const Scalar _tmp41 = _tmp40 + _tmp9;
  const Scalar _tmp42 = -_tmp29;
  const Scalar _tmp43 = _tmp33 + _tmp42;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = _tmp22 - _tmp25;
  const Scalar _tmp46 = _tmp20 + _tmp45;
  const Scalar _tmp47 = _tmp46 + position_vector(0, 0);
  const Scalar _tmp48 =
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp44 - 1), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp47 - 1), Scalar(2));
  const Scalar _tmp49 = Scalar(1.0) * _tmp43;
  const Scalar _tmp50 = -_tmp49;
  const Scalar _tmp51 = _tmp30 + _tmp32;
  const Scalar _tmp52 = _tmp42 + _tmp51;
  const Scalar _tmp53 = _tmp50 + _tmp52;
  const Scalar _tmp54 = _tmp29 + _tmp51;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp50 + _tmp54);
  const Scalar _tmp56 = Scalar(1.0) * _tmp46;
  const Scalar _tmp57 = _tmp19 + _tmp26;
  const Scalar _tmp58 = _tmp56 - _tmp57;
  const Scalar _tmp59 = _tmp55 * _tmp58;
  const Scalar _tmp60 = _tmp53 * _tmp59;
  const Scalar _tmp61 = _tmp19 + _tmp45;
  const Scalar _tmp62 = Scalar(1.0) / (_tmp56 - _tmp60 - _tmp61);
  const Scalar _tmp63 = Scalar(1.0) * _tmp62;
  const Scalar _tmp64 = _tmp54 + position_vector(1, 0);
  const Scalar _tmp65 = _tmp64 + Scalar(-110.0);
  const Scalar _tmp66 = _tmp57 + position_vector(0, 0);
  const Scalar _tmp67 = _tmp66 + Scalar(-125.0);
  const Scalar _tmp68 = std::pow(Scalar(std::pow(_tmp65, Scalar(2)) + std::pow(_tmp67, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp69 = _tmp67 * _tmp68;
  const Scalar _tmp70 = _tmp65 * _tmp68;
  const Scalar _tmp71 = _tmp44 + Scalar(110.0);
  const Scalar _tmp72 = _tmp47 + Scalar(125.0);
  const Scalar _tmp73 =
      std::sqrt(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp72, Scalar(2))));
  const Scalar _tmp74 = Scalar(1.0) / (_tmp73);
  const Scalar _tmp75 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp76 * (-_tmp43 * _tmp72 * _tmp74 + _tmp46 * _tmp71 * _tmp74);
  const Scalar _tmp78 = _tmp54 * _tmp69 - _tmp57 * _tmp70 + _tmp69 * _tmp77;
  const Scalar _tmp79 = _tmp71 * _tmp75;
  const Scalar _tmp80 = Scalar(1.0) / (_tmp69 * _tmp79 - _tmp70);
  const Scalar _tmp81 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(110.0);
  const Scalar _tmp83 = _tmp61 + position_vector(0, 0);
  const Scalar _tmp84 = _tmp83 + Scalar(-125.0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp82 * _tmp85;
  const Scalar _tmp87 = _tmp84 * _tmp85;
  const Scalar _tmp88 = _tmp79 * _tmp87 - _tmp86;
  const Scalar _tmp89 = _tmp80 * _tmp88;
  const Scalar _tmp90 = _tmp52 * _tmp87 - _tmp61 * _tmp86 + _tmp77 * _tmp87 - _tmp78 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp41 * _tmp69;
  const Scalar _tmp93 = _tmp16 + _tmp8;
  const Scalar _tmp94 = -_tmp69 * _tmp93 + _tmp92;
  const Scalar _tmp95 = _tmp40 + _tmp8;
  const Scalar _tmp96 = _tmp41 * _tmp87;
  const Scalar _tmp97 = _tmp80 * (_tmp70 * _tmp93 - _tmp79 * _tmp92);
  const Scalar _tmp98 = -_tmp79 * _tmp96 + _tmp86 * _tmp95 - _tmp88 * _tmp97;
  const Scalar _tmp99 = -_tmp59 * _tmp98 - _tmp87 * _tmp95 - _tmp89 * _tmp94 + _tmp96;
  const Scalar _tmp100 = _tmp91 * _tmp99;
  const Scalar _tmp101 = _tmp100 * _tmp63;
  const Scalar _tmp102 = Scalar(1.0) * _tmp91;
  const Scalar _tmp103 = _tmp101 * _tmp53 - _tmp102 * _tmp98;
  const Scalar _tmp104 = Scalar(1.0) * _tmp55;
  const Scalar _tmp105 = _tmp35 + Scalar(-110.0);
  const Scalar _tmp106 = _tmp28 + Scalar(125.0);
  const Scalar _tmp107 =
      std::pow(Scalar(std::pow(_tmp105, Scalar(2)) + std::pow(_tmp106, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp106 * _tmp107;
  const Scalar _tmp110 = fh1 * (_tmp108 * _tmp27 - _tmp109 * _tmp34);
  const Scalar _tmp111 = _tmp60 * _tmp63 + Scalar(1.0);
  const Scalar _tmp112 = _tmp59 * _tmp63;
  const Scalar _tmp113 = _tmp17 * fh1;
  const Scalar _tmp114 = -_tmp108 * _tmp113 - Scalar(40.024799999999999) * _tmp31 - _tmp34 * fv1;
  const Scalar _tmp115 = _tmp109 * _tmp113 + Scalar(40.024799999999999) * _tmp21 + _tmp27 * fv1;
  const Scalar _tmp116 = _tmp53 * _tmp55;
  const Scalar _tmp117 = Scalar(1.0) * _tmp80;
  const Scalar _tmp118 = _tmp104 * _tmp58 * _tmp97 - _tmp117 * _tmp94;
  const Scalar _tmp119 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp120 = _tmp119 * _tmp90;
  const Scalar _tmp121 = _tmp100 * (-_tmp117 * _tmp78 - _tmp118 * _tmp120);
  const Scalar _tmp122 = _tmp118 + _tmp121;
  const Scalar _tmp123 = _tmp119 * _tmp98;
  const Scalar _tmp124 = _tmp53 * _tmp62;
  const Scalar _tmp125 = _tmp121 * _tmp124 - _tmp122 * _tmp123 - Scalar(1.0) * _tmp97;
  const Scalar _tmp126 = _tmp108 * fh1;
  const Scalar _tmp127 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp128 = _tmp49 * _tmp59 + _tmp56;
  const Scalar _tmp129 = 0;
  const Scalar _tmp130 = _tmp128 * _tmp62;
  const Scalar _tmp131 = -_tmp129 * _tmp98 - _tmp130 * _tmp53 + _tmp50;
  const Scalar _tmp132 = _tmp79 * _tmp80;
  const Scalar _tmp133 = _tmp41 * _tmp79 + _tmp79 * _tmp97;
  const Scalar _tmp134 = _tmp132 * _tmp94 - _tmp133 * _tmp59 - _tmp41;
  const Scalar _tmp135 = _tmp100 * (-_tmp120 * _tmp134 + _tmp132 * _tmp78 - _tmp77);
  const Scalar _tmp136 = _tmp134 + _tmp135;
  const Scalar _tmp137 = -_tmp123 * _tmp136 + _tmp124 * _tmp135 + _tmp133;
  const Scalar _tmp138 = _tmp109 * fh1;
  const Scalar _tmp139 = _tmp119 * _tmp87;
  const Scalar _tmp140 = _tmp119 * _tmp88;
  const Scalar _tmp141 = -_tmp122 * _tmp140 + Scalar(1.0);
  const Scalar _tmp142 = _tmp69 * _tmp80;
  const Scalar _tmp143 = _tmp69 * _tmp89;
  const Scalar _tmp144 = -_tmp136 * _tmp140 - _tmp79;
  const Scalar _tmp145 = -_tmp110 * _tmp76 * (-_tmp102 * _tmp143 + _tmp102 * _tmp87) -
                         _tmp126 * _tmp76 * (_tmp122 * _tmp139 + _tmp141 * _tmp142) -
                         _tmp127 * _tmp76 * (-_tmp129 * _tmp143 + _tmp129 * _tmp87) -
                         _tmp138 * _tmp76 * (_tmp136 * _tmp139 + _tmp142 * _tmp144 + Scalar(1.0));
  const Scalar _tmp146 = Scalar(1.0) / (_tmp145);
  const Scalar _tmp147 = std::asinh(
      _tmp146 * (Scalar(1.0) * _tmp110 * (_tmp101 - _tmp103 * _tmp104) +
                 Scalar(1.0) * _tmp114 * (-_tmp104 * _tmp111 + _tmp112) +
                 Scalar(1.0) * _tmp115 * (_tmp116 * _tmp63 - _tmp63) +
                 Scalar(1.0) * _tmp126 * (-_tmp104 * _tmp125 + _tmp121 * _tmp63) +
                 Scalar(1.0) * _tmp127 * (-_tmp104 * _tmp131 - _tmp128 * _tmp63 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp138 * (-_tmp104 * _tmp137 + _tmp135 * _tmp63)));
  const Scalar _tmp148 = Scalar(1.4083112389913199) * _tmp145;
  const Scalar _tmp149 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp64), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp66), Scalar(2));
  const Scalar _tmp150 = _tmp102 * _tmp110;
  const Scalar _tmp151 = _tmp127 * _tmp129;
  const Scalar _tmp152 =
      _tmp126 * _tmp141 * _tmp80 + _tmp138 * _tmp144 * _tmp80 - _tmp150 * _tmp89 - _tmp151 * _tmp89;
  const Scalar _tmp153 = Scalar(1.0) / (_tmp152);
  const Scalar _tmp154 = _tmp115 * _tmp63;
  const Scalar _tmp155 =
      std::asinh(_tmp153 * (_tmp103 * _tmp110 * _tmp55 + _tmp111 * _tmp114 * _tmp55 -
                            _tmp116 * _tmp154 + _tmp125 * _tmp126 * _tmp55 +
                            _tmp127 * _tmp131 * _tmp55 + _tmp137 * _tmp138 * _tmp55));
  const Scalar _tmp156 = Scalar(1.4083112389913199) * _tmp152;
  const Scalar _tmp157 =
      _tmp119 * _tmp122 * _tmp126 + _tmp119 * _tmp136 * _tmp138 + _tmp150 + _tmp151;
  const Scalar _tmp158 = Scalar(1.0) / (_tmp157);
  const Scalar _tmp159 =
      std::asinh(_tmp158 * (-_tmp101 * _tmp110 - _tmp112 * _tmp114 - _tmp121 * _tmp126 * _tmp62 +
                            _tmp127 * _tmp130 - _tmp135 * _tmp138 * _tmp62 + _tmp154));
  const Scalar _tmp160 = Scalar(1.4083112389913199) * _tmp157;
  const Scalar _tmp161 =
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp83), Scalar(2)) +
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp81 - 1), Scalar(2));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp39 *
          (-std::sinh(Scalar(1.0) * _tmp38) - std::sinh(Scalar(0.71007031138673404) * _tmp37 *
                                                        (-std::sqrt(_tmp36) - _tmp38 * _tmp39))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp36 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp17 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(1, 0) =
      _tmp148 * (-std::sinh(Scalar(1.0) * _tmp147) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp146 *
                           (-_tmp147 * _tmp148 - std::sqrt(_tmp48)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp48 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp41 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(2, 0) =
      _tmp156 * (-std::sinh(Scalar(1.0) * _tmp155) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp153 *
                           (-std::sqrt(_tmp149) - _tmp155 * _tmp156))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp149 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp93 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(3, 0) =
      _tmp160 * (-std::sinh(Scalar(1.0) * _tmp159) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp158 *
                           (-_tmp159 * _tmp160 - std::sqrt(_tmp161)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp161 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp95 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
