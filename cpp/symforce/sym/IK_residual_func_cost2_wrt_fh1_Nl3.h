// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl3
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl3(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 639

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (211)
  const Scalar _tmp0 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp1 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp2 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp3 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp3 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp5 = -_tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp7 = 2 * _tmp6;
  const Scalar _tmp8 = _tmp0 * _tmp7;
  const Scalar _tmp9 = -_DeltaRot[0] * _Rot_init[0] - _DeltaRot[1] * _Rot_init[1] -
                       _DeltaRot[2] * _Rot_init[2] + _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = 2 * _tmp2 * _tmp9;
  const Scalar _tmp11 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp12 = _tmp2 * _tmp7;
  const Scalar _tmp13 = 2 * _tmp0;
  const Scalar _tmp14 = _tmp13 * _tmp9;
  const Scalar _tmp15 = _tmp12 - _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp11 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp5;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp3;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = _tmp13 * _tmp2;
  const Scalar _tmp24 = _tmp7 * _tmp9;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = Scalar(1.0) / (fh1);
  const Scalar _tmp32 = _tmp31 * fv1;
  const Scalar _tmp33 = std::asinh(_tmp32);
  const Scalar _tmp34 = Scalar(9.6622558468725703) * _tmp33;
  const Scalar _tmp35 =
      -_tmp34 * fh1 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp19 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp30 - 1), Scalar(2))));
  const Scalar _tmp36 = Scalar(0.1034955) * _tmp31;
  const Scalar _tmp37 = _tmp35 * _tmp36;
  const Scalar _tmp38 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp39 =
      std::pow(Scalar(_tmp38 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp40 = Scalar(1.0) * _tmp33;
  const Scalar _tmp41 = _tmp26 + _tmp27;
  const Scalar _tmp42 = _tmp22 + _tmp41;
  const Scalar _tmp43 = _tmp42 + position_vector(0, 0);
  const Scalar _tmp44 = _tmp43 + Scalar(1.79662371);
  const Scalar _tmp45 = _tmp17 + _tmp4;
  const Scalar _tmp46 = _tmp45 + position_vector(1, 0);
  const Scalar _tmp47 = _tmp46 + Scalar(-4.8333311099999996);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp44 * _tmp48;
  const Scalar _tmp50 = _tmp47 * _tmp48;
  const Scalar _tmp51 = _tmp11 + _tmp16;
  const Scalar _tmp52 = _tmp4 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(-4.7752063900000001);
  const Scalar _tmp55 = _tmp21 + _tmp41;
  const Scalar _tmp56 = _tmp55 + position_vector(0, 0);
  const Scalar _tmp57 = _tmp56 + Scalar(-2.71799795);
  const Scalar _tmp58 = Scalar(1.0) / (_tmp57);
  const Scalar _tmp59 = _tmp54 * _tmp58;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp49 * _tmp59 - _tmp50);
  const Scalar _tmp61 = _tmp5 + _tmp51;
  const Scalar _tmp62 = _tmp61 + position_vector(1, 0);
  const Scalar _tmp63 = _tmp62 + Scalar(8.3888750099999996);
  const Scalar _tmp64 = _tmp21 + _tmp28;
  const Scalar _tmp65 = _tmp64 + position_vector(0, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-2.5202214700000001);
  const Scalar _tmp67 = std::pow(Scalar(std::pow(_tmp63, Scalar(2)) + std::pow(_tmp66, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp68 = _tmp63 * _tmp67;
  const Scalar _tmp69 = _tmp66 * _tmp67;
  const Scalar _tmp70 = _tmp59 * _tmp69 - _tmp68;
  const Scalar _tmp71 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp72 =
      -Scalar(0.010999999999999999) * _tmp1 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp73 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp74 = _tmp72 + _tmp73;
  const Scalar _tmp75 = _tmp71 + _tmp74;
  const Scalar _tmp76 = _tmp59 * _tmp75;
  const Scalar _tmp77 = -_tmp71;
  const Scalar _tmp78 = _tmp74 + _tmp77;
  const Scalar _tmp79 = _tmp72 - _tmp73;
  const Scalar _tmp80 = _tmp71 + _tmp79;
  const Scalar _tmp81 = _tmp60 * (-_tmp49 * _tmp76 + _tmp50 * _tmp80);
  const Scalar _tmp82 = _tmp68 * _tmp78 - _tmp69 * _tmp76 - _tmp70 * _tmp81;
  const Scalar _tmp83 = Scalar(1.0) * _tmp52;
  const Scalar _tmp84 = -_tmp83;
  const Scalar _tmp85 = Scalar(1.0) / (_tmp45 + _tmp84);
  const Scalar _tmp86 = Scalar(1.0) * _tmp55;
  const Scalar _tmp87 = _tmp85 * (-_tmp42 + _tmp86);
  const Scalar _tmp88 = _tmp49 * _tmp75 - _tmp49 * _tmp80;
  const Scalar _tmp89 = _tmp60 * _tmp70;
  const Scalar _tmp90 = _tmp69 * _tmp75 - _tmp69 * _tmp78 - _tmp82 * _tmp87 - _tmp88 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = Scalar(1.0) * _tmp81;
  const Scalar _tmp93 = Scalar(1.0) * _tmp60;
  const Scalar _tmp94 = _tmp87 * _tmp92 - _tmp88 * _tmp93;
  const Scalar _tmp95 =
      std::sqrt(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp57, Scalar(2))));
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp58 * _tmp95;
  const Scalar _tmp98 = _tmp97 * (-_tmp52 * _tmp57 * _tmp96 + _tmp54 * _tmp55 * _tmp96);
  const Scalar _tmp99 = -_tmp42 * _tmp50 + _tmp45 * _tmp49 + _tmp49 * _tmp98;
  const Scalar _tmp100 = _tmp61 * _tmp69 - _tmp64 * _tmp68 + _tmp69 * _tmp98 - _tmp89 * _tmp99;
  const Scalar _tmp101 = _tmp100 * _tmp91;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp90;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp94 - _tmp93 * _tmp99);
  const Scalar _tmp105 = _tmp91 * (_tmp104 + _tmp94);
  const Scalar _tmp106 = _tmp60 * (-_tmp105 * _tmp70 + Scalar(1.0));
  const Scalar _tmp107 = _tmp19 + Scalar(8.3196563700000006);
  const Scalar _tmp108 = _tmp30 + Scalar(1.9874742000000001);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp107 * _tmp109;
  const Scalar _tmp111 = _tmp110 * _tmp97 * (_tmp105 * _tmp69 + _tmp106 * _tmp49);
  const Scalar _tmp112 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp113 = _tmp83 * _tmp87 + _tmp86;
  const Scalar _tmp114 = 0;
  const Scalar _tmp115 = _tmp59 * _tmp60;
  const Scalar _tmp116 = _tmp59 * _tmp81 + _tmp76;
  const Scalar _tmp117 = _tmp115 * _tmp88 - _tmp116 * _tmp87 - _tmp75;
  const Scalar _tmp118 = _tmp103 * (-_tmp101 * _tmp117 + _tmp115 * _tmp99 - _tmp98);
  const Scalar _tmp119 = _tmp91 * (_tmp117 + _tmp118);
  const Scalar _tmp120 = _tmp60 * (-_tmp119 * _tmp70 - _tmp59);
  const Scalar _tmp121 = _tmp108 * _tmp109;
  const Scalar _tmp122 = _tmp121 * _tmp97 * (_tmp119 * _tmp69 + _tmp120 * _tmp49 + Scalar(1.0));
  const Scalar _tmp123 = _tmp110 * _tmp29 - _tmp121 * _tmp18;
  const Scalar _tmp124 = Scalar(1.0) * _tmp102;
  const Scalar _tmp125 = _tmp102 * _tmp70 * _tmp93;
  const Scalar _tmp126 = _tmp123 * _tmp97 * (_tmp124 * _tmp69 - _tmp125 * _tmp49);
  const Scalar _tmp127 = -_tmp111 * fh1 -
                         _tmp112 * _tmp97 * (-_tmp114 * _tmp49 * _tmp89 + _tmp114 * _tmp69) -
                         _tmp122 * fh1 - _tmp126 * fh1;
  const Scalar _tmp128 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp129 = _tmp61 + _tmp84;
  const Scalar _tmp130 = _tmp129 * _tmp87;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 - _tmp64 + _tmp86);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp129 * _tmp131;
  const Scalar _tmp134 = _tmp104 * _tmp133 - _tmp105 * _tmp82 - _tmp92;
  const Scalar _tmp135 = Scalar(1.0) * _tmp85;
  const Scalar _tmp136 = Scalar(1.0) * _tmp110 * (_tmp104 * _tmp132 - _tmp134 * _tmp135);
  const Scalar _tmp137 = _tmp116 + _tmp118 * _tmp133 - _tmp119 * _tmp82;
  const Scalar _tmp138 = Scalar(1.0) * _tmp121 * (_tmp118 * _tmp132 - _tmp135 * _tmp137);
  const Scalar _tmp139 = _tmp77 + _tmp79;
  const Scalar _tmp140 = _tmp139 * fh1;
  const Scalar _tmp141 = -_tmp110 * _tmp140 - Scalar(5.1796800000000003) * _tmp15 - _tmp18 * fv1;
  const Scalar _tmp142 = _tmp132 * _tmp87;
  const Scalar _tmp143 = _tmp130 * _tmp132 + Scalar(1.0);
  const Scalar _tmp144 = -Scalar(1.0) * _tmp135 * _tmp143 + Scalar(1.0) * _tmp142;
  const Scalar _tmp145 = _tmp103 * _tmp132;
  const Scalar _tmp146 = -_tmp124 * _tmp82 + _tmp129 * _tmp145;
  const Scalar _tmp147 = Scalar(1.0) * _tmp123 * (-_tmp135 * _tmp146 + _tmp145);
  const Scalar _tmp148 = _tmp121 * _tmp140 + Scalar(5.1796800000000003) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp149 = _tmp129 * _tmp85;
  const Scalar _tmp150 = Scalar(1.0) * _tmp132 * _tmp149 - Scalar(1.0) * _tmp132;
  const Scalar _tmp151 = _tmp113 * _tmp131;
  const Scalar _tmp152 = -_tmp114 * _tmp82 - _tmp129 * _tmp151 + _tmp84;
  const Scalar _tmp153 =
      Scalar(1.0) * _tmp112 * (-_tmp113 * _tmp132 - _tmp135 * _tmp152 + Scalar(1.0)) +
      _tmp136 * fh1 + _tmp138 * fh1 + _tmp141 * _tmp144 + _tmp147 * fh1 + _tmp148 * _tmp150;
  const Scalar _tmp154 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp155 = -_tmp111 - _tmp122 - _tmp126;
  const Scalar _tmp156 = _tmp154 * _tmp155;
  const Scalar _tmp157 = _tmp110 * _tmp139;
  const Scalar _tmp158 = _tmp121 * _tmp139;
  const Scalar _tmp159 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp160 = (-_tmp153 * _tmp156 + _tmp159 * (_tmp136 + _tmp138 - _tmp144 * _tmp157 +
                                                          _tmp147 + _tmp150 * _tmp158)) /
                         std::sqrt(Scalar(std::pow(_tmp153, Scalar(2)) * _tmp154 + 1));
  const Scalar _tmp161 = std::asinh(_tmp153 * _tmp159);
  const Scalar _tmp162 = Scalar(9.6622558468725703) * _tmp155;
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp159;
  const Scalar _tmp164 =
      -_tmp128 * _tmp161 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp53), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp56), Scalar(2))));
  const Scalar _tmp165 = _tmp163 * _tmp164;
  const Scalar _tmp166 = Scalar(1.0) * _tmp161;
  const Scalar _tmp167 = _tmp121 * _tmp85;
  const Scalar _tmp168 = _tmp137 * _tmp167;
  const Scalar _tmp169 = _tmp123 * _tmp146 * _tmp85;
  const Scalar _tmp170 = _tmp132 * _tmp148;
  const Scalar _tmp171 = _tmp110 * _tmp134 * _tmp85;
  const Scalar _tmp172 = _tmp143 * _tmp85;
  const Scalar _tmp173 = _tmp112 * _tmp152 * _tmp85 + _tmp141 * _tmp172 - _tmp149 * _tmp170 +
                         _tmp168 * fh1 + _tmp169 * fh1 + _tmp171 * fh1;
  const Scalar _tmp174 = _tmp120 * _tmp121;
  const Scalar _tmp175 = _tmp112 * _tmp114;
  const Scalar _tmp176 = _tmp123 * _tmp125;
  const Scalar _tmp177 = _tmp106 * _tmp110;
  const Scalar _tmp178 = _tmp174 * fh1 - _tmp175 * _tmp89 - _tmp176 * fh1 + _tmp177 * fh1;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = std::asinh(_tmp173 * _tmp179);
  const Scalar _tmp181 = Scalar(1.0) * _tmp180;
  const Scalar _tmp182 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp183 = _tmp174 - _tmp176 + _tmp177;
  const Scalar _tmp184 = _tmp182 * _tmp183;
  const Scalar _tmp185 =
      (-_tmp173 * _tmp184 + _tmp179 * (-_tmp129 * _tmp132 * _tmp139 * _tmp167 - _tmp157 * _tmp172 +
                                       _tmp168 + _tmp169 + _tmp171)) /
      std::sqrt(Scalar(std::pow(_tmp173, Scalar(2)) * _tmp182 + 1));
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp187 =
      -_tmp178 * _tmp186 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp46), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp43 - 1), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp191 = _tmp123 * _tmp145;
  const Scalar _tmp192 = _tmp104 * _tmp110 * _tmp131;
  const Scalar _tmp193 = _tmp118 * _tmp121 * _tmp131;
  const Scalar _tmp194 = _tmp112 * _tmp151 - _tmp141 * _tmp142 + _tmp170 - _tmp191 * fh1 -
                         _tmp192 * fh1 - _tmp193 * fh1;
  const Scalar _tmp195 = _tmp123 * _tmp124;
  const Scalar _tmp196 = _tmp105 * _tmp110;
  const Scalar _tmp197 = _tmp119 * _tmp121;
  const Scalar _tmp198 = _tmp175 + _tmp195 * fh1 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp199 = Scalar(1.0) / (_tmp198);
  const Scalar _tmp200 = std::asinh(_tmp194 * _tmp199);
  const Scalar _tmp201 = Scalar(9.6622558468725703) * _tmp198;
  const Scalar _tmp202 =
      -_tmp200 * _tmp201 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp65), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp62 - 1), Scalar(2))));
  const Scalar _tmp203 = Scalar(0.1034955) * _tmp199;
  const Scalar _tmp204 = _tmp202 * _tmp203;
  const Scalar _tmp205 = Scalar(1.0) * _tmp200;
  const Scalar _tmp206 = _tmp195 + _tmp196 + _tmp197;
  const Scalar _tmp207 = Scalar(9.6622558468725703) * _tmp206;
  const Scalar _tmp208 = std::pow(_tmp198, Scalar(-2));
  const Scalar _tmp209 = _tmp206 * _tmp208;
  const Scalar _tmp210 = (-_tmp194 * _tmp209 + _tmp199 * (_tmp132 * _tmp158 + _tmp142 * _tmp157 -
                                                          _tmp191 - _tmp192 - _tmp193)) /
                         std::sqrt(Scalar(std::pow(_tmp194, Scalar(2)) * _tmp208 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp38 * _tmp39 * fv1 * std::cosh(_tmp40) -
                    (-Scalar(0.1034955) * _tmp35 * _tmp38 +
                     _tmp36 * (Scalar(9.6622558468725703) * _tmp32 * _tmp39 - _tmp34)) *
                        std::cosh(_tmp37)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp37) -
               Scalar(9.6622558468725703) * std::sinh(_tmp40);
  _res(1, 0) = _tmp128 * (-Scalar(1.0) * _tmp160 * std::cosh(_tmp166) -
                          (-Scalar(0.1034955) * _tmp156 * _tmp164 +
                           _tmp163 * (-_tmp128 * _tmp160 - _tmp161 * _tmp162)) *
                              std::cosh(_tmp165)) +
               _tmp162 * (-std::sinh(_tmp165) - std::sinh(_tmp166));
  _res(2, 0) = Scalar(9.6622558468725703) * _tmp183 * (-std::sinh(_tmp181) - std::sinh(_tmp189)) +
               _tmp190 * (-Scalar(1.0) * _tmp185 * std::cosh(_tmp181) -
                          (-Scalar(0.1034955) * _tmp184 * _tmp187 +
                           _tmp188 * (-_tmp183 * _tmp186 - _tmp185 * _tmp190)) *
                              std::cosh(_tmp189));
  _res(3, 0) = _tmp201 * (-Scalar(1.0) * _tmp210 * std::cosh(_tmp205) -
                          (-Scalar(0.1034955) * _tmp202 * _tmp209 +
                           _tmp203 * (-_tmp200 * _tmp207 - _tmp201 * _tmp210)) *
                              std::cosh(_tmp204)) +
               _tmp207 * (-std::sinh(_tmp204) - std::sinh(_tmp205));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
