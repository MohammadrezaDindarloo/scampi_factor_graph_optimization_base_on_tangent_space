// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_Nl22
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1Nl22(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot, const Scalar p_init0,
    const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x, const Scalar rot_init_y,
    const Scalar rot_init_z, const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 502

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (152)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = _tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp1 * _tmp8;
  const Scalar _tmp15 = _tmp10 * _tmp3;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + p_init0;
  const Scalar _tmp20 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp23 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + p_init1;
  const Scalar _tmp30 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp32 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp33 = -_tmp32;
  const Scalar _tmp34 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp35 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp36 = _tmp34 + _tmp35;
  const Scalar _tmp37 = _tmp33 + _tmp36;
  const Scalar _tmp38 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp39 = _tmp34 - _tmp35;
  const Scalar _tmp40 = _tmp32 + _tmp39;
  const Scalar _tmp41 = _tmp13 - _tmp16;
  const Scalar _tmp42 = _tmp41 + _tmp5;
  const Scalar _tmp43 = _tmp42 + p_init0;
  const Scalar _tmp44 = _tmp43 + Scalar(-2.5202214700000001);
  const Scalar _tmp45 = -_tmp21;
  const Scalar _tmp46 = _tmp22 + _tmp26;
  const Scalar _tmp47 = _tmp45 + _tmp46;
  const Scalar _tmp48 = _tmp47 + p_init1;
  const Scalar _tmp49 = _tmp48 + Scalar(8.3888750099999996);
  const Scalar _tmp50 = std::pow(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp49, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp51 = _tmp49 * _tmp50;
  const Scalar _tmp52 = _tmp32 + _tmp36;
  const Scalar _tmp53 = _tmp17 + _tmp5;
  const Scalar _tmp54 = _tmp53 + p_init0;
  const Scalar _tmp55 = _tmp54 + Scalar(-2.71799795);
  const Scalar _tmp56 = Scalar(1.0) / (_tmp55);
  const Scalar _tmp57 = _tmp21 + _tmp46;
  const Scalar _tmp58 = _tmp57 + p_init1;
  const Scalar _tmp59 = _tmp58 + Scalar(-4.7752063900000001);
  const Scalar _tmp60 = _tmp56 * _tmp59;
  const Scalar _tmp61 = _tmp52 * _tmp60;
  const Scalar _tmp62 = _tmp27 + _tmp45;
  const Scalar _tmp63 = _tmp62 + p_init1;
  const Scalar _tmp64 = _tmp63 + Scalar(8.3196563700000006);
  const Scalar _tmp65 = _tmp41 + _tmp6;
  const Scalar _tmp66 = _tmp65 + p_init0;
  const Scalar _tmp67 = _tmp66 + Scalar(1.9874742000000001);
  const Scalar _tmp68 = std::pow(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp67, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp69 = _tmp67 * _tmp68;
  const Scalar _tmp70 = _tmp33 + _tmp39;
  const Scalar _tmp71 = _tmp64 * _tmp68;
  const Scalar _tmp72 = -_tmp61 * _tmp69 + _tmp70 * _tmp71;
  const Scalar _tmp73 = _tmp44 * _tmp50;
  const Scalar _tmp74 = -_tmp51 + _tmp60 * _tmp73;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp60 * _tmp69 - _tmp71);
  const Scalar _tmp76 = _tmp74 * _tmp75;
  const Scalar _tmp77 = _tmp40 * _tmp51 - _tmp61 * _tmp73 - _tmp72 * _tmp76;
  const Scalar _tmp78 = _tmp52 * _tmp69 - _tmp69 * _tmp70;
  const Scalar _tmp79 = Scalar(1.0) * _tmp57;
  const Scalar _tmp80 = -_tmp79;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp62 + _tmp80);
  const Scalar _tmp82 = Scalar(1.0) * _tmp53;
  const Scalar _tmp83 = -_tmp65 + _tmp82;
  const Scalar _tmp84 = _tmp81 * _tmp83;
  const Scalar _tmp85 = -_tmp40 * _tmp73 + _tmp52 * _tmp73 - _tmp76 * _tmp78 - _tmp77 * _tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp85);
  const Scalar _tmp87 = _tmp79 * _tmp84 + _tmp82;
  const Scalar _tmp88 = 0;
  const Scalar _tmp89 = _tmp47 + _tmp80;
  const Scalar _tmp90 = _tmp84 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) / (-_tmp42 + _tmp82 - _tmp90);
  const Scalar _tmp92 = _tmp87 * _tmp91;
  const Scalar _tmp93 = -_tmp77 * _tmp88 + _tmp80 - _tmp89 * _tmp92;
  const Scalar _tmp94 = Scalar(1.0) * _tmp81;
  const Scalar _tmp95 = Scalar(1.0) * _tmp91;
  const Scalar _tmp96 = _tmp84 * _tmp95;
  const Scalar _tmp97 = _tmp90 * _tmp95 + Scalar(1.0);
  const Scalar _tmp98 = _tmp29 + Scalar(-4.8333311099999996);
  const Scalar _tmp99 = _tmp19 + Scalar(1.79662371);
  const Scalar _tmp100 = std::pow(Scalar(std::pow(_tmp98, Scalar(2)) + std::pow(_tmp99, Scalar(2))),
                                  Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp101 = _tmp100 * _tmp98;
  const Scalar _tmp102 = _tmp37 * fh1;
  const Scalar _tmp103 = -_tmp101 * _tmp102 - Scalar(3.29616) * _tmp25 - _tmp28 * fv1;
  const Scalar _tmp104 =
      std::sqrt(Scalar(std::pow(_tmp55, Scalar(2)) + std::pow(_tmp59, Scalar(2))));
  const Scalar _tmp105 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp106 = _tmp104 * _tmp56;
  const Scalar _tmp107 = _tmp106 * (_tmp105 * _tmp53 * _tmp59 - _tmp105 * _tmp55 * _tmp57);
  const Scalar _tmp108 = _tmp60 * _tmp75;
  const Scalar _tmp109 = _tmp108 * _tmp72 + _tmp61;
  const Scalar _tmp110 = _tmp108 * _tmp78 - _tmp109 * _tmp84 - _tmp52;
  const Scalar _tmp111 = _tmp107 * _tmp69 + _tmp62 * _tmp69 - _tmp65 * _tmp71;
  const Scalar _tmp112 = _tmp107 * _tmp73 - _tmp111 * _tmp76 - _tmp42 * _tmp51 + _tmp47 * _tmp73;
  const Scalar _tmp113 = _tmp112 * _tmp86;
  const Scalar _tmp114 = Scalar(1.0) / (_tmp112);
  const Scalar _tmp115 = _tmp114 * _tmp85;
  const Scalar _tmp116 = _tmp115 * (-_tmp107 + _tmp108 * _tmp111 - _tmp110 * _tmp113);
  const Scalar _tmp117 = _tmp86 * (_tmp110 + _tmp116);
  const Scalar _tmp118 = _tmp89 * _tmp91;
  const Scalar _tmp119 = _tmp109 + _tmp116 * _tmp118 - _tmp117 * _tmp77;
  const Scalar _tmp120 = _tmp100 * _tmp99;
  const Scalar _tmp121 = _tmp120 * fh1;
  const Scalar _tmp122 = Scalar(1.0) * _tmp75;
  const Scalar _tmp123 = -_tmp122 * _tmp78 + _tmp72 * _tmp75 * _tmp83 * _tmp94;
  const Scalar _tmp124 = _tmp115 * (-_tmp111 * _tmp122 - _tmp113 * _tmp123);
  const Scalar _tmp125 = _tmp86 * (_tmp123 + _tmp124);
  const Scalar _tmp126 = _tmp118 * _tmp124 - _tmp122 * _tmp72 - _tmp125 * _tmp77;
  const Scalar _tmp127 = _tmp101 * fh1;
  const Scalar _tmp128 = _tmp115 * _tmp95;
  const Scalar _tmp129 = Scalar(1.0) * _tmp114;
  const Scalar _tmp130 = _tmp128 * _tmp89 - _tmp129 * _tmp77;
  const Scalar _tmp131 = fh1 * (_tmp101 * _tmp18 - _tmp120 * _tmp28);
  const Scalar _tmp132 = _tmp102 * _tmp120 + Scalar(3.29616) * _tmp12 + _tmp18 * fv1;
  const Scalar _tmp133 = _tmp81 * _tmp89;
  const Scalar _tmp134 = -_tmp125 * _tmp74 + Scalar(1.0);
  const Scalar _tmp135 = _tmp69 * _tmp75;
  const Scalar _tmp136 = -_tmp117 * _tmp74 - _tmp60;
  const Scalar _tmp137 = _tmp114 * _tmp122 * _tmp74;
  const Scalar _tmp138 = -_tmp106 * _tmp121 * (_tmp117 * _tmp73 + _tmp135 * _tmp136 + Scalar(1.0)) -
                         _tmp106 * _tmp127 * (_tmp125 * _tmp73 + _tmp134 * _tmp135) -
                         _tmp106 * _tmp131 * (_tmp129 * _tmp73 - _tmp137 * _tmp69) -
                         _tmp106 * _tmp38 * (-_tmp69 * _tmp76 * _tmp88 + _tmp73 * _tmp88);
  const Scalar _tmp139 = Scalar(1.0) / (_tmp138);
  const Scalar _tmp140 = std::asinh(
      _tmp139 * (Scalar(1.0) * _tmp103 * (-_tmp94 * _tmp97 + _tmp96) +
                 Scalar(1.0) * _tmp121 * (_tmp116 * _tmp95 - _tmp119 * _tmp94) +
                 Scalar(1.0) * _tmp127 * (_tmp124 * _tmp95 - _tmp126 * _tmp94) +
                 Scalar(1.0) * _tmp131 * (_tmp128 - _tmp130 * _tmp94) +
                 Scalar(1.0) * _tmp132 * (_tmp133 * _tmp95 - _tmp95) +
                 Scalar(1.0) * _tmp38 * (-_tmp87 * _tmp95 - _tmp93 * _tmp94 + Scalar(1.0))));
  const Scalar _tmp141 = Scalar(9.6622558468725703) * _tmp138;
  const Scalar _tmp142 = _tmp38 * _tmp88;
  const Scalar _tmp143 = _tmp121 * _tmp136 * _tmp75 + _tmp127 * _tmp134 * _tmp75 -
                         _tmp131 * _tmp137 - _tmp142 * _tmp76;
  const Scalar _tmp144 = Scalar(1.0) / (_tmp143);
  const Scalar _tmp145 = _tmp132 * _tmp95;
  const Scalar _tmp146 =
      std::asinh(_tmp144 * (_tmp103 * _tmp81 * _tmp97 + _tmp119 * _tmp121 * _tmp81 +
                            _tmp126 * _tmp127 * _tmp81 + _tmp130 * _tmp131 * _tmp81 -
                            _tmp133 * _tmp145 + _tmp38 * _tmp81 * _tmp93));
  const Scalar _tmp147 = Scalar(9.6622558468725703) * _tmp143;
  const Scalar _tmp148 = _tmp117 * _tmp121 + _tmp125 * _tmp127 + _tmp129 * _tmp131 + _tmp142;
  const Scalar _tmp149 = Scalar(1.0) / (_tmp148);
  const Scalar _tmp150 = std::asinh(_tmp149 * (-_tmp103 * _tmp96 - _tmp116 * _tmp121 * _tmp91 -
                                               _tmp124 * _tmp127 * _tmp91 - _tmp128 * _tmp131 +
                                               _tmp145 + _tmp38 * _tmp92));
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp148;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp31 *
          (Scalar(0.86625939559540499) * _tmp0 + std::cosh(Scalar(1.0) * _tmp30) -
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp30 * _tmp31 -
                Scalar(4.8333311099999996) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp29), Scalar(2)) +
                        Scalar(0.13817235445745474) *
                            std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp19 - 1),
                                     Scalar(2))))))) +
      _tmp37 + p_init2;
  _res(1, 0) =
      -_tmp141 *
          (Scalar(0.86565325453551001) * _tmp139 + std::cosh(Scalar(1.0) * _tmp140) -
           std::cosh(
               Scalar(0.1034955) * _tmp139 *
               (-_tmp140 * _tmp141 -
                Scalar(4.7752063900000001) *
                    std::sqrt(Scalar(
                        Scalar(0.32397683292140877) *
                            std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp54), Scalar(2)) +
                        std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp58), Scalar(2))))))) +
      _tmp52 + p_init2;
  _res(2, 0) =
      -_tmp147 *
          (Scalar(0.87679799772039002) * _tmp144 + std::cosh(Scalar(1.0) * _tmp146) -
           std::cosh(
               Scalar(0.1034955) * _tmp144 *
               (-_tmp146 * _tmp147 -
                Scalar(8.3196563700000006) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp63 - 1), Scalar(2)) +
                        Scalar(0.057067943376852184) *
                            std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp66 - 1),
                                     Scalar(2))))))) +
      _tmp70 + p_init2;
  _res(3, 0) =
      -_tmp151 *
          (Scalar(0.87653584775870996) * _tmp149 + std::cosh(Scalar(1.0) * _tmp150) -
           std::cosh(
               Scalar(0.1034955) * _tmp149 *
               (-_tmp150 * _tmp151 -
                Scalar(8.3888750099999996) *
                    std::sqrt(Scalar(
                        Scalar(0.090254729040973036) *
                            std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp43), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp48 - 1),
                                 Scalar(2))))))) +
      _tmp40 + p_init2;

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
