// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl1(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot, const Scalar p_init0,
    const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x, const Scalar rot_init_y,
    const Scalar rot_init_z, const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 646

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = _tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp3 * _tmp8;
  const Scalar _tmp15 = _tmp1 * _tmp10;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 - _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + p_init0;
  const Scalar _tmp20 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp20 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp24 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp25 = _tmp10 * _tmp7;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp23 + _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + p_init1;
  const Scalar _tmp31 = _tmp0 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -Scalar(0.1034955) * _tmp33 * fh1 -
      Scalar(0.86104699584133515) *
          std::sqrt(
              Scalar(Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp19 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp30 - 1), Scalar(2))));
  const Scalar _tmp35 = _tmp0 * _tmp34;
  const Scalar _tmp36 = Scalar(1.0) * _tmp32;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = _tmp17 + _tmp5;
  const Scalar _tmp40 = _tmp39 + p_init0;
  const Scalar _tmp41 = _tmp23 + _tmp27;
  const Scalar _tmp42 = _tmp22 + _tmp41;
  const Scalar _tmp43 = _tmp42 + p_init1;
  const Scalar _tmp44 = Scalar(1.0) * _tmp39;
  const Scalar _tmp45 = _tmp13 + _tmp16;
  const Scalar _tmp46 = _tmp45 + _tmp5;
  const Scalar _tmp47 = Scalar(1.0) * _tmp42;
  const Scalar _tmp48 = -_tmp47;
  const Scalar _tmp49 = _tmp21 + _tmp41;
  const Scalar _tmp50 = _tmp48 + _tmp49;
  const Scalar _tmp51 = _tmp21 + _tmp28;
  const Scalar _tmp52 = Scalar(1.0) / (_tmp48 + _tmp51);
  const Scalar _tmp53 = _tmp45 + _tmp6;
  const Scalar _tmp54 = _tmp44 - _tmp53;
  const Scalar _tmp55 = _tmp52 * _tmp54;
  const Scalar _tmp56 = _tmp50 * _tmp55;
  const Scalar _tmp57 = Scalar(1.0) / (_tmp44 - _tmp46 - _tmp56);
  const Scalar _tmp58 = Scalar(1.0) * _tmp57;
  const Scalar _tmp59 = _tmp51 + p_init1;
  const Scalar _tmp60 = _tmp59 + Scalar(-4.8333311099999996);
  const Scalar _tmp61 = _tmp53 + p_init0;
  const Scalar _tmp62 = _tmp61 + Scalar(1.79662371);
  const Scalar _tmp63 = std::pow(Scalar(std::pow(_tmp60, Scalar(2)) + std::pow(_tmp62, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp64 = _tmp60 * _tmp63;
  const Scalar _tmp65 = _tmp62 * _tmp63;
  const Scalar _tmp66 = _tmp40 + Scalar(-2.5202214700000001);
  const Scalar _tmp67 = _tmp43 + Scalar(8.3888750099999996);
  const Scalar _tmp68 =
      std::sqrt(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp67, Scalar(2))));
  const Scalar _tmp69 = Scalar(1.0) / (_tmp68);
  const Scalar _tmp70 = Scalar(1.0) / (_tmp66);
  const Scalar _tmp71 = _tmp68 * _tmp70;
  const Scalar _tmp72 = _tmp71 * (_tmp39 * _tmp67 * _tmp69 - _tmp42 * _tmp66 * _tmp69);
  const Scalar _tmp73 = _tmp51 * _tmp65 - _tmp53 * _tmp64 + _tmp65 * _tmp72;
  const Scalar _tmp74 = _tmp67 * _tmp70;
  const Scalar _tmp75 = Scalar(1.0) / (-_tmp64 + _tmp65 * _tmp74);
  const Scalar _tmp76 = _tmp74 * _tmp75;
  const Scalar _tmp77 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp78 =
      -Scalar(0.010999999999999999) * _tmp20 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp79 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp80 = _tmp78 - _tmp79;
  const Scalar _tmp81 = _tmp77 + _tmp80;
  const Scalar _tmp82 = _tmp74 * _tmp81;
  const Scalar _tmp83 = _tmp65 * _tmp81;
  const Scalar _tmp84 = -_tmp77;
  const Scalar _tmp85 = _tmp78 + _tmp79;
  const Scalar _tmp86 = _tmp84 + _tmp85;
  const Scalar _tmp87 = _tmp64 * _tmp86 - _tmp74 * _tmp83;
  const Scalar _tmp88 = _tmp76 * _tmp87 + _tmp82;
  const Scalar _tmp89 = -_tmp65 * _tmp86 + _tmp83;
  const Scalar _tmp90 = -_tmp55 * _tmp88 + _tmp76 * _tmp89 - _tmp81;
  const Scalar _tmp91 = _tmp46 + p_init0;
  const Scalar _tmp92 = _tmp91 + Scalar(-2.71799795);
  const Scalar _tmp93 = _tmp49 + p_init1;
  const Scalar _tmp94 = _tmp93 + Scalar(-4.7752063900000001);
  const Scalar _tmp95 = std::pow(Scalar(std::pow(_tmp92, Scalar(2)) + std::pow(_tmp94, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp96 = _tmp92 * _tmp95;
  const Scalar _tmp97 = _tmp94 * _tmp95;
  const Scalar _tmp98 = _tmp74 * _tmp96 - _tmp97;
  const Scalar _tmp99 = _tmp75 * _tmp98;
  const Scalar _tmp100 = -_tmp46 * _tmp97 + _tmp49 * _tmp96 + _tmp72 * _tmp96 - _tmp73 * _tmp99;
  const Scalar _tmp101 = _tmp77 + _tmp85;
  const Scalar _tmp102 = _tmp101 * _tmp97 - _tmp82 * _tmp96 - _tmp87 * _tmp99;
  const Scalar _tmp103 = -_tmp101 * _tmp96 - _tmp102 * _tmp55 + _tmp81 * _tmp96 - _tmp89 * _tmp99;
  const Scalar _tmp104 = Scalar(1.0) / (_tmp103);
  const Scalar _tmp105 = _tmp100 * _tmp104;
  const Scalar _tmp106 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp107 = _tmp103 * _tmp106;
  const Scalar _tmp108 = _tmp107 * (-_tmp105 * _tmp90 - _tmp72 + _tmp73 * _tmp76);
  const Scalar _tmp109 = _tmp108 + _tmp90;
  const Scalar _tmp110 = _tmp102 * _tmp104;
  const Scalar _tmp111 = _tmp50 * _tmp57;
  const Scalar _tmp112 = _tmp108 * _tmp111 - _tmp109 * _tmp110 + _tmp88;
  const Scalar _tmp113 = Scalar(1.0) * _tmp52;
  const Scalar _tmp114 = _tmp30 + Scalar(8.3196563700000006);
  const Scalar _tmp115 = _tmp19 + Scalar(1.9874742000000001);
  const Scalar _tmp116 =
      std::pow(Scalar(std::pow(_tmp114, Scalar(2)) + std::pow(_tmp115, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp117 = _tmp115 * _tmp116;
  const Scalar _tmp118 = Scalar(1.0) * _tmp117 * (_tmp108 * _tmp58 - _tmp112 * _tmp113);
  const Scalar _tmp119 = _tmp80 + _tmp84;
  const Scalar _tmp120 = _tmp119 * fh1;
  const Scalar _tmp121 = _tmp117 * _tmp120 + Scalar(3.29616) * _tmp12 + _tmp18 * fv1;
  const Scalar _tmp122 = _tmp50 * _tmp52;
  const Scalar _tmp123 = Scalar(1.0) * _tmp122 * _tmp58 - Scalar(1.0) * _tmp58;
  const Scalar _tmp124 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp125 = _tmp44 + _tmp47 * _tmp55;
  const Scalar _tmp126 = _tmp125 * _tmp57;
  const Scalar _tmp127 = 0;
  const Scalar _tmp128 = -_tmp110 * _tmp127 - _tmp126 * _tmp50 + _tmp48;
  const Scalar _tmp129 = _tmp114 * _tmp116;
  const Scalar _tmp130 = -_tmp120 * _tmp129 - Scalar(3.29616) * _tmp26 - _tmp29 * fv1;
  const Scalar _tmp131 = _tmp56 * _tmp58 + Scalar(1.0);
  const Scalar _tmp132 = _tmp55 * _tmp58;
  const Scalar _tmp133 = -Scalar(1.0) * _tmp113 * _tmp131 + Scalar(1.0) * _tmp132;
  const Scalar _tmp134 = Scalar(1.0) * _tmp75;
  const Scalar _tmp135 = _tmp113 * _tmp54 * _tmp75 * _tmp87 - _tmp134 * _tmp89;
  const Scalar _tmp136 = _tmp107 * (-_tmp105 * _tmp135 - _tmp134 * _tmp73);
  const Scalar _tmp137 = _tmp135 + _tmp136;
  const Scalar _tmp138 = -_tmp110 * _tmp137 + _tmp111 * _tmp136 - _tmp134 * _tmp87;
  const Scalar _tmp139 = Scalar(1.0) * _tmp129 * (-_tmp113 * _tmp138 + _tmp136 * _tmp58);
  const Scalar _tmp140 = _tmp107 * _tmp58;
  const Scalar _tmp141 = Scalar(1.0) * _tmp106;
  const Scalar _tmp142 = -_tmp102 * _tmp141 + _tmp140 * _tmp50;
  const Scalar _tmp143 = -_tmp117 * _tmp29 + _tmp129 * _tmp18;
  const Scalar _tmp144 = Scalar(1.0) * _tmp143;
  const Scalar _tmp145 = _tmp144 * (-_tmp113 * _tmp142 + _tmp140);
  const Scalar _tmp146 =
      _tmp118 * fh1 + _tmp121 * _tmp123 +
      Scalar(1.0) * _tmp124 * (-_tmp113 * _tmp128 - _tmp125 * _tmp58 + Scalar(1.0)) +
      _tmp130 * _tmp133 + _tmp139 * fh1 + _tmp145 * fh1;
  const Scalar _tmp147 = _tmp104 * _tmp96;
  const Scalar _tmp148 = _tmp104 * _tmp98;
  const Scalar _tmp149 = _tmp75 * (-_tmp137 * _tmp148 + Scalar(1.0));
  const Scalar _tmp150 = _tmp129 * _tmp71 * (_tmp137 * _tmp147 + _tmp149 * _tmp65);
  const Scalar _tmp151 = _tmp104 * _tmp127;
  const Scalar _tmp152 = _tmp65 * _tmp99;
  const Scalar _tmp153 = _tmp75 * (-_tmp109 * _tmp148 - _tmp74);
  const Scalar _tmp154 = _tmp117 * _tmp71 * (_tmp109 * _tmp147 + _tmp153 * _tmp65 + Scalar(1.0));
  const Scalar _tmp155 = _tmp143 * _tmp71 * (-_tmp141 * _tmp152 + _tmp141 * _tmp96);
  const Scalar _tmp156 = -_tmp124 * _tmp71 * (-_tmp151 * _tmp152 + _tmp151 * _tmp96) -
                         _tmp150 * fh1 - _tmp154 * fh1 - _tmp155 * fh1;
  const Scalar _tmp157 = Scalar(1.0) / (_tmp156);
  const Scalar _tmp158 = std::asinh(_tmp146 * _tmp157);
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp156;
  const Scalar _tmp160 =
      -_tmp158 * _tmp159 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp40), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp43 - 1), Scalar(2))));
  const Scalar _tmp161 = Scalar(0.1034955) * _tmp157;
  const Scalar _tmp162 = _tmp160 * _tmp161;
  const Scalar _tmp163 = Scalar(1.0) * _tmp158;
  const Scalar _tmp164 = -_tmp150 - _tmp154 - _tmp155;
  const Scalar _tmp165 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp166 = std::pow(_tmp156, Scalar(-2));
  const Scalar _tmp167 = _tmp164 * _tmp166;
  const Scalar _tmp168 = _tmp117 * _tmp119;
  const Scalar _tmp169 = _tmp119 * _tmp129;
  const Scalar _tmp170 = (-_tmp146 * _tmp167 + _tmp157 * (_tmp118 + _tmp123 * _tmp168 -
                                                          _tmp133 * _tmp169 + _tmp139 + _tmp145)) /
                         std::sqrt(Scalar(std::pow(_tmp146, Scalar(2)) * _tmp166 + 1));
  const Scalar _tmp171 = _tmp106 * _tmp144;
  const Scalar _tmp172 = _tmp171 * fh1;
  const Scalar _tmp173 = _tmp117 * _tmp153;
  const Scalar _tmp174 = _tmp124 * _tmp151;
  const Scalar _tmp175 = _tmp129 * _tmp149;
  const Scalar _tmp176 = -_tmp172 * _tmp99 + _tmp173 * fh1 - _tmp174 * _tmp99 + _tmp175 * fh1;
  const Scalar _tmp177 = Scalar(1.0) / (_tmp176);
  const Scalar _tmp178 = _tmp129 * _tmp138 * _tmp52;
  const Scalar _tmp179 = _tmp142 * _tmp143 * _tmp52;
  const Scalar _tmp180 = _tmp117 * _tmp52;
  const Scalar _tmp181 = _tmp112 * _tmp180;
  const Scalar _tmp182 = _tmp131 * _tmp52;
  const Scalar _tmp183 = _tmp121 * _tmp58;
  const Scalar _tmp184 = -_tmp122 * _tmp183 + _tmp124 * _tmp128 * _tmp52 + _tmp130 * _tmp182 +
                         _tmp178 * fh1 + _tmp179 * fh1 + _tmp181 * fh1;
  const Scalar _tmp185 = std::asinh(_tmp177 * _tmp184);
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp185;
  const Scalar _tmp187 =
      -_tmp176 * _tmp186 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp59), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp61 - 1), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp177;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(1.0) * _tmp185;
  const Scalar _tmp191 = -_tmp171 * _tmp99 + _tmp173 + _tmp175;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp176;
  const Scalar _tmp193 = std::pow(_tmp176, Scalar(-2));
  const Scalar _tmp194 = _tmp191 * _tmp193;
  const Scalar _tmp195 = (_tmp177 * (-_tmp119 * _tmp180 * _tmp50 * _tmp58 - _tmp169 * _tmp182 +
                                     _tmp178 + _tmp179 + _tmp181) -
                          _tmp184 * _tmp194) /
                         std::sqrt(Scalar(std::pow(_tmp184, Scalar(2)) * _tmp193 + 1));
  const Scalar _tmp196 = _tmp104 * _tmp129 * _tmp137;
  const Scalar _tmp197 = _tmp104 * _tmp109 * _tmp117;
  const Scalar _tmp198 = _tmp172 + _tmp174 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp199 = Scalar(1.0) / (_tmp198);
  const Scalar _tmp200 = _tmp140 * _tmp143;
  const Scalar _tmp201 = _tmp108 * _tmp117 * _tmp57;
  const Scalar _tmp202 = _tmp129 * _tmp136 * _tmp57;
  const Scalar _tmp203 = _tmp124 * _tmp126 - _tmp130 * _tmp132 + _tmp183 - _tmp200 * fh1 -
                         _tmp201 * fh1 - _tmp202 * fh1;
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(1.0) * _tmp204;
  const Scalar _tmp206 = std::pow(_tmp198, Scalar(-2));
  const Scalar _tmp207 = _tmp171 + _tmp196 + _tmp197;
  const Scalar _tmp208 = _tmp206 * _tmp207;
  const Scalar _tmp209 =
      (_tmp199 * (_tmp132 * _tmp169 + _tmp168 * _tmp58 - _tmp200 - _tmp201 - _tmp202) -
       _tmp203 * _tmp208) /
      std::sqrt(Scalar(std::pow(_tmp203, Scalar(2)) * _tmp206 + 1));
  const Scalar _tmp210 = Scalar(9.6622558468725703) * _tmp204;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp198;
  const Scalar _tmp212 = Scalar(0.1034955) * _tmp199;
  const Scalar _tmp213 =
      -_tmp198 * _tmp210 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp91), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp93), Scalar(2))));
  const Scalar _tmp214 = _tmp212 * _tmp213;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::cosh(_tmp36) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp31 * _tmp38 - _tmp33) -
            _tmp34 * _tmp37) *
               std::cosh(_tmp35)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp35) -
      Scalar(9.6622558468725703) * std::sinh(_tmp36);
  _res(1, 0) = _tmp159 * (-Scalar(1.0) * _tmp170 * std::cosh(_tmp163) -
                          (-Scalar(0.1034955) * _tmp160 * _tmp167 +
                           _tmp161 * (-_tmp158 * _tmp165 - _tmp159 * _tmp170)) *
                              std::cosh(_tmp162)) +
               _tmp165 * (-std::sinh(_tmp162) - std::sinh(_tmp163));
  _res(2, 0) = Scalar(9.6622558468725703) * _tmp191 * (-std::sinh(_tmp189) - std::sinh(_tmp190)) +
               _tmp192 * (-Scalar(1.0) * _tmp195 * std::cosh(_tmp190) -
                          (-Scalar(0.1034955) * _tmp187 * _tmp194 +
                           _tmp188 * (-_tmp186 * _tmp191 - _tmp192 * _tmp195)) *
                              std::cosh(_tmp189));
  _res(3, 0) = Scalar(9.6622558468725703) * _tmp207 * (-std::sinh(_tmp205) - std::sinh(_tmp214)) +
               _tmp211 * (-Scalar(1.0) * _tmp209 * std::cosh(_tmp205) -
                          (-Scalar(0.1034955) * _tmp208 * _tmp213 +
                           _tmp212 * (-_tmp207 * _tmp210 - _tmp209 * _tmp211)) *
                              std::cosh(_tmp214));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
