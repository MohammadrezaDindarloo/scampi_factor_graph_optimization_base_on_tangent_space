// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl14
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl14(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot, const Scalar p_init0,
    const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x, const Scalar rot_init_y,
    const Scalar rot_init_z, const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 594

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (187)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp7 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp6;
  const Scalar _tmp11 = _tmp10 * _tmp4;
  const Scalar _tmp12 = _tmp3 * _tmp7;
  const Scalar _tmp13 = _tmp11 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + p_init0;
  const Scalar _tmp21 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp15 +
                        Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999);
  const Scalar _tmp23 = _tmp10 * _tmp3;
  const Scalar _tmp24 = _tmp4 * _tmp7;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + p_init1;
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = -_tmp9;
  const Scalar _tmp34 = _tmp18 + _tmp33;
  const Scalar _tmp35 = _tmp34 + p_init0;
  const Scalar _tmp36 = -_tmp22;
  const Scalar _tmp37 = _tmp29 + _tmp36;
  const Scalar _tmp38 = _tmp37 + p_init1;
  const Scalar _tmp39 = -_tmp28;
  const Scalar _tmp40 = _tmp22 + _tmp27 + _tmp39;
  const Scalar _tmp41 = _tmp40 + p_init1;
  const Scalar _tmp42 = _tmp41 + Scalar(-4.8333311099999996);
  const Scalar _tmp43 = _tmp14 - _tmp17;
  const Scalar _tmp44 = _tmp43 + _tmp9;
  const Scalar _tmp45 = _tmp44 + p_init0;
  const Scalar _tmp46 = _tmp45 + Scalar(1.79662371);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp42, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp42 * _tmp47;
  const Scalar _tmp49 = _tmp35 + Scalar(-2.5202214700000001);
  const Scalar _tmp50 = Scalar(1.0) / (_tmp49);
  const Scalar _tmp51 = _tmp38 + Scalar(8.3888750099999996);
  const Scalar _tmp52 = _tmp50 * _tmp51;
  const Scalar _tmp53 = _tmp46 * _tmp47;
  const Scalar _tmp54 = -_tmp48 + _tmp52 * _tmp53;
  const Scalar _tmp55 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp56 =
      -Scalar(0.010999999999999999) * _tmp16 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp57 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp58 = _tmp56 - _tmp57;
  const Scalar _tmp59 = _tmp55 + _tmp58;
  const Scalar _tmp60 = _tmp53 * _tmp59;
  const Scalar _tmp61 = -_tmp55;
  const Scalar _tmp62 = _tmp56 + _tmp57;
  const Scalar _tmp63 = _tmp61 + _tmp62;
  const Scalar _tmp64 = _tmp36 + _tmp39;
  const Scalar _tmp65 = _tmp27 + _tmp64;
  const Scalar _tmp66 = _tmp65 + p_init1;
  const Scalar _tmp67 = _tmp66 + Scalar(8.3196563700000006);
  const Scalar _tmp68 = _tmp33 + _tmp43;
  const Scalar _tmp69 = _tmp68 + p_init0;
  const Scalar _tmp70 = _tmp69 + Scalar(1.9874742000000001);
  const Scalar _tmp71 = std::pow(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp70, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp72 = _tmp67 * _tmp71;
  const Scalar _tmp73 = _tmp70 * _tmp71;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp52 * _tmp73 - _tmp72);
  const Scalar _tmp75 = _tmp58 + _tmp61;
  const Scalar _tmp76 = _tmp59 * _tmp73;
  const Scalar _tmp77 = _tmp74 * (-_tmp52 * _tmp76 + _tmp72 * _tmp75);
  const Scalar _tmp78 = _tmp48 * _tmp63 - _tmp52 * _tmp60 - _tmp54 * _tmp77;
  const Scalar _tmp79 = Scalar(1.0) * _tmp37;
  const Scalar _tmp80 = -_tmp79;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp65 + _tmp80);
  const Scalar _tmp82 = Scalar(1.0) * _tmp34;
  const Scalar _tmp83 = _tmp81 * (-_tmp68 + _tmp82);
  const Scalar _tmp84 = _tmp74 * (-_tmp73 * _tmp75 + _tmp76);
  const Scalar _tmp85 = -_tmp53 * _tmp63 - _tmp54 * _tmp84 + _tmp60 - _tmp78 * _tmp83;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp85);
  const Scalar _tmp87 = _tmp52 * _tmp59 + _tmp52 * _tmp77;
  const Scalar _tmp88 = _tmp52 * _tmp84 - _tmp59 - _tmp83 * _tmp87;
  const Scalar _tmp89 =
      std::sqrt(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp51, Scalar(2))));
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 = _tmp50 * _tmp89;
  const Scalar _tmp92 = _tmp91 * (_tmp34 * _tmp51 * _tmp90 - _tmp37 * _tmp49 * _tmp90);
  const Scalar _tmp93 = _tmp74 * (_tmp65 * _tmp73 - _tmp68 * _tmp72 + _tmp73 * _tmp92);
  const Scalar _tmp94 = _tmp40 * _tmp53 - _tmp44 * _tmp48 + _tmp53 * _tmp92 - _tmp54 * _tmp93;
  const Scalar _tmp95 = _tmp86 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp97 = _tmp85 * _tmp96;
  const Scalar _tmp98 = _tmp97 * (_tmp52 * _tmp93 - _tmp88 * _tmp95 - _tmp92);
  const Scalar _tmp99 = _tmp86 * (_tmp88 + _tmp98);
  const Scalar _tmp100 = -_tmp52 - _tmp54 * _tmp99;
  const Scalar _tmp101 = _tmp73 * _tmp74;
  const Scalar _tmp102 = _tmp20 + Scalar(-2.71799795);
  const Scalar _tmp103 = _tmp31 + Scalar(-4.7752063900000001);
  const Scalar _tmp104 =
      std::pow(Scalar(std::pow(_tmp102, Scalar(2)) + std::pow(_tmp103, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp105 = _tmp102 * _tmp104;
  const Scalar _tmp106 = _tmp105 * fh1;
  const Scalar _tmp107 = Scalar(1.0) * _tmp77;
  const Scalar _tmp108 = _tmp107 * _tmp83 - Scalar(1.0) * _tmp84;
  const Scalar _tmp109 = _tmp97 * (-_tmp108 * _tmp95 - Scalar(1.0) * _tmp93);
  const Scalar _tmp110 = _tmp86 * (_tmp108 + _tmp109);
  const Scalar _tmp111 = -_tmp110 * _tmp54 + Scalar(1.0);
  const Scalar _tmp112 = _tmp103 * _tmp104;
  const Scalar _tmp113 = _tmp112 * fh1;
  const Scalar _tmp114 = Scalar(1.0) * _tmp96;
  const Scalar _tmp115 = _tmp101 * _tmp54;
  const Scalar _tmp116 = fh1 * (-_tmp105 * _tmp30 + _tmp112 * _tmp19);
  const Scalar _tmp117 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp118 = _tmp79 * _tmp83 + _tmp82;
  const Scalar _tmp119 = 0;
  const Scalar _tmp120 = _tmp91 * (-_tmp115 * _tmp119 + _tmp119 * _tmp53);
  const Scalar _tmp121 = -_tmp106 * _tmp91 * (_tmp100 * _tmp101 + _tmp53 * _tmp99 + Scalar(1.0)) -
                         _tmp113 * _tmp91 * (_tmp101 * _tmp111 + _tmp110 * _tmp53) -
                         _tmp116 * _tmp91 * (-_tmp114 * _tmp115 + _tmp114 * _tmp53) -
                         _tmp117 * _tmp120;
  const Scalar _tmp122 = Scalar(1.0) / (_tmp121);
  const Scalar _tmp123 = _tmp40 + _tmp80;
  const Scalar _tmp124 = _tmp123 * _tmp83;
  const Scalar _tmp125 = Scalar(1.0) / (-_tmp124 - _tmp44 + _tmp82);
  const Scalar _tmp126 = _tmp118 * _tmp125;
  const Scalar _tmp127 = _tmp81 * (-_tmp119 * _tmp78 - _tmp123 * _tmp126 + _tmp80);
  const Scalar _tmp128 = -Scalar(1.0) * _tmp126 - Scalar(1.0) * _tmp127 + Scalar(1.0);
  const Scalar _tmp129 = Scalar(1.0) * _tmp125;
  const Scalar _tmp130 = _tmp123 * _tmp125;
  const Scalar _tmp131 = -_tmp107 + _tmp109 * _tmp130 - _tmp110 * _tmp78;
  const Scalar _tmp132 = Scalar(1.0) * _tmp81;
  const Scalar _tmp133 = fh1 * (_tmp55 + _tmp62);
  const Scalar _tmp134 = -_tmp112 * _tmp133 - Scalar(3.29616) * _tmp25 - _tmp30 * fv1;
  const Scalar _tmp135 = _tmp81 * (_tmp124 * _tmp129 + Scalar(1.0));
  const Scalar _tmp136 = _tmp129 * _tmp83;
  const Scalar _tmp137 = -Scalar(1.0) * _tmp135 + Scalar(1.0) * _tmp136;
  const Scalar _tmp138 = _tmp129 * _tmp97;
  const Scalar _tmp139 = -_tmp114 * _tmp78 + _tmp123 * _tmp138;
  const Scalar _tmp140 = _tmp105 * _tmp133 + Scalar(3.29616) * _tmp13 + _tmp19 * fv1;
  const Scalar _tmp141 = _tmp123 * _tmp81;
  const Scalar _tmp142 = Scalar(1.0) * _tmp129 * _tmp141 - Scalar(1.0) * _tmp129;
  const Scalar _tmp143 = _tmp130 * _tmp98 - _tmp78 * _tmp99 + _tmp87;
  const Scalar _tmp144 = Scalar(1.0) * _tmp106 * (_tmp129 * _tmp98 - _tmp132 * _tmp143) +
                         Scalar(1.0) * _tmp113 * (_tmp109 * _tmp129 - _tmp131 * _tmp132) +
                         Scalar(1.0) * _tmp116 * (-_tmp132 * _tmp139 + _tmp138) +
                         _tmp117 * _tmp128 + _tmp134 * _tmp137 + _tmp140 * _tmp142;
  const Scalar _tmp145 = std::asinh(_tmp122 * _tmp144);
  const Scalar _tmp146 = Scalar(9.6622558468725703) * _tmp121;
  const Scalar _tmp147 =
      -_tmp145 * _tmp146 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp35), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp38 - 1), Scalar(2))));
  const Scalar _tmp148 = Scalar(0.1034955) * _tmp122;
  const Scalar _tmp149 = _tmp147 * _tmp148;
  const Scalar _tmp150 = Scalar(1.0) * _tmp145;
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp120;
  const Scalar _tmp152 = std::pow(_tmp121, Scalar(-2));
  const Scalar _tmp153 = _tmp120 * _tmp152;
  const Scalar _tmp154 = _tmp26 + _tmp64;
  const Scalar _tmp155 =
      (_tmp122 * (-_tmp128 + _tmp137 * _tmp154 + _tmp142 * _tmp19) - _tmp144 * _tmp153) /
      std::sqrt(Scalar(std::pow(_tmp144, Scalar(2)) * _tmp152 + 1));
  const Scalar _tmp156 = _tmp117 * _tmp119;
  const Scalar _tmp157 = _tmp54 * _tmp74;
  const Scalar _tmp158 = _tmp114 * _tmp116;
  const Scalar _tmp159 = _tmp100 * _tmp106 * _tmp74 + _tmp111 * _tmp113 * _tmp74 -
                         _tmp156 * _tmp157 - _tmp157 * _tmp158;
  const Scalar _tmp160 = Scalar(1.0) / (_tmp159);
  const Scalar _tmp161 = _tmp129 * _tmp140;
  const Scalar _tmp162 = _tmp106 * _tmp143 * _tmp81 + _tmp113 * _tmp131 * _tmp81 +
                         _tmp116 * _tmp139 * _tmp81 + _tmp117 * _tmp127 + _tmp134 * _tmp135 -
                         _tmp141 * _tmp161;
  const Scalar _tmp163 = std::asinh(_tmp160 * _tmp162);
  const Scalar _tmp164 = Scalar(1.0) * _tmp163;
  const Scalar _tmp165 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp166 =
      -_tmp163 * _tmp165 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp66 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp69 - 1), Scalar(2))));
  const Scalar _tmp167 = Scalar(0.1034955) * _tmp160;
  const Scalar _tmp168 = _tmp166 * _tmp167;
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp119;
  const Scalar _tmp170 = _tmp157 * _tmp169;
  const Scalar _tmp171 = _tmp129 * _tmp19;
  const Scalar _tmp172 = std::pow(_tmp159, Scalar(-2));
  const Scalar _tmp173 = _tmp119 * _tmp157 * _tmp172;
  const Scalar _tmp174 =
      (_tmp160 * (-_tmp127 + _tmp135 * _tmp154 - _tmp141 * _tmp171) - _tmp162 * _tmp173) /
      std::sqrt(Scalar(std::pow(_tmp162, Scalar(2)) * _tmp172 + 1));
  const Scalar _tmp175 = -_tmp106 * _tmp125 * _tmp98 - _tmp109 * _tmp113 * _tmp125 -
                         _tmp116 * _tmp138 + _tmp117 * _tmp126 - _tmp134 * _tmp136 + _tmp161;
  const Scalar _tmp176 = _tmp106 * _tmp99 + _tmp110 * _tmp113 + _tmp156 + _tmp158;
  const Scalar _tmp177 = Scalar(1.0) / (_tmp176);
  const Scalar _tmp178 = std::asinh(_tmp175 * _tmp177);
  const Scalar _tmp179 = Scalar(9.6622558468725703) * _tmp176;
  const Scalar _tmp180 =
      -_tmp178 * _tmp179 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp41), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp45 - 1), Scalar(2))));
  const Scalar _tmp181 = Scalar(0.1034955) * _tmp177;
  const Scalar _tmp182 = _tmp180 * _tmp181;
  const Scalar _tmp183 = Scalar(1.0) * _tmp178;
  const Scalar _tmp184 = std::pow(_tmp176, Scalar(-2));
  const Scalar _tmp185 = _tmp119 * _tmp184;
  const Scalar _tmp186 = (_tmp175 * _tmp185 + _tmp177 * (-_tmp126 - _tmp136 * _tmp154 + _tmp171)) /
                         std::sqrt(Scalar(std::pow(_tmp175, Scalar(2)) * _tmp184 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 *
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp32 -
                Scalar(4.7752063900000001) *
                    std::sqrt(Scalar(
                        Scalar(0.32397683292140877) *
                            std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp20), Scalar(2)) +
                        std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp31), Scalar(2)))))));
  _res(1, 0) = _tmp146 * (-Scalar(1.0) * _tmp155 * std::cosh(_tmp150) -
                          (-Scalar(0.1034955) * _tmp147 * _tmp153 +
                           _tmp148 * (-_tmp145 * _tmp151 - _tmp146 * _tmp155)) *
                              std::cosh(_tmp149)) +
               _tmp151 * (-std::sinh(_tmp149) - std::sinh(_tmp150));
  _res(2, 0) = _tmp165 * (-Scalar(1.0) * _tmp174 * std::cosh(_tmp164) -
                          (-Scalar(0.1034955) * _tmp166 * _tmp173 +
                           _tmp167 * (-_tmp163 * _tmp170 - _tmp165 * _tmp174)) *
                              std::cosh(_tmp168)) +
               _tmp170 * (-std::sinh(_tmp164) - std::sinh(_tmp168));
  _res(3, 0) = -_tmp169 * (-std::sinh(_tmp182) - std::sinh(_tmp183)) +
               _tmp179 * (-Scalar(1.0) * _tmp186 * std::cosh(_tmp183) -
                          (Scalar(0.1034955) * _tmp180 * _tmp185 +
                           _tmp181 * (_tmp169 * _tmp178 - _tmp179 * _tmp186)) *
                              std::cosh(_tmp182));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
