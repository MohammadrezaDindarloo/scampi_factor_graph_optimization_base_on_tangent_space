// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl18
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl18(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot, const Scalar p_init0,
    const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x, const Scalar rot_init_y,
    const Scalar rot_init_z, const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 639

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (211)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp2 = 2 * _tmp1;
  const Scalar _tmp3 = _tmp0 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp5 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = 2 * _tmp0 * _tmp4;
  const Scalar _tmp9 = _tmp1 * _tmp5;
  const Scalar _tmp10 = _tmp8 + _tmp9;
  const Scalar _tmp11 = -Scalar(0.010999999999999999) * _tmp10;
  const Scalar _tmp12 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp13 = 1 - 2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp14 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp15 = _tmp11 - _tmp14;
  const Scalar _tmp16 = _tmp15 + _tmp7;
  const Scalar _tmp17 = _tmp16 + p_init0;
  const Scalar _tmp18 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp12 +
                        Scalar(0.20999999999999999) * _tmp18 + Scalar(0.20999999999999999);
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp21 = _tmp2 * _tmp4;
  const Scalar _tmp22 = _tmp0 * _tmp5;
  const Scalar _tmp23 = _tmp21 - _tmp22;
  const Scalar _tmp24 = -Scalar(0.010999999999999999) * _tmp23;
  const Scalar _tmp25 = -_tmp20 + _tmp24;
  const Scalar _tmp26 = _tmp19 + _tmp25;
  const Scalar _tmp27 = _tmp26 + p_init1;
  const Scalar _tmp28 = Scalar(1.0) / (fh1);
  const Scalar _tmp29 = _tmp28 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -_tmp31 * fh1 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp27), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp17 - 1), Scalar(2))));
  const Scalar _tmp33 = Scalar(0.1034955) * _tmp28;
  const Scalar _tmp34 = _tmp32 * _tmp33;
  const Scalar _tmp35 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp36 =
      std::pow(Scalar(_tmp35 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp37 = Scalar(1.0) * _tmp30;
  const Scalar _tmp38 = _tmp27 + Scalar(-4.8333311099999996);
  const Scalar _tmp39 = _tmp17 + Scalar(1.79662371);
  const Scalar _tmp40 = std::pow(Scalar(std::pow(_tmp38, Scalar(2)) + std::pow(_tmp39, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp41 = _tmp38 * _tmp40;
  const Scalar _tmp42 = _tmp39 * _tmp40;
  const Scalar _tmp43 = _tmp16 * _tmp41 - _tmp26 * _tmp42;
  const Scalar _tmp44 = _tmp11 + _tmp14;
  const Scalar _tmp45 = _tmp44 + _tmp7;
  const Scalar _tmp46 = _tmp20 + _tmp24;
  const Scalar _tmp47 = _tmp19 + _tmp46;
  const Scalar _tmp48 = _tmp47 + p_init1;
  const Scalar _tmp49 = _tmp48 + Scalar(-4.7752063900000001);
  const Scalar _tmp50 = _tmp45 + p_init0;
  const Scalar _tmp51 = _tmp50 + Scalar(-2.71799795);
  const Scalar _tmp52 = std::pow(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp51, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp53 = _tmp49 * _tmp52;
  const Scalar _tmp54 = _tmp51 * _tmp52;
  const Scalar _tmp55 = -_tmp19;
  const Scalar _tmp56 = _tmp25 + _tmp55;
  const Scalar _tmp57 = _tmp56 + p_init1;
  const Scalar _tmp58 = _tmp57 + Scalar(8.3196563700000006);
  const Scalar _tmp59 = -_tmp7;
  const Scalar _tmp60 = _tmp15 + _tmp59;
  const Scalar _tmp61 = _tmp60 + p_init0;
  const Scalar _tmp62 = _tmp61 + Scalar(1.9874742000000001);
  const Scalar _tmp63 =
      std::sqrt(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp62, Scalar(2))));
  const Scalar _tmp64 = Scalar(1.0) / (_tmp63);
  const Scalar _tmp65 = Scalar(1.0) / (_tmp62);
  const Scalar _tmp66 = _tmp63 * _tmp65;
  const Scalar _tmp67 = _tmp66 * (-_tmp56 * _tmp62 * _tmp64 + _tmp58 * _tmp60 * _tmp64);
  const Scalar _tmp68 = _tmp44 + _tmp59;
  const Scalar _tmp69 = _tmp68 + p_init0;
  const Scalar _tmp70 = _tmp69 + Scalar(-2.5202214700000001);
  const Scalar _tmp71 = _tmp46 + _tmp55;
  const Scalar _tmp72 = _tmp71 + p_init1;
  const Scalar _tmp73 = _tmp72 + Scalar(8.3888750099999996);
  const Scalar _tmp74 = std::pow(Scalar(std::pow(_tmp70, Scalar(2)) + std::pow(_tmp73, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp75 = _tmp70 * _tmp74;
  const Scalar _tmp76 = _tmp73 * _tmp74;
  const Scalar _tmp77 = _tmp67 * _tmp75 - _tmp68 * _tmp76 + _tmp71 * _tmp75;
  const Scalar _tmp78 = _tmp58 * _tmp65;
  const Scalar _tmp79 = Scalar(1.0) / (_tmp75 * _tmp78 - _tmp76);
  const Scalar _tmp80 = -_tmp53 + _tmp54 * _tmp78;
  const Scalar _tmp81 = _tmp79 * _tmp80;
  const Scalar _tmp82 = -_tmp45 * _tmp53 + _tmp47 * _tmp54 + _tmp54 * _tmp67 - _tmp77 * _tmp81;
  const Scalar _tmp83 = Scalar(1.0) / (_tmp82);
  const Scalar _tmp84 = Scalar(1.0) * _tmp83;
  const Scalar _tmp85 = Scalar(1.0) * _tmp79;
  const Scalar _tmp86 = _tmp80 * _tmp83 * _tmp85;
  const Scalar _tmp87 = _tmp43 * _tmp66 * (_tmp54 * _tmp84 - _tmp75 * _tmp86);
  const Scalar _tmp88 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp89 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp22;
  const Scalar _tmp90 = -_tmp89;
  const Scalar _tmp91 =
      -Scalar(0.010999999999999999) * _tmp13 - Scalar(0.010999999999999999) * _tmp18;
  const Scalar _tmp92 = Scalar(0.20999999999999999) * _tmp8 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp93 = _tmp91 + _tmp92;
  const Scalar _tmp94 = _tmp90 + _tmp93;
  const Scalar _tmp95 = _tmp91 - _tmp92;
  const Scalar _tmp96 = _tmp90 + _tmp95;
  const Scalar _tmp97 = _tmp75 * _tmp96;
  const Scalar _tmp98 = -_tmp75 * _tmp94 + _tmp97;
  const Scalar _tmp99 = _tmp89 + _tmp93;
  const Scalar _tmp100 = _tmp78 * _tmp96;
  const Scalar _tmp101 = _tmp76 * _tmp94 - _tmp78 * _tmp97;
  const Scalar _tmp102 = -_tmp100 * _tmp54 - _tmp101 * _tmp81 + _tmp53 * _tmp99;
  const Scalar _tmp103 = Scalar(1.0) * _tmp56;
  const Scalar _tmp104 = -_tmp103;
  const Scalar _tmp105 = Scalar(1.0) / (_tmp104 + _tmp71);
  const Scalar _tmp106 = Scalar(1.0) * _tmp60;
  const Scalar _tmp107 = _tmp105 * (_tmp106 - _tmp68);
  const Scalar _tmp108 = -_tmp102 * _tmp107 + _tmp54 * _tmp96 - _tmp54 * _tmp99 - _tmp81 * _tmp98;
  const Scalar _tmp109 = Scalar(1.0) / (_tmp108);
  const Scalar _tmp110 = _tmp103 * _tmp107 + _tmp106;
  const Scalar _tmp111 = 0;
  const Scalar _tmp112 = _tmp78 * _tmp79;
  const Scalar _tmp113 = _tmp100 + _tmp101 * _tmp112;
  const Scalar _tmp114 = -_tmp107 * _tmp113 + _tmp112 * _tmp98 - _tmp96;
  const Scalar _tmp115 = _tmp109 * _tmp82;
  const Scalar _tmp116 = _tmp108 * _tmp83;
  const Scalar _tmp117 = _tmp116 * (_tmp112 * _tmp77 - _tmp114 * _tmp115 - _tmp67);
  const Scalar _tmp118 = _tmp109 * (_tmp114 + _tmp117);
  const Scalar _tmp119 = _tmp79 * (-_tmp118 * _tmp80 - _tmp78);
  const Scalar _tmp120 = _tmp42 * _tmp66 * (_tmp118 * _tmp54 + _tmp119 * _tmp75 + Scalar(1.0));
  const Scalar _tmp121 = _tmp101 * _tmp85;
  const Scalar _tmp122 = _tmp107 * _tmp121 - _tmp85 * _tmp98;
  const Scalar _tmp123 = _tmp116 * (-_tmp115 * _tmp122 - _tmp77 * _tmp85);
  const Scalar _tmp124 = _tmp109 * (_tmp122 + _tmp123);
  const Scalar _tmp125 = _tmp79 * (-_tmp124 * _tmp80 + Scalar(1.0));
  const Scalar _tmp126 = _tmp41 * _tmp66 * (_tmp124 * _tmp54 + _tmp125 * _tmp75);
  const Scalar _tmp127 = -_tmp120 * fh1 - _tmp126 * fh1 -
                         _tmp66 * _tmp88 * (_tmp111 * _tmp54 - _tmp111 * _tmp75 * _tmp81) -
                         _tmp87 * fh1;
  const Scalar _tmp128 = _tmp104 + _tmp47;
  const Scalar _tmp129 = _tmp107 * _tmp128;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp106 - _tmp129 - _tmp45);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp116 * _tmp131;
  const Scalar _tmp133 = -_tmp102 * _tmp84 + _tmp128 * _tmp132;
  const Scalar _tmp134 = Scalar(1.0) * _tmp105;
  const Scalar _tmp135 = Scalar(1.0) * _tmp43 * (_tmp132 - _tmp133 * _tmp134);
  const Scalar _tmp136 = _tmp89 + _tmp95;
  const Scalar _tmp137 = _tmp136 * fh1;
  const Scalar _tmp138 = -_tmp137 * _tmp41 - Scalar(3.29616) * _tmp23 - _tmp26 * fv1;
  const Scalar _tmp139 = _tmp129 * _tmp131 + Scalar(1.0);
  const Scalar _tmp140 = _tmp107 * _tmp131;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp134 * _tmp139 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = _tmp128 * _tmp130;
  const Scalar _tmp143 = -_tmp102 * _tmp118 + _tmp113 + _tmp117 * _tmp142;
  const Scalar _tmp144 = Scalar(1.0) * _tmp42 * (_tmp117 * _tmp131 - _tmp134 * _tmp143);
  const Scalar _tmp145 = Scalar(3.29616) * _tmp10 + _tmp137 * _tmp42 + _tmp16 * fv1;
  const Scalar _tmp146 = _tmp105 * _tmp128;
  const Scalar _tmp147 = _tmp131 * _tmp146;
  const Scalar _tmp148 = -Scalar(1.0) * _tmp131 + Scalar(1.0) * _tmp147;
  const Scalar _tmp149 = -_tmp102 * _tmp124 - _tmp121 + _tmp123 * _tmp142;
  const Scalar _tmp150 = Scalar(1.0) * _tmp41 * (_tmp123 * _tmp131 - _tmp134 * _tmp149);
  const Scalar _tmp151 = _tmp110 * _tmp130;
  const Scalar _tmp152 = -_tmp102 * _tmp111 + _tmp104 - _tmp128 * _tmp151;
  const Scalar _tmp153 =
      _tmp135 * fh1 + _tmp138 * _tmp141 + _tmp144 * fh1 + _tmp145 * _tmp148 + _tmp150 * fh1 +
      Scalar(1.0) * _tmp88 * (-_tmp110 * _tmp131 - _tmp134 * _tmp152 + Scalar(1.0));
  const Scalar _tmp154 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp155 = std::asinh(_tmp153 * _tmp154);
  const Scalar _tmp156 = Scalar(9.6622558468725703) * _tmp155;
  const Scalar _tmp157 =
      -_tmp127 * _tmp156 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp57 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp61 - 1), Scalar(2))));
  const Scalar _tmp158 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp159 = -_tmp120 - _tmp126 - _tmp87;
  const Scalar _tmp160 = _tmp158 * _tmp159;
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp162 = _tmp136 * _tmp42;
  const Scalar _tmp163 = _tmp136 * _tmp41;
  const Scalar _tmp164 = (-_tmp153 * _tmp160 + _tmp154 * (_tmp135 - _tmp141 * _tmp163 + _tmp144 +
                                                          _tmp148 * _tmp162 + _tmp150)) /
                         std::sqrt(Scalar(std::pow(_tmp153, Scalar(2)) * _tmp158 + 1));
  const Scalar _tmp165 = Scalar(0.1034955) * _tmp154;
  const Scalar _tmp166 = _tmp157 * _tmp165;
  const Scalar _tmp167 = Scalar(1.0) * _tmp155;
  const Scalar _tmp168 = _tmp43 * _tmp86;
  const Scalar _tmp169 = _tmp119 * _tmp42;
  const Scalar _tmp170 = _tmp111 * _tmp88;
  const Scalar _tmp171 = _tmp125 * _tmp41;
  const Scalar _tmp172 = -_tmp168 * fh1 + _tmp169 * fh1 - _tmp170 * _tmp81 + _tmp171 * fh1;
  const Scalar _tmp173 = Scalar(9.6622558468725703) * _tmp172;
  const Scalar _tmp174 = std::pow(_tmp172, Scalar(-2));
  const Scalar _tmp175 = _tmp105 * _tmp143 * _tmp42;
  const Scalar _tmp176 = _tmp105 * _tmp139;
  const Scalar _tmp177 = _tmp131 * _tmp145;
  const Scalar _tmp178 = _tmp105 * _tmp149 * _tmp41;
  const Scalar _tmp179 = _tmp105 * _tmp133 * _tmp43;
  const Scalar _tmp180 = _tmp105 * _tmp152 * _tmp88 + _tmp138 * _tmp176 - _tmp146 * _tmp177 +
                         _tmp175 * fh1 + _tmp178 * fh1 + _tmp179 * fh1;
  const Scalar _tmp181 = -_tmp168 + _tmp169 + _tmp171;
  const Scalar _tmp182 = _tmp174 * _tmp181;
  const Scalar _tmp183 = Scalar(1.0) / (_tmp172);
  const Scalar _tmp184 = (-_tmp180 * _tmp182 + _tmp183 * (-_tmp147 * _tmp162 - _tmp163 * _tmp176 +
                                                          _tmp175 + _tmp178 + _tmp179)) /
                         std::sqrt(Scalar(_tmp174 * std::pow(_tmp180, Scalar(2)) + 1));
  const Scalar _tmp185 = std::asinh(_tmp180 * _tmp183);
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp183;
  const Scalar _tmp188 =
      -_tmp173 * _tmp185 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp69), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp72 - 1), Scalar(2))));
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(1.0) * _tmp185;
  const Scalar _tmp191 = _tmp43 * _tmp84;
  const Scalar _tmp192 = _tmp118 * _tmp42;
  const Scalar _tmp193 = _tmp124 * _tmp41;
  const Scalar _tmp194 = _tmp170 + _tmp191 * fh1 + _tmp192 * fh1 + _tmp193 * fh1;
  const Scalar _tmp195 = Scalar(1.0) / (_tmp194);
  const Scalar _tmp196 = _tmp123 * _tmp130 * _tmp41;
  const Scalar _tmp197 = _tmp117 * _tmp130 * _tmp42;
  const Scalar _tmp198 = _tmp132 * _tmp43;
  const Scalar _tmp199 = -_tmp138 * _tmp140 + _tmp151 * _tmp88 + _tmp177 - _tmp196 * fh1 -
                         _tmp197 * fh1 - _tmp198 * fh1;
  const Scalar _tmp200 = std::asinh(_tmp195 * _tmp199);
  const Scalar _tmp201 = Scalar(1.0) * _tmp200;
  const Scalar _tmp202 = Scalar(9.6622558468725703) * _tmp194;
  const Scalar _tmp203 =
      -_tmp200 * _tmp202 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp48), Scalar(2)) +
                     Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp50), Scalar(2))));
  const Scalar _tmp204 = Scalar(0.1034955) * _tmp195;
  const Scalar _tmp205 = _tmp203 * _tmp204;
  const Scalar _tmp206 = _tmp191 + _tmp192 + _tmp193;
  const Scalar _tmp207 = Scalar(9.6622558468725703) * _tmp206;
  const Scalar _tmp208 = std::pow(_tmp194, Scalar(-2));
  const Scalar _tmp209 = _tmp206 * _tmp208;
  const Scalar _tmp210 =
      (_tmp195 * (_tmp131 * _tmp162 + _tmp140 * _tmp163 - _tmp196 - _tmp197 - _tmp198) -
       _tmp199 * _tmp209) /
      std::sqrt(Scalar(std::pow(_tmp199, Scalar(2)) * _tmp208 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp35 * _tmp36 * fv1 * std::cosh(_tmp37) -
                    (-Scalar(0.1034955) * _tmp32 * _tmp35 +
                     _tmp33 * (Scalar(9.6622558468725703) * _tmp29 * _tmp36 - _tmp31)) *
                        std::cosh(_tmp34)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp34) -
               Scalar(9.6622558468725703) * std::sinh(_tmp37);
  _res(1, 0) = Scalar(9.6622558468725703) * _tmp159 * (-std::sinh(_tmp166) - std::sinh(_tmp167)) +
               _tmp161 * (-Scalar(1.0) * _tmp164 * std::cosh(_tmp167) -
                          (-Scalar(0.1034955) * _tmp157 * _tmp160 +
                           _tmp165 * (-_tmp156 * _tmp159 - _tmp161 * _tmp164)) *
                              std::cosh(_tmp166));
  _res(2, 0) = _tmp173 * (-Scalar(1.0) * _tmp184 * std::cosh(_tmp190) -
                          (-Scalar(0.1034955) * _tmp182 * _tmp188 +
                           _tmp187 * (-_tmp173 * _tmp184 - _tmp185 * _tmp186)) *
                              std::cosh(_tmp189)) +
               _tmp186 * (-std::sinh(_tmp189) - std::sinh(_tmp190));
  _res(3, 0) = _tmp202 * (-Scalar(1.0) * _tmp210 * std::cosh(_tmp201) -
                          (-Scalar(0.1034955) * _tmp203 * _tmp209 +
                           _tmp204 * (-_tmp200 * _tmp207 - _tmp202 * _tmp210)) *
                              std::cosh(_tmp205)) +
               _tmp207 * (-std::sinh(_tmp201) - std::sinh(_tmp205));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
