// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl16
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl16(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot, const Scalar p_init0,
    const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x, const Scalar rot_init_y,
    const Scalar rot_init_z, const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 640

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (212)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp2 = 2 * _tmp0 * _tmp1;
  const Scalar _tmp3 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp4 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp5 = _tmp3 * _tmp4;
  const Scalar _tmp6 = Scalar(0.20999999999999999) * _tmp2 - Scalar(0.20999999999999999) * _tmp5;
  const Scalar _tmp7 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp8 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp7 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp3;
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = _tmp0 * _tmp4;
  const Scalar _tmp13 = _tmp11 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = _tmp14 + _tmp9;
  const Scalar _tmp16 = _tmp15 + _tmp6;
  const Scalar _tmp17 = _tmp16 + p_init0;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp5;
  const Scalar _tmp19 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp21 = _tmp0 * _tmp10;
  const Scalar _tmp22 = _tmp1 * _tmp4;
  const Scalar _tmp23 = _tmp21 - _tmp22;
  const Scalar _tmp24 = -Scalar(0.010999999999999999) * _tmp23;
  const Scalar _tmp25 = _tmp20 + _tmp24;
  const Scalar _tmp26 = _tmp18 + _tmp25;
  const Scalar _tmp27 = _tmp26 + p_init1;
  const Scalar _tmp28 = Scalar(1.0) / (fh1);
  const Scalar _tmp29 = _tmp28 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -_tmp31 * fh1 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp17), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp27), Scalar(2))));
  const Scalar _tmp33 = Scalar(0.1034955) * _tmp28;
  const Scalar _tmp34 = _tmp32 * _tmp33;
  const Scalar _tmp35 = Scalar(1.0) * _tmp30;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = _tmp14 - _tmp9;
  const Scalar _tmp39 = _tmp38 + _tmp6;
  const Scalar _tmp40 = _tmp39 + p_init0;
  const Scalar _tmp41 = -_tmp18;
  const Scalar _tmp42 = _tmp25 + _tmp41;
  const Scalar _tmp43 = _tmp42 + p_init1;
  const Scalar _tmp44 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp45 = -_tmp6;
  const Scalar _tmp46 = _tmp15 + _tmp45;
  const Scalar _tmp47 = _tmp46 + p_init0;
  const Scalar _tmp48 = _tmp47 + Scalar(-2.5202214700000001);
  const Scalar _tmp49 = -_tmp20 + _tmp24;
  const Scalar _tmp50 = _tmp18 + _tmp49;
  const Scalar _tmp51 = _tmp50 + p_init1;
  const Scalar _tmp52 = _tmp51 + Scalar(8.3888750099999996);
  const Scalar _tmp53 = std::pow(Scalar(std::pow(_tmp48, Scalar(2)) + std::pow(_tmp52, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp54 = _tmp48 * _tmp53;
  const Scalar _tmp55 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp22;
  const Scalar _tmp56 = -Scalar(0.010999999999999999) * _tmp19 -
                        Scalar(0.010999999999999999) * _tmp7 + Scalar(-0.010999999999999999);
  const Scalar _tmp57 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp58 = _tmp56 - _tmp57;
  const Scalar _tmp59 = _tmp55 + _tmp58;
  const Scalar _tmp60 = -_tmp55;
  const Scalar _tmp61 = _tmp56 + _tmp57;
  const Scalar _tmp62 = _tmp60 + _tmp61;
  const Scalar _tmp63 = _tmp41 + _tmp49;
  const Scalar _tmp64 = _tmp63 + p_init1;
  const Scalar _tmp65 = _tmp64 + Scalar(8.3196563700000006);
  const Scalar _tmp66 = _tmp38 + _tmp45;
  const Scalar _tmp67 = _tmp66 + p_init0;
  const Scalar _tmp68 = _tmp67 + Scalar(1.9874742000000001);
  const Scalar _tmp69 = std::pow(Scalar(std::pow(_tmp65, Scalar(2)) + std::pow(_tmp68, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp70 = _tmp68 * _tmp69;
  const Scalar _tmp71 = _tmp58 + _tmp60;
  const Scalar _tmp72 = _tmp59 * _tmp70 - _tmp70 * _tmp71;
  const Scalar _tmp73 = _tmp65 * _tmp69;
  const Scalar _tmp74 = _tmp43 + Scalar(-4.8333311099999996);
  const Scalar _tmp75 = _tmp40 + Scalar(1.79662371);
  const Scalar _tmp76 = Scalar(1.0) / (_tmp75);
  const Scalar _tmp77 = _tmp74 * _tmp76;
  const Scalar _tmp78 = Scalar(1.0) / (_tmp70 * _tmp77 - _tmp73);
  const Scalar _tmp79 = _tmp52 * _tmp53;
  const Scalar _tmp80 = _tmp54 * _tmp77 - _tmp79;
  const Scalar _tmp81 = _tmp78 * _tmp80;
  const Scalar _tmp82 = _tmp59 * _tmp77;
  const Scalar _tmp83 = -_tmp70 * _tmp82 + _tmp71 * _tmp73;
  const Scalar _tmp84 = -_tmp54 * _tmp82 + _tmp62 * _tmp79 - _tmp81 * _tmp83;
  const Scalar _tmp85 = Scalar(1.0) * _tmp42;
  const Scalar _tmp86 = -_tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp63 + _tmp86);
  const Scalar _tmp88 = Scalar(1.0) * _tmp39;
  const Scalar _tmp89 = -_tmp66 + _tmp88;
  const Scalar _tmp90 = _tmp87 * _tmp89;
  const Scalar _tmp91 = _tmp54 * _tmp59 - _tmp54 * _tmp62 - _tmp72 * _tmp81 - _tmp84 * _tmp90;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp85 * _tmp90 + _tmp88;
  const Scalar _tmp94 = 0;
  const Scalar _tmp95 = _tmp70 * _tmp81;
  const Scalar _tmp96 =
      std::sqrt(Scalar(std::pow(_tmp74, Scalar(2)) + std::pow(_tmp75, Scalar(2))));
  const Scalar _tmp97 = _tmp76 * _tmp96;
  const Scalar _tmp98 = _tmp27 + Scalar(-4.7752063900000001);
  const Scalar _tmp99 = _tmp17 + Scalar(-2.71799795);
  const Scalar _tmp100 = std::pow(Scalar(std::pow(_tmp98, Scalar(2)) + std::pow(_tmp99, Scalar(2))),
                                  Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp101 = _tmp100 * _tmp98;
  const Scalar _tmp102 = _tmp100 * _tmp99;
  const Scalar _tmp103 = _tmp101 * _tmp16 - _tmp102 * _tmp26;
  const Scalar _tmp104 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp105 = _tmp97 * (_tmp104 * _tmp39 * _tmp74 - _tmp104 * _tmp42 * _tmp75);
  const Scalar _tmp106 = _tmp105 * _tmp70 + _tmp63 * _tmp70 - _tmp66 * _tmp73;
  const Scalar _tmp107 = _tmp105 * _tmp54 - _tmp106 * _tmp81 - _tmp46 * _tmp79 + _tmp50 * _tmp54;
  const Scalar _tmp108 = Scalar(1.0) / (_tmp107);
  const Scalar _tmp109 = Scalar(1.0) * _tmp108;
  const Scalar _tmp110 = _tmp103 * _tmp97 * (_tmp109 * _tmp54 - _tmp109 * _tmp95);
  const Scalar _tmp111 = Scalar(1.0) * _tmp78;
  const Scalar _tmp112 = _tmp111 * _tmp83;
  const Scalar _tmp113 = -_tmp111 * _tmp72 + _tmp112 * _tmp90;
  const Scalar _tmp114 = _tmp107 * _tmp92;
  const Scalar _tmp115 = _tmp108 * _tmp91;
  const Scalar _tmp116 = _tmp115 * (-_tmp106 * _tmp111 - _tmp113 * _tmp114);
  const Scalar _tmp117 = _tmp92 * (_tmp113 + _tmp116);
  const Scalar _tmp118 = _tmp78 * (-_tmp117 * _tmp80 + Scalar(1.0));
  const Scalar _tmp119 = _tmp101 * _tmp97 * (_tmp117 * _tmp54 + _tmp118 * _tmp70);
  const Scalar _tmp120 = _tmp77 * _tmp78;
  const Scalar _tmp121 = _tmp120 * _tmp83 + _tmp82;
  const Scalar _tmp122 = _tmp120 * _tmp72 - _tmp121 * _tmp90 - _tmp59;
  const Scalar _tmp123 = _tmp115 * (-_tmp105 + _tmp106 * _tmp120 - _tmp114 * _tmp122);
  const Scalar _tmp124 = _tmp92 * (_tmp122 + _tmp123);
  const Scalar _tmp125 = _tmp78 * (-_tmp124 * _tmp80 - _tmp77);
  const Scalar _tmp126 = _tmp102 * _tmp97 * (_tmp124 * _tmp54 + _tmp125 * _tmp70 + Scalar(1.0));
  const Scalar _tmp127 = -_tmp110 * fh1 - _tmp119 * fh1 - _tmp126 * fh1 -
                         _tmp44 * _tmp97 * (_tmp54 * _tmp94 - _tmp94 * _tmp95);
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = _tmp50 + _tmp86;
  const Scalar _tmp130 = _tmp129 * _tmp90;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 - _tmp46 + _tmp88);
  const Scalar _tmp132 = _tmp129 * _tmp131;
  const Scalar _tmp133 = _tmp121 + _tmp123 * _tmp132 - _tmp124 * _tmp84;
  const Scalar _tmp134 = Scalar(1.0) * _tmp87;
  const Scalar _tmp135 = Scalar(1.0) * _tmp131;
  const Scalar _tmp136 = Scalar(1.0) * _tmp102 * (_tmp123 * _tmp135 - _tmp133 * _tmp134);
  const Scalar _tmp137 = _tmp131 * _tmp93;
  const Scalar _tmp138 = -_tmp129 * _tmp137 - _tmp84 * _tmp94 + _tmp86;
  const Scalar _tmp139 = _tmp115 * _tmp135;
  const Scalar _tmp140 = -_tmp109 * _tmp84 + _tmp129 * _tmp139;
  const Scalar _tmp141 = Scalar(1.0) * _tmp103 * (-_tmp134 * _tmp140 + _tmp139);
  const Scalar _tmp142 = _tmp55 + _tmp61;
  const Scalar _tmp143 = _tmp142 * fh1;
  const Scalar _tmp144 = -_tmp101 * _tmp143 - Scalar(3.29616) * _tmp23 - _tmp26 * fv1;
  const Scalar _tmp145 = _tmp130 * _tmp135 + Scalar(1.0);
  const Scalar _tmp146 = _tmp135 * _tmp90;
  const Scalar _tmp147 = -Scalar(1.0) * _tmp134 * _tmp145 + Scalar(1.0) * _tmp146;
  const Scalar _tmp148 = _tmp102 * _tmp143 + Scalar(3.29616) * _tmp13 + _tmp16 * fv1;
  const Scalar _tmp149 = _tmp129 * _tmp87;
  const Scalar _tmp150 = Scalar(1.0) * _tmp135 * _tmp149 - Scalar(1.0) * _tmp135;
  const Scalar _tmp151 = -_tmp112 + _tmp116 * _tmp132 - _tmp117 * _tmp84;
  const Scalar _tmp152 = Scalar(1.0) * _tmp101 * (_tmp116 * _tmp135 - _tmp134 * _tmp151);
  const Scalar _tmp153 =
      _tmp136 * fh1 + _tmp141 * fh1 + _tmp144 * _tmp147 + _tmp148 * _tmp150 + _tmp152 * fh1 +
      Scalar(1.0) * _tmp44 * (-_tmp134 * _tmp138 - _tmp135 * _tmp93 + Scalar(1.0));
  const Scalar _tmp154 = std::asinh(_tmp128 * _tmp153);
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp156 =
      -_tmp154 * _tmp155 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp43), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp40 - 1), Scalar(2))));
  const Scalar _tmp157 = Scalar(0.1034955) * _tmp128;
  const Scalar _tmp158 = _tmp156 * _tmp157;
  const Scalar _tmp159 = Scalar(1.0) * _tmp154;
  const Scalar _tmp160 = -_tmp110 - _tmp119 - _tmp126;
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp160;
  const Scalar _tmp162 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp163 = _tmp160 * _tmp162;
  const Scalar _tmp164 = _tmp101 * _tmp142;
  const Scalar _tmp165 = _tmp102 * _tmp142;
  const Scalar _tmp166 =
      (_tmp128 * (_tmp136 + _tmp141 - _tmp147 * _tmp164 + _tmp150 * _tmp165 + _tmp152) -
       _tmp153 * _tmp163) /
      std::sqrt(Scalar(std::pow(_tmp153, Scalar(2)) * _tmp162 + 1));
  const Scalar _tmp167 = _tmp103 * _tmp109;
  const Scalar _tmp168 = _tmp167 * fh1;
  const Scalar _tmp169 = _tmp101 * _tmp118;
  const Scalar _tmp170 = _tmp102 * _tmp125;
  const Scalar _tmp171 = _tmp44 * _tmp94;
  const Scalar _tmp172 = -_tmp168 * _tmp81 + _tmp169 * fh1 + _tmp170 * fh1 - _tmp171 * _tmp81;
  const Scalar _tmp173 = Scalar(1.0) / (_tmp172);
  const Scalar _tmp174 = _tmp101 * _tmp87;
  const Scalar _tmp175 = _tmp151 * _tmp174;
  const Scalar _tmp176 = _tmp102 * _tmp87;
  const Scalar _tmp177 = _tmp133 * _tmp176;
  const Scalar _tmp178 = _tmp103 * _tmp140 * _tmp87;
  const Scalar _tmp179 = _tmp135 * _tmp148;
  const Scalar _tmp180 = _tmp144 * _tmp87;
  const Scalar _tmp181 = _tmp138 * _tmp44 * _tmp87 + _tmp145 * _tmp180 - _tmp149 * _tmp179 +
                         _tmp175 * fh1 + _tmp177 * fh1 + _tmp178 * fh1;
  const Scalar _tmp182 = std::asinh(_tmp173 * _tmp181);
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp172;
  const Scalar _tmp184 =
      -_tmp182 * _tmp183 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp64 - 1), Scalar(2)) +
                     Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp67 - 1), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp173;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = Scalar(1.0) * _tmp182;
  const Scalar _tmp188 = -_tmp167 * _tmp81 + _tmp169 + _tmp170;
  const Scalar _tmp189 = Scalar(9.6622558468725703) * _tmp188;
  const Scalar _tmp190 = std::pow(_tmp172, Scalar(-2));
  const Scalar _tmp191 = _tmp188 * _tmp190;
  const Scalar _tmp192 = (_tmp173 * (-_tmp129 * _tmp135 * _tmp142 * _tmp176 -
                                     _tmp142 * _tmp145 * _tmp174 + _tmp175 + _tmp177 + _tmp178) -
                          _tmp181 * _tmp191) /
                         std::sqrt(Scalar(std::pow(_tmp181, Scalar(2)) * _tmp190 + 1));
  const Scalar _tmp193 = _tmp102 * _tmp124;
  const Scalar _tmp194 = _tmp101 * _tmp117;
  const Scalar _tmp195 = _tmp168 + _tmp171 + _tmp193 * fh1 + _tmp194 * fh1;
  const Scalar _tmp196 = Scalar(1.0) / (_tmp195);
  const Scalar _tmp197 = _tmp101 * _tmp116 * _tmp131;
  const Scalar _tmp198 = _tmp103 * _tmp139;
  const Scalar _tmp199 = _tmp102 * _tmp123 * _tmp131;
  const Scalar _tmp200 = -_tmp135 * _tmp180 * _tmp89 + _tmp137 * _tmp44 + _tmp179 - _tmp197 * fh1 -
                         _tmp198 * fh1 - _tmp199 * fh1;
  const Scalar _tmp201 = std::asinh(_tmp196 * _tmp200);
  const Scalar _tmp202 = Scalar(9.6622558468725703) * _tmp195;
  const Scalar _tmp203 =
      -_tmp201 * _tmp202 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp47), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp51 - 1), Scalar(2))));
  const Scalar _tmp204 = Scalar(0.1034955) * _tmp196;
  const Scalar _tmp205 = _tmp203 * _tmp204;
  const Scalar _tmp206 = Scalar(1.0) * _tmp201;
  const Scalar _tmp207 = _tmp167 + _tmp193 + _tmp194;
  const Scalar _tmp208 = Scalar(9.6622558468725703) * _tmp207;
  const Scalar _tmp209 = std::pow(_tmp195, Scalar(-2));
  const Scalar _tmp210 = _tmp207 * _tmp209;
  const Scalar _tmp211 =
      (_tmp196 * (_tmp135 * _tmp165 + _tmp146 * _tmp164 - _tmp197 - _tmp198 - _tmp199) -
       _tmp200 * _tmp210) /
      std::sqrt(Scalar(std::pow(_tmp200, Scalar(2)) * _tmp209 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp35) -
                    (-Scalar(0.1034955) * _tmp32 * _tmp36 +
                     _tmp33 * (Scalar(9.6622558468725703) * _tmp29 * _tmp37 - _tmp31)) *
                        std::cosh(_tmp34)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp34) -
               Scalar(9.6622558468725703) * std::sinh(_tmp35);
  _res(1, 0) = _tmp155 * (-Scalar(1.0) * _tmp166 * std::cosh(_tmp159) -
                          (-Scalar(0.1034955) * _tmp156 * _tmp163 +
                           _tmp157 * (-_tmp154 * _tmp161 - _tmp155 * _tmp166)) *
                              std::cosh(_tmp158)) +
               _tmp161 * (-std::sinh(_tmp158) - std::sinh(_tmp159));
  _res(2, 0) = _tmp183 * (-Scalar(1.0) * _tmp192 * std::cosh(_tmp187) -
                          (-Scalar(0.1034955) * _tmp184 * _tmp191 +
                           _tmp185 * (-_tmp182 * _tmp189 - _tmp183 * _tmp192)) *
                              std::cosh(_tmp186)) +
               _tmp189 * (-std::sinh(_tmp186) - std::sinh(_tmp187));
  _res(3, 0) = _tmp202 * (-Scalar(1.0) * _tmp211 * std::cosh(_tmp206) -
                          (-Scalar(0.1034955) * _tmp203 * _tmp210 +
                           _tmp204 * (-_tmp201 * _tmp208 - _tmp202 * _tmp211)) *
                              std::cosh(_tmp205)) +
               _tmp208 * (-std::sinh(_tmp205) - std::sinh(_tmp206));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
