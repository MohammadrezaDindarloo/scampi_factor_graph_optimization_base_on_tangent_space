// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl18
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl18(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 595

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (189)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp10 = 2 * _tmp5 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp3;
  const Scalar _tmp13 = _tmp10 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp3;
  const Scalar _tmp16 = _tmp15 * _tmp9;
  const Scalar _tmp17 = _tmp11 * _tmp5;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = _tmp14 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp8;
  const Scalar _tmp21 = _tmp20 + position_vector(0, 0);
  const Scalar _tmp22 = 1 - 2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp25 = _tmp15 * _tmp5;
  const Scalar _tmp26 = _tmp11 * _tmp9;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = -_tmp28;
  const Scalar _tmp30 = -_tmp24 + _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(1, 0);
  const Scalar _tmp33 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp34 = -_tmp23;
  const Scalar _tmp35 = _tmp30 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = _tmp14 - _tmp18;
  const Scalar _tmp38 = _tmp37 + _tmp8;
  const Scalar _tmp39 = _tmp38 + position_vector(0, 0);
  const Scalar _tmp40 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp41 = Scalar(1.0) * _tmp35;
  const Scalar _tmp42 = -_tmp41;
  const Scalar _tmp43 = _tmp24 + _tmp34;
  const Scalar _tmp44 = _tmp29 + _tmp43;
  const Scalar _tmp45 = Scalar(1.0) / (_tmp42 + _tmp44);
  const Scalar _tmp46 = Scalar(0.20999999999999999) * _tmp25 + Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp47 = -_tmp46;
  const Scalar _tmp48 =
      -Scalar(0.010999999999999999) * _tmp22 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp49 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp50 = _tmp48 + _tmp49;
  const Scalar _tmp51 = _tmp47 + _tmp50;
  const Scalar _tmp52 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp53 = _tmp52 + Scalar(110.0);
  const Scalar _tmp54 = _tmp37 + _tmp7;
  const Scalar _tmp55 = _tmp54 + position_vector(0, 0);
  const Scalar _tmp56 = _tmp55 + Scalar(-125.0);
  const Scalar _tmp57 = std::pow(Scalar(std::pow(_tmp53, Scalar(2)) + std::pow(_tmp56, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp58 = _tmp53 * _tmp57;
  const Scalar _tmp59 = _tmp36 + Scalar(110.0);
  const Scalar _tmp60 = _tmp39 + Scalar(125.0);
  const Scalar _tmp61 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp62 = _tmp59 * _tmp61;
  const Scalar _tmp63 = _tmp48 - _tmp49;
  const Scalar _tmp64 = _tmp47 + _tmp63;
  const Scalar _tmp65 = _tmp56 * _tmp57;
  const Scalar _tmp66 = _tmp64 * _tmp65;
  const Scalar _tmp67 = _tmp51 * _tmp58 - _tmp62 * _tmp66;
  const Scalar _tmp68 = Scalar(1.0) / (-_tmp58 + _tmp62 * _tmp65);
  const Scalar _tmp69 = _tmp23 + _tmp24 + _tmp29;
  const Scalar _tmp70 = _tmp69 + position_vector(1, 0);
  const Scalar _tmp71 = _tmp70 + Scalar(-110.0);
  const Scalar _tmp72 = _tmp19 + _tmp7;
  const Scalar _tmp73 = _tmp72 + position_vector(0, 0);
  const Scalar _tmp74 = _tmp73 + Scalar(-125.0);
  const Scalar _tmp75 = std::pow(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp74, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp76 = _tmp74 * _tmp75;
  const Scalar _tmp77 = _tmp71 * _tmp75;
  const Scalar _tmp78 = _tmp62 * _tmp76 - _tmp77;
  const Scalar _tmp79 = _tmp68 * _tmp78;
  const Scalar _tmp80 = _tmp46 + _tmp50;
  const Scalar _tmp81 = _tmp64 * _tmp76;
  const Scalar _tmp82 = -_tmp62 * _tmp81 - _tmp67 * _tmp79 + _tmp77 * _tmp80;
  const Scalar _tmp83 = Scalar(1.0) * _tmp38;
  const Scalar _tmp84 = _tmp45 * (-_tmp54 + _tmp83);
  const Scalar _tmp85 = _tmp41 * _tmp84 + _tmp83;
  const Scalar _tmp86 = -_tmp51 * _tmp65 + _tmp66;
  const Scalar _tmp87 = -_tmp76 * _tmp80 - _tmp79 * _tmp86 + _tmp81 - _tmp82 * _tmp84;
  const Scalar _tmp88 = Scalar(1.0) / (_tmp87);
  const Scalar _tmp89 = 0;
  const Scalar _tmp90 = _tmp42 + _tmp69;
  const Scalar _tmp91 = _tmp84 * _tmp90;
  const Scalar _tmp92 = Scalar(1.0) / (-_tmp72 + _tmp83 - _tmp91);
  const Scalar _tmp93 = _tmp85 * _tmp92;
  const Scalar _tmp94 = _tmp45 * (_tmp42 - _tmp82 * _tmp89 - _tmp90 * _tmp93);
  const Scalar _tmp95 = Scalar(1.0) * _tmp92;
  const Scalar _tmp96 = -Scalar(1.0) * _tmp85 * _tmp95 - Scalar(1.0) * _tmp94 + Scalar(1.0);
  const Scalar _tmp97 = _tmp21 + Scalar(125.0);
  const Scalar _tmp98 = _tmp32 + Scalar(-110.0);
  const Scalar _tmp99 = std::pow(Scalar(std::pow(_tmp97, Scalar(2)) + std::pow(_tmp98, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp100 = _tmp98 * _tmp99;
  const Scalar _tmp101 = fh1 * (_tmp46 + _tmp63);
  const Scalar _tmp102 = -_tmp100 * _tmp101 - Scalar(40.024799999999999) * _tmp27 - _tmp31 * fv1;
  const Scalar _tmp103 = _tmp45 * (_tmp91 * _tmp95 + Scalar(1.0));
  const Scalar _tmp104 = _tmp84 * _tmp95;
  const Scalar _tmp105 = -Scalar(1.0) * _tmp103 + Scalar(1.0) * _tmp104;
  const Scalar _tmp106 = _tmp97 * _tmp99;
  const Scalar _tmp107 = _tmp101 * _tmp106 + Scalar(40.024799999999999) * _tmp13 + _tmp20 * fv1;
  const Scalar _tmp108 = _tmp45 * _tmp90;
  const Scalar _tmp109 = Scalar(1.0) * _tmp108 * _tmp95 - Scalar(1.0) * _tmp95;
  const Scalar _tmp110 =
      std::sqrt(Scalar(std::pow(_tmp59, Scalar(2)) + std::pow(_tmp60, Scalar(2))));
  const Scalar _tmp111 = Scalar(1.0) / (_tmp110);
  const Scalar _tmp112 = _tmp110 * _tmp61;
  const Scalar _tmp113 = _tmp112 * (-_tmp111 * _tmp35 * _tmp60 + _tmp111 * _tmp38 * _tmp59);
  const Scalar _tmp114 = _tmp113 * _tmp65 + _tmp44 * _tmp65 - _tmp54 * _tmp58;
  const Scalar _tmp115 = _tmp62 * _tmp68;
  const Scalar _tmp116 = _tmp115 * _tmp67 + _tmp62 * _tmp64;
  const Scalar _tmp117 = _tmp115 * _tmp86 - _tmp116 * _tmp84 - _tmp64;
  const Scalar _tmp118 = _tmp113 * _tmp76 - _tmp114 * _tmp79 + _tmp69 * _tmp76 - _tmp72 * _tmp77;
  const Scalar _tmp119 = _tmp118 * _tmp88;
  const Scalar _tmp120 = Scalar(1.0) / (_tmp118);
  const Scalar _tmp121 = _tmp120 * _tmp87;
  const Scalar _tmp122 = _tmp121 * (-_tmp113 + _tmp114 * _tmp115 - _tmp117 * _tmp119);
  const Scalar _tmp123 = _tmp88 * (_tmp117 + _tmp122);
  const Scalar _tmp124 = _tmp90 * _tmp92;
  const Scalar _tmp125 = _tmp116 + _tmp122 * _tmp124 - _tmp123 * _tmp82;
  const Scalar _tmp126 = Scalar(1.0) * _tmp45;
  const Scalar _tmp127 = Scalar(1.0) * fh1;
  const Scalar _tmp128 = Scalar(1.0) * _tmp68;
  const Scalar _tmp129 = _tmp128 * _tmp67;
  const Scalar _tmp130 = -_tmp128 * _tmp86 + _tmp129 * _tmp84;
  const Scalar _tmp131 = _tmp121 * (-_tmp114 * _tmp128 - _tmp119 * _tmp130);
  const Scalar _tmp132 = _tmp88 * (_tmp130 + _tmp131);
  const Scalar _tmp133 = _tmp124 * _tmp131 - _tmp129 - _tmp132 * _tmp82;
  const Scalar _tmp134 = _tmp121 * _tmp95;
  const Scalar _tmp135 = Scalar(1.0) * _tmp120;
  const Scalar _tmp136 = _tmp134 * _tmp90 - _tmp135 * _tmp82;
  const Scalar _tmp137 = fh1 * (_tmp100 * _tmp20 - _tmp106 * _tmp31);
  const Scalar _tmp138 =
      _tmp100 * _tmp127 * (-_tmp126 * _tmp133 + _tmp131 * _tmp95) + _tmp102 * _tmp105 +
      _tmp106 * _tmp127 * (_tmp122 * _tmp95 - _tmp125 * _tmp126) + _tmp107 * _tmp109 +
      Scalar(1.0) * _tmp137 * (-_tmp126 * _tmp136 + _tmp134) + _tmp40 * _tmp96;
  const Scalar _tmp139 = -_tmp123 * _tmp78 - _tmp62;
  const Scalar _tmp140 = _tmp65 * _tmp68;
  const Scalar _tmp141 = _tmp106 * fh1;
  const Scalar _tmp142 = _tmp79 * _tmp89;
  const Scalar _tmp143 = _tmp112 * (-_tmp142 * _tmp65 + _tmp76 * _tmp89);
  const Scalar _tmp144 = _tmp120 * _tmp128 * _tmp78;
  const Scalar _tmp145 = -_tmp132 * _tmp78 + Scalar(1.0);
  const Scalar _tmp146 = _tmp100 * fh1;
  const Scalar _tmp147 = -_tmp112 * _tmp137 * (_tmp135 * _tmp76 - _tmp144 * _tmp65) -
                         _tmp112 * _tmp141 * (_tmp123 * _tmp76 + _tmp139 * _tmp140 + Scalar(1.0)) -
                         _tmp112 * _tmp146 * (_tmp132 * _tmp76 + _tmp140 * _tmp145) -
                         _tmp143 * _tmp40;
  const Scalar _tmp148 = Scalar(1.0) / (_tmp147);
  const Scalar _tmp149 = std::asinh(_tmp138 * _tmp148);
  const Scalar _tmp150 = Scalar(1.4083112389913199) * _tmp147;
  const Scalar _tmp151 =
      -_tmp149 * _tmp150 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp36 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp39 - 1), Scalar(2))));
  const Scalar _tmp152 = Scalar(0.71007031138673404) * _tmp148;
  const Scalar _tmp153 = _tmp151 * _tmp152;
  const Scalar _tmp154 = Scalar(1.0) * _tmp149;
  const Scalar _tmp155 = Scalar(1.4083112389913199) * _tmp143;
  const Scalar _tmp156 = std::pow(_tmp147, Scalar(-2));
  const Scalar _tmp157 = _tmp143 * _tmp156;
  const Scalar _tmp158 = _tmp28 + _tmp43;
  const Scalar _tmp159 =
      (-_tmp138 * _tmp157 + _tmp148 * (_tmp105 * _tmp158 + _tmp109 * _tmp20 - _tmp96)) /
      std::sqrt(Scalar(std::pow(_tmp138, Scalar(2)) * _tmp156 + 1));
  const Scalar _tmp160 = _tmp107 * _tmp95;
  const Scalar _tmp161 = _tmp102 * _tmp103 - _tmp108 * _tmp160 + _tmp125 * _tmp141 * _tmp45 +
                         _tmp133 * _tmp146 * _tmp45 + _tmp136 * _tmp137 * _tmp45 + _tmp40 * _tmp94;
  const Scalar _tmp162 = _tmp40 * _tmp89;
  const Scalar _tmp163 = -_tmp137 * _tmp144 + _tmp139 * _tmp141 * _tmp68 +
                         _tmp145 * _tmp146 * _tmp68 - _tmp162 * _tmp79;
  const Scalar _tmp164 = Scalar(1.0) / (_tmp163);
  const Scalar _tmp165 = std::asinh(_tmp161 * _tmp164);
  const Scalar _tmp166 = Scalar(1.4083112389913199) * _tmp163;
  const Scalar _tmp167 =
      -_tmp165 * _tmp166 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp55), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp52 - 1), Scalar(2))));
  const Scalar _tmp168 = Scalar(0.71007031138673404) * _tmp164;
  const Scalar _tmp169 = _tmp167 * _tmp168;
  const Scalar _tmp170 = Scalar(1.0) * _tmp165;
  const Scalar _tmp171 = Scalar(1.4083112389913199) * _tmp89;
  const Scalar _tmp172 = _tmp171 * _tmp79;
  const Scalar _tmp173 = std::pow(_tmp163, Scalar(-2));
  const Scalar _tmp174 = _tmp142 * _tmp173;
  const Scalar _tmp175 = _tmp20 * _tmp95;
  const Scalar _tmp176 =
      (-_tmp161 * _tmp174 + _tmp164 * (_tmp103 * _tmp158 - _tmp108 * _tmp175 - _tmp94)) /
      std::sqrt(Scalar(std::pow(_tmp161, Scalar(2)) * _tmp173 + 1));
  const Scalar _tmp177 = _tmp123 * _tmp141 + _tmp132 * _tmp146 + _tmp135 * _tmp137 + _tmp162;
  const Scalar _tmp178 = Scalar(1.0) / (_tmp177);
  const Scalar _tmp179 = -_tmp102 * _tmp104 - _tmp122 * _tmp141 * _tmp92 -
                         _tmp131 * _tmp146 * _tmp92 - _tmp134 * _tmp137 + _tmp160 + _tmp40 * _tmp93;
  const Scalar _tmp180 = std::asinh(_tmp178 * _tmp179);
  const Scalar _tmp181 = Scalar(1.4083112389913199) * _tmp177;
  const Scalar _tmp182 =
      -_tmp180 * _tmp181 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp70), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp73), Scalar(2))));
  const Scalar _tmp183 = std::pow(_tmp177, Scalar(-2));
  const Scalar _tmp184 = _tmp183 * _tmp89;
  const Scalar _tmp185 = (_tmp178 * (-_tmp104 * _tmp158 + _tmp175 - _tmp93) + _tmp179 * _tmp184) /
                         std::sqrt(Scalar(std::pow(_tmp179, Scalar(2)) * _tmp183 + 1));
  const Scalar _tmp186 = Scalar(0.71007031138673404) * _tmp178;
  const Scalar _tmp187 = _tmp182 * _tmp186;
  const Scalar _tmp188 = Scalar(1.0) * _tmp180;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.71007031138673404) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp32),
                                               Scalar(2)) +
                                  std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp21 - 1),
                                           Scalar(2)))))));
  _res(1, 0) = _tmp150 * (-Scalar(1.0) * _tmp159 * std::cosh(_tmp154) -
                          (-Scalar(0.71007031138673404) * _tmp151 * _tmp157 +
                           _tmp152 * (-_tmp149 * _tmp155 - _tmp150 * _tmp159)) *
                              std::cosh(_tmp153)) +
               _tmp155 * (-std::sinh(_tmp153) - std::sinh(_tmp154));
  _res(2, 0) = _tmp166 * (-Scalar(1.0) * _tmp176 * std::cosh(_tmp170) -
                          (-Scalar(0.71007031138673404) * _tmp167 * _tmp174 +
                           _tmp168 * (-_tmp165 * _tmp172 - _tmp166 * _tmp176)) *
                              std::cosh(_tmp169)) +
               _tmp172 * (-std::sinh(_tmp169) - std::sinh(_tmp170));
  _res(3, 0) = -_tmp171 * (-std::sinh(_tmp187) - std::sinh(_tmp188)) +
               _tmp181 * (-Scalar(1.0) * _tmp185 * std::cosh(_tmp188) -
                          (Scalar(0.71007031138673404) * _tmp182 * _tmp184 +
                           _tmp186 * (_tmp171 * _tmp180 - _tmp181 * _tmp185)) *
                              std::cosh(_tmp187));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
