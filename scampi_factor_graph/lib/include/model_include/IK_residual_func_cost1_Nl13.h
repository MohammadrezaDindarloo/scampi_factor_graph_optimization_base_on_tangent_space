// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_Nl13
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1Nl13(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 502

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (152)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp7 = 2 * _tmp3 * _tmp6;
  const Scalar _tmp8 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = _tmp7 + _tmp9;
  const Scalar _tmp11 = -Scalar(0.010999999999999999) * _tmp10;
  const Scalar _tmp12 = 2 * _tmp1;
  const Scalar _tmp13 = _tmp12 * _tmp6;
  const Scalar _tmp14 = _tmp3 * _tmp8;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp11 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(0, 0);
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp20 = _tmp12 * _tmp3;
  const Scalar _tmp21 = _tmp6 * _tmp8;
  const Scalar _tmp22 = _tmp20 - _tmp21;
  const Scalar _tmp23 = -Scalar(0.010999999999999999) * _tmp22;
  const Scalar _tmp24 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp26 = _tmp23 + _tmp25;
  const Scalar _tmp27 = _tmp19 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(1, 0);
  const Scalar _tmp29 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp30 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp31 = Scalar(0.20999999999999999) * _tmp7 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp32 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp34 = _tmp32 + _tmp33;
  const Scalar _tmp35 = _tmp31 + _tmp34;
  const Scalar _tmp36 = _tmp23 - _tmp25;
  const Scalar _tmp37 = _tmp19 + _tmp36;
  const Scalar _tmp38 = _tmp37 + position_vector(1, 0);
  const Scalar _tmp39 = _tmp38 + Scalar(110.0);
  const Scalar _tmp40 = _tmp11 - _tmp15;
  const Scalar _tmp41 = _tmp40 + _tmp5;
  const Scalar _tmp42 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(-125.0);
  const Scalar _tmp44 = std::pow(Scalar(std::pow(_tmp39, Scalar(2)) + std::pow(_tmp43, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp45 = _tmp43 * _tmp44;
  const Scalar _tmp46 = _tmp32 - _tmp33;
  const Scalar _tmp47 = _tmp31 + _tmp46;
  const Scalar _tmp48 = _tmp39 * _tmp44;
  const Scalar _tmp49 = -_tmp31;
  const Scalar _tmp50 = _tmp46 + _tmp49;
  const Scalar _tmp51 = -_tmp19;
  const Scalar _tmp52 = _tmp36 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(110.0);
  const Scalar _tmp55 = -_tmp5;
  const Scalar _tmp56 = _tmp40 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(0, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(125.0);
  const Scalar _tmp59 = Scalar(1.0) / (_tmp58);
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp50 * _tmp60;
  const Scalar _tmp62 = _tmp16 + _tmp55;
  const Scalar _tmp63 = _tmp62 + position_vector(0, 0);
  const Scalar _tmp64 = _tmp63 + Scalar(125.0);
  const Scalar _tmp65 = _tmp26 + _tmp51;
  const Scalar _tmp66 = _tmp65 + position_vector(1, 0);
  const Scalar _tmp67 = _tmp66 + Scalar(-110.0);
  const Scalar _tmp68 = std::pow(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp67, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp69 = _tmp64 * _tmp68;
  const Scalar _tmp70 = _tmp34 + _tmp49;
  const Scalar _tmp71 = _tmp67 * _tmp68;
  const Scalar _tmp72 = -_tmp61 * _tmp69 + _tmp70 * _tmp71;
  const Scalar _tmp73 = _tmp45 * _tmp60 - _tmp48;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp60 * _tmp69 - _tmp71);
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = -_tmp45 * _tmp61 + _tmp47 * _tmp48 - _tmp72 * _tmp75;
  const Scalar _tmp77 = Scalar(1.0) * _tmp52;
  const Scalar _tmp78 = -_tmp77;
  const Scalar _tmp79 = Scalar(1.0) / (_tmp65 + _tmp78);
  const Scalar _tmp80 = Scalar(1.0) * _tmp56;
  const Scalar _tmp81 = -_tmp62 + _tmp80;
  const Scalar _tmp82 = _tmp79 * _tmp81;
  const Scalar _tmp83 = _tmp50 * _tmp69 - _tmp69 * _tmp70;
  const Scalar _tmp84 = -_tmp45 * _tmp47 + _tmp45 * _tmp50 - _tmp75 * _tmp83 - _tmp76 * _tmp82;
  const Scalar _tmp85 = Scalar(1.0) / (_tmp84);
  const Scalar _tmp86 = Scalar(1.0) * _tmp79;
  const Scalar _tmp87 = Scalar(1.0) * _tmp74;
  const Scalar _tmp88 = _tmp72 * _tmp74 * _tmp81 * _tmp86 - _tmp83 * _tmp87;
  const Scalar _tmp89 =
      std::sqrt(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))));
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 = _tmp59 * _tmp89;
  const Scalar _tmp92 = _tmp91 * (-_tmp52 * _tmp58 * _tmp90 + _tmp54 * _tmp56 * _tmp90);
  const Scalar _tmp93 = -_tmp62 * _tmp71 + _tmp65 * _tmp69 + _tmp69 * _tmp92;
  const Scalar _tmp94 = _tmp37 * _tmp45 - _tmp41 * _tmp48 + _tmp45 * _tmp92 - _tmp75 * _tmp93;
  const Scalar _tmp95 = _tmp85 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp97 = _tmp84 * _tmp96;
  const Scalar _tmp98 = _tmp97 * (-_tmp87 * _tmp93 - _tmp88 * _tmp95);
  const Scalar _tmp99 = _tmp85 * (_tmp88 + _tmp98);
  const Scalar _tmp100 = -_tmp73 * _tmp99 + Scalar(1.0);
  const Scalar _tmp101 = _tmp69 * _tmp74;
  const Scalar _tmp102 = _tmp28 + Scalar(-110.0);
  const Scalar _tmp103 = _tmp18 + Scalar(-125.0);
  const Scalar _tmp104 =
      std::pow(Scalar(std::pow(_tmp102, Scalar(2)) + std::pow(_tmp103, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp105 = _tmp102 * _tmp104;
  const Scalar _tmp106 = _tmp105 * fh1;
  const Scalar _tmp107 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp108 = _tmp77 * _tmp82 + _tmp80;
  const Scalar _tmp109 = 0;
  const Scalar _tmp110 = _tmp60 * _tmp74;
  const Scalar _tmp111 = _tmp110 * _tmp72 + _tmp61;
  const Scalar _tmp112 = _tmp110 * _tmp83 - _tmp111 * _tmp82 - _tmp50;
  const Scalar _tmp113 = _tmp97 * (_tmp110 * _tmp93 - _tmp112 * _tmp95 - _tmp92);
  const Scalar _tmp114 = _tmp85 * (_tmp112 + _tmp113);
  const Scalar _tmp115 = -_tmp114 * _tmp73 - _tmp60;
  const Scalar _tmp116 = _tmp103 * _tmp104;
  const Scalar _tmp117 = _tmp116 * fh1;
  const Scalar _tmp118 = Scalar(1.0) * _tmp96;
  const Scalar _tmp119 = _tmp73 * _tmp87 * _tmp96;
  const Scalar _tmp120 = fh1 * (_tmp105 * _tmp17 - _tmp116 * _tmp27);
  const Scalar _tmp121 = -_tmp106 * _tmp91 * (_tmp100 * _tmp101 + _tmp45 * _tmp99) -
                         _tmp107 * _tmp91 * (_tmp109 * _tmp45 - _tmp109 * _tmp69 * _tmp75) -
                         _tmp117 * _tmp91 * (_tmp101 * _tmp115 + _tmp114 * _tmp45 + Scalar(1.0)) -
                         _tmp120 * _tmp91 * (_tmp118 * _tmp45 - _tmp119 * _tmp69);
  const Scalar _tmp122 = Scalar(1.0) / (_tmp121);
  const Scalar _tmp123 = _tmp37 + _tmp78;
  const Scalar _tmp124 = _tmp123 * _tmp82;
  const Scalar _tmp125 = Scalar(1.0) / (-_tmp124 - _tmp41 + _tmp80);
  const Scalar _tmp126 = _tmp108 * _tmp125;
  const Scalar _tmp127 = -_tmp109 * _tmp76 - _tmp123 * _tmp126 + _tmp78;
  const Scalar _tmp128 = Scalar(1.0) * _tmp125;
  const Scalar _tmp129 = _tmp123 * _tmp125;
  const Scalar _tmp130 = _tmp129 * _tmp98 - _tmp72 * _tmp87 - _tmp76 * _tmp99;
  const Scalar _tmp131 = _tmp123 * _tmp79;
  const Scalar _tmp132 = _tmp35 * fh1;
  const Scalar _tmp133 = Scalar(40.024799999999999) * _tmp10 + _tmp116 * _tmp132 + _tmp17 * fv1;
  const Scalar _tmp134 = _tmp111 + _tmp113 * _tmp129 - _tmp114 * _tmp76;
  const Scalar _tmp135 = _tmp128 * _tmp82;
  const Scalar _tmp136 = _tmp124 * _tmp128 + Scalar(1.0);
  const Scalar _tmp137 = -_tmp105 * _tmp132 - Scalar(40.024799999999999) * _tmp22 - _tmp27 * fv1;
  const Scalar _tmp138 = _tmp128 * _tmp97;
  const Scalar _tmp139 = -_tmp118 * _tmp76 + _tmp123 * _tmp138;
  const Scalar _tmp140 = std::asinh(
      _tmp122 * (Scalar(1.0) * _tmp106 * (_tmp128 * _tmp98 - _tmp130 * _tmp86) +
                 Scalar(1.0) * _tmp107 * (-_tmp108 * _tmp128 - _tmp127 * _tmp86 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp117 * (_tmp113 * _tmp128 - _tmp134 * _tmp86) +
                 Scalar(1.0) * _tmp120 * (_tmp138 - _tmp139 * _tmp86) +
                 Scalar(1.0) * _tmp133 * (_tmp128 * _tmp131 - _tmp128) +
                 Scalar(1.0) * _tmp137 * (_tmp135 - _tmp136 * _tmp86)));
  const Scalar _tmp141 = Scalar(1.4083112389913199) * _tmp121;
  const Scalar _tmp142 = _tmp107 * _tmp109;
  const Scalar _tmp143 = _tmp100 * _tmp106 * _tmp74 + _tmp115 * _tmp117 * _tmp74 -
                         _tmp119 * _tmp120 - _tmp142 * _tmp75;
  const Scalar _tmp144 = Scalar(1.0) / (_tmp143);
  const Scalar _tmp145 = _tmp128 * _tmp133;
  const Scalar _tmp146 =
      std::asinh(_tmp144 * (_tmp106 * _tmp130 * _tmp79 + _tmp107 * _tmp127 * _tmp79 +
                            _tmp117 * _tmp134 * _tmp79 + _tmp120 * _tmp139 * _tmp79 -
                            _tmp131 * _tmp145 + _tmp136 * _tmp137 * _tmp79));
  const Scalar _tmp147 = Scalar(1.4083112389913199) * _tmp143;
  const Scalar _tmp148 = _tmp106 * _tmp99 + _tmp114 * _tmp117 + _tmp118 * _tmp120 + _tmp142;
  const Scalar _tmp149 = Scalar(1.0) / (_tmp148);
  const Scalar _tmp150 = std::asinh(_tmp149 * (-_tmp106 * _tmp125 * _tmp98 + _tmp107 * _tmp126 -
                                               _tmp113 * _tmp117 * _tmp125 - _tmp120 * _tmp138 -
                                               _tmp135 * _tmp137 + _tmp145));
  const Scalar _tmp151 = Scalar(1.4083112389913199) * _tmp148;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp30 *
          (Scalar(34.083374946563197) * _tmp0 + std::cosh(Scalar(1.0) * _tmp29) -
           std::cosh(
               Scalar(0.71007031138673404) * _tmp0 *
               (-_tmp29 * _tmp30 -
                Scalar(125.0) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp18), Scalar(2)) +
                        Scalar(0.77439999999999998) *
                            std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp28),
                                     Scalar(2))))))) +
      _tmp35 + position_vector(2, 0);
  _res(1, 0) =
      -_tmp141 *
          (Scalar(34.083374946563197) * _tmp122 + std::cosh(Scalar(1.0) * _tmp140) -
           std::cosh(Scalar(0.71007031138673404) * _tmp122 *
                     (-_tmp140 * _tmp141 -
                      Scalar(125.0) *
                          std::sqrt(Scalar(
                              Scalar(0.77439999999999998) *
                                  std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp53 - 1),
                                           Scalar(2)) +
                              std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp57 - 1),
                                       Scalar(2))))))) +
      _tmp50 + position_vector(2, 0);
  _res(2, 0) =
      -_tmp147 *
          (Scalar(34.083374946563197) * _tmp144 + std::cosh(Scalar(1.0) * _tmp146) -
           std::cosh(
               Scalar(0.71007031138673404) * _tmp144 *
               (-_tmp146 * _tmp147 -
                Scalar(125.0) * std::sqrt(Scalar(
                                    Scalar(0.77439999999999998) *
                                        std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp66),
                                                 Scalar(2)) +
                                    std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp63 - 1),
                                             Scalar(2))))))) +
      _tmp70 + position_vector(2, 0);
  _res(3, 0) =
      -_tmp151 *
          (Scalar(34.083374946563197) * _tmp149 + std::cosh(Scalar(1.0) * _tmp150) -
           std::cosh(
               Scalar(0.71007031138673404) * _tmp149 *
               (-_tmp150 * _tmp151 -
                Scalar(125.0) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp42), Scalar(2)) +
                        Scalar(0.77439999999999998) *
                            std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp38 - 1),
                                     Scalar(2))))))) +
      _tmp47 + position_vector(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
