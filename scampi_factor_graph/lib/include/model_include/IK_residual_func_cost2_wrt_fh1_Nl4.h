// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl4
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl4(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot, const Scalar p_init0,
    const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x, const Scalar rot_init_y,
    const Scalar rot_init_z, const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 643

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp1 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp2 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp3 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp3 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp5 = -_tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp7 = 2 * _tmp2;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp10 = _tmp0 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp0 * _tmp6;
  const Scalar _tmp14 = _tmp2 * _tmp9;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + p_init0;
  const Scalar _tmp19 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp3;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp23 = _tmp0 * _tmp7;
  const Scalar _tmp24 = _tmp6 * _tmp9;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + p_init1;
  const Scalar _tmp30 = Scalar(1.0) / (fh1);
  const Scalar _tmp31 = _tmp30 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -_tmp33 * fh1 -
      Scalar(8.3196563700000006) *
          std::sqrt(
              Scalar(Scalar(0.057067943376852184) *
                         std::pow(Scalar(-Scalar(0.50315118556004401) * _tmp18 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727204189803) * _tmp29 - 1), Scalar(2))));
  const Scalar _tmp35 = Scalar(0.1034955) * _tmp30;
  const Scalar _tmp36 = _tmp34 * _tmp35;
  const Scalar _tmp37 = Scalar(1.0) * _tmp32;
  const Scalar _tmp38 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp39 =
      std::pow(Scalar(_tmp38 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp40 = _tmp12 + _tmp15;
  const Scalar _tmp41 = _tmp40 + _tmp5;
  const Scalar _tmp42 = Scalar(1.0) * _tmp41;
  const Scalar _tmp43 = _tmp4 + _tmp40;
  const Scalar _tmp44 = _tmp20 + _tmp27;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 = _tmp22 + _tmp26;
  const Scalar _tmp48 = _tmp20 + _tmp47;
  const Scalar _tmp49 = _tmp46 + _tmp48;
  const Scalar _tmp50 = _tmp21 + _tmp47;
  const Scalar _tmp51 = Scalar(1.0) / (_tmp46 + _tmp50);
  const Scalar _tmp52 = _tmp16 + _tmp4;
  const Scalar _tmp53 = _tmp51 * (_tmp42 - _tmp52);
  const Scalar _tmp54 = _tmp49 * _tmp53;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp42 - _tmp43 - _tmp54);
  const Scalar _tmp56 = Scalar(1.0) * _tmp55;
  const Scalar _tmp57 = _tmp43 + p_init0;
  const Scalar _tmp58 = _tmp57 + Scalar(-2.71799795);
  const Scalar _tmp59 = _tmp48 + p_init1;
  const Scalar _tmp60 = _tmp59 + Scalar(-4.7752063900000001);
  const Scalar _tmp61 = std::pow(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp60, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp62 = _tmp58 * _tmp61;
  const Scalar _tmp63 = _tmp44 + p_init1;
  const Scalar _tmp64 = _tmp63 + Scalar(-4.8333311099999996);
  const Scalar _tmp65 = _tmp41 + p_init0;
  const Scalar _tmp66 = _tmp65 + Scalar(1.79662371);
  const Scalar _tmp67 =
      std::sqrt(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp66, Scalar(2))));
  const Scalar _tmp68 = Scalar(1.0) / (_tmp67);
  const Scalar _tmp69 = Scalar(1.0) / (_tmp66);
  const Scalar _tmp70 = _tmp67 * _tmp69;
  const Scalar _tmp71 = _tmp70 * (_tmp41 * _tmp64 * _tmp68 - _tmp44 * _tmp66 * _tmp68);
  const Scalar _tmp72 = _tmp60 * _tmp61;
  const Scalar _tmp73 = _tmp52 + p_init0;
  const Scalar _tmp74 = _tmp73 + Scalar(-2.5202214700000001);
  const Scalar _tmp75 = _tmp50 + p_init1;
  const Scalar _tmp76 = _tmp75 + Scalar(8.3888750099999996);
  const Scalar _tmp77 = std::pow(Scalar(std::pow(_tmp74, Scalar(2)) + std::pow(_tmp76, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp78 = _tmp74 * _tmp77;
  const Scalar _tmp79 = _tmp76 * _tmp77;
  const Scalar _tmp80 = _tmp50 * _tmp78 - _tmp52 * _tmp79 + _tmp71 * _tmp78;
  const Scalar _tmp81 = _tmp64 * _tmp69;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp78 * _tmp81 - _tmp79);
  const Scalar _tmp83 = _tmp62 * _tmp81 - _tmp72;
  const Scalar _tmp84 = _tmp82 * _tmp83;
  const Scalar _tmp85 = -_tmp43 * _tmp72 + _tmp48 * _tmp62 + _tmp62 * _tmp71 - _tmp80 * _tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp85);
  const Scalar _tmp87 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp88 =
      -Scalar(0.010999999999999999) * _tmp1 - Scalar(0.010999999999999999) * _tmp19;
  const Scalar _tmp89 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp90 = _tmp88 + _tmp89;
  const Scalar _tmp91 = _tmp87 + _tmp90;
  const Scalar _tmp92 = -_tmp87;
  const Scalar _tmp93 = _tmp90 + _tmp92;
  const Scalar _tmp94 = _tmp88 - _tmp89;
  const Scalar _tmp95 = _tmp87 + _tmp94;
  const Scalar _tmp96 = _tmp81 * _tmp93;
  const Scalar _tmp97 = -_tmp78 * _tmp96 + _tmp79 * _tmp95;
  const Scalar _tmp98 = -_tmp62 * _tmp96 + _tmp72 * _tmp91 - _tmp84 * _tmp97;
  const Scalar _tmp99 = _tmp78 * _tmp93 - _tmp78 * _tmp95;
  const Scalar _tmp100 = -_tmp53 * _tmp98 - _tmp62 * _tmp91 + _tmp62 * _tmp93 - _tmp84 * _tmp99;
  const Scalar _tmp101 = _tmp100 * _tmp86;
  const Scalar _tmp102 = _tmp101 * _tmp56;
  const Scalar _tmp103 = Scalar(1.0) * _tmp86;
  const Scalar _tmp104 = _tmp49 * _tmp56;
  const Scalar _tmp105 = _tmp51 * (_tmp101 * _tmp104 - _tmp103 * _tmp98);
  const Scalar _tmp106 = _tmp29 + Scalar(8.3196563700000006);
  const Scalar _tmp107 = _tmp18 + Scalar(1.9874742000000001);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp107 * _tmp108;
  const Scalar _tmp111 = _tmp109 * _tmp17 - _tmp110 * _tmp28;
  const Scalar _tmp112 = Scalar(1.0) * _tmp111;
  const Scalar _tmp113 = _tmp112 * (_tmp102 - Scalar(1.0) * _tmp105);
  const Scalar _tmp114 = _tmp81 * _tmp82;
  const Scalar _tmp115 = _tmp114 * _tmp97 + _tmp96;
  const Scalar _tmp116 = _tmp114 * _tmp99 - _tmp115 * _tmp53 - _tmp93;
  const Scalar _tmp117 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp118 = _tmp117 * _tmp85;
  const Scalar _tmp119 = _tmp101 * (_tmp114 * _tmp80 - _tmp116 * _tmp118 - _tmp71);
  const Scalar _tmp120 = _tmp49 * _tmp55;
  const Scalar _tmp121 = _tmp116 + _tmp119;
  const Scalar _tmp122 = _tmp117 * _tmp98;
  const Scalar _tmp123 = _tmp115 + _tmp119 * _tmp120 - _tmp121 * _tmp122;
  const Scalar _tmp124 = Scalar(1.0) * _tmp51;
  const Scalar _tmp125 = Scalar(1.0) * _tmp110 * (_tmp119 * _tmp56 - _tmp123 * _tmp124);
  const Scalar _tmp126 = Scalar(1.0) * _tmp82;
  const Scalar _tmp127 = _tmp126 * _tmp97;
  const Scalar _tmp128 = -_tmp126 * _tmp99 + _tmp127 * _tmp53;
  const Scalar _tmp129 = _tmp101 * (-_tmp118 * _tmp128 - _tmp126 * _tmp80);
  const Scalar _tmp130 = _tmp128 + _tmp129;
  const Scalar _tmp131 = _tmp120 * _tmp129 - _tmp122 * _tmp130 - _tmp127;
  const Scalar _tmp132 = Scalar(1.0) * _tmp109 * (-_tmp124 * _tmp131 + _tmp129 * _tmp56);
  const Scalar _tmp133 = _tmp92 + _tmp94;
  const Scalar _tmp134 = _tmp133 * fh1;
  const Scalar _tmp135 = Scalar(3.29616) * _tmp11 + _tmp110 * _tmp134 + _tmp17 * fv1;
  const Scalar _tmp136 = _tmp104 * _tmp51;
  const Scalar _tmp137 = Scalar(1.0) * _tmp136 - Scalar(1.0) * _tmp56;
  const Scalar _tmp138 = -_tmp109 * _tmp134 - Scalar(3.29616) * _tmp25 - _tmp28 * fv1;
  const Scalar _tmp139 = _tmp51 * (_tmp54 * _tmp56 + Scalar(1.0));
  const Scalar _tmp140 = _tmp53 * _tmp56;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp139 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp143 = _tmp42 + _tmp45 * _tmp53;
  const Scalar _tmp144 = _tmp143 * _tmp55;
  const Scalar _tmp145 = 0;
  const Scalar _tmp146 = _tmp51 * (-_tmp122 * _tmp145 - _tmp144 * _tmp49 + _tmp46);
  const Scalar _tmp147 =
      _tmp113 * fh1 + _tmp125 * fh1 + _tmp132 * fh1 + _tmp135 * _tmp137 + _tmp138 * _tmp141 +
      Scalar(1.0) * _tmp142 * (-_tmp143 * _tmp56 - Scalar(1.0) * _tmp146 + Scalar(1.0));
  const Scalar _tmp148 = _tmp117 * _tmp83;
  const Scalar _tmp149 = _tmp82 * (-_tmp121 * _tmp148 - _tmp81);
  const Scalar _tmp150 = _tmp117 * _tmp121;
  const Scalar _tmp151 = _tmp110 * _tmp70 * (_tmp149 * _tmp78 + _tmp150 * _tmp62 + Scalar(1.0));
  const Scalar _tmp152 = _tmp117 * _tmp145;
  const Scalar _tmp153 = _tmp145 * _tmp148 * _tmp82;
  const Scalar _tmp154 = _tmp126 * _tmp83 * _tmp86;
  const Scalar _tmp155 = _tmp111 * _tmp70 * (_tmp103 * _tmp62 - _tmp154 * _tmp78);
  const Scalar _tmp156 = _tmp117 * _tmp130;
  const Scalar _tmp157 = _tmp82 * (-_tmp130 * _tmp148 + Scalar(1.0));
  const Scalar _tmp158 = _tmp109 * _tmp70 * (_tmp156 * _tmp62 + _tmp157 * _tmp78);
  const Scalar _tmp159 = -_tmp142 * _tmp70 * (_tmp152 * _tmp62 - _tmp153 * _tmp78) - _tmp151 * fh1 -
                         _tmp155 * fh1 - _tmp158 * fh1;
  const Scalar _tmp160 = Scalar(1.0) / (_tmp159);
  const Scalar _tmp161 = std::asinh(_tmp147 * _tmp160);
  const Scalar _tmp162 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp163 =
      -_tmp161 * _tmp162 -
      Scalar(4.8333311099999996) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20689664689659551) * _tmp63), Scalar(2)) +
                     Scalar(0.13817235445745474) *
                         std::pow(Scalar(-Scalar(0.55659957866191134) * _tmp65 - 1), Scalar(2))));
  const Scalar _tmp164 = Scalar(0.1034955) * _tmp160;
  const Scalar _tmp165 = _tmp163 * _tmp164;
  const Scalar _tmp166 = Scalar(1.0) * _tmp161;
  const Scalar _tmp167 = -_tmp151 - _tmp155 - _tmp158;
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp167;
  const Scalar _tmp169 = std::pow(_tmp159, Scalar(-2));
  const Scalar _tmp170 = _tmp110 * _tmp133;
  const Scalar _tmp171 = _tmp109 * _tmp133;
  const Scalar _tmp172 = _tmp167 * _tmp169;
  const Scalar _tmp173 = (-_tmp147 * _tmp172 + _tmp160 * (_tmp113 + _tmp125 + _tmp132 +
                                                          _tmp137 * _tmp170 - _tmp141 * _tmp171)) /
                         std::sqrt(Scalar(std::pow(_tmp147, Scalar(2)) * _tmp169 + 1));
  const Scalar _tmp174 = _tmp111 * _tmp154;
  const Scalar _tmp175 = _tmp109 * _tmp157;
  const Scalar _tmp176 = _tmp110 * _tmp149;
  const Scalar _tmp177 = -_tmp142 * _tmp153 - _tmp174 * fh1 + _tmp175 * fh1 + _tmp176 * fh1;
  const Scalar _tmp178 = Scalar(1.0) / (_tmp177);
  const Scalar _tmp179 = _tmp105 * _tmp111;
  const Scalar _tmp180 = _tmp109 * _tmp131 * _tmp51;
  const Scalar _tmp181 = _tmp135 * _tmp56;
  const Scalar _tmp182 = _tmp110 * _tmp123 * _tmp51;
  const Scalar _tmp183 = _tmp138 * _tmp139 + _tmp142 * _tmp146 + _tmp179 * fh1 + _tmp180 * fh1 -
                         _tmp181 * _tmp49 * _tmp51 + _tmp182 * fh1;
  const Scalar _tmp184 = std::asinh(_tmp178 * _tmp183);
  const Scalar _tmp185 = Scalar(1.0) * _tmp184;
  const Scalar _tmp186 = std::pow(_tmp177, Scalar(-2));
  const Scalar _tmp187 = -_tmp174 + _tmp175 + _tmp176;
  const Scalar _tmp188 = _tmp186 * _tmp187;
  const Scalar _tmp189 =
      (_tmp178 * (-_tmp136 * _tmp170 - _tmp139 * _tmp171 + _tmp179 + _tmp180 + _tmp182) -
       _tmp183 * _tmp188) /
      std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp186 + 1));
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp177;
  const Scalar _tmp191 =
      -_tmp184 * _tmp190 -
      Scalar(8.3888750099999996) *
          std::sqrt(
              Scalar(Scalar(0.090254729040973036) *
                         std::pow(Scalar(1 - Scalar(0.39679052492160538) * _tmp73), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11920549523123722) * _tmp75 - 1), Scalar(2))));
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp178;
  const Scalar _tmp193 = _tmp191 * _tmp192;
  const Scalar _tmp194 = Scalar(9.6622558468725703) * _tmp187;
  const Scalar _tmp195 = _tmp110 * _tmp150;
  const Scalar _tmp196 = _tmp112 * _tmp86;
  const Scalar _tmp197 = _tmp109 * _tmp156;
  const Scalar _tmp198 = _tmp142 * _tmp152 + _tmp195 * fh1 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp199 = Scalar(1.0) / (_tmp198);
  const Scalar _tmp200 = _tmp110 * _tmp119 * _tmp55;
  const Scalar _tmp201 = _tmp102 * _tmp111;
  const Scalar _tmp202 = _tmp109 * _tmp129 * _tmp55;
  const Scalar _tmp203 = -_tmp138 * _tmp140 + _tmp142 * _tmp144 + _tmp181 - _tmp200 * fh1 -
                         _tmp201 * fh1 - _tmp202 * fh1;
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(9.6622558468725703) * _tmp204;
  const Scalar _tmp206 =
      -_tmp198 * _tmp205 -
      Scalar(4.7752063900000001) *
          std::sqrt(
              Scalar(Scalar(0.32397683292140877) *
                         std::pow(Scalar(1 - Scalar(0.36791786395571047) * _tmp57), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.20941503221602112) * _tmp59), Scalar(2))));
  const Scalar _tmp207 = Scalar(0.1034955) * _tmp199;
  const Scalar _tmp208 = _tmp206 * _tmp207;
  const Scalar _tmp209 = Scalar(1.0) * _tmp204;
  const Scalar _tmp210 = _tmp195 + _tmp196 + _tmp197;
  const Scalar _tmp211 = std::pow(_tmp198, Scalar(-2));
  const Scalar _tmp212 = _tmp210 * _tmp211;
  const Scalar _tmp213 =
      (_tmp199 * (_tmp140 * _tmp171 + _tmp170 * _tmp56 - _tmp200 - _tmp201 - _tmp202) -
       _tmp203 * _tmp212) /
      std::sqrt(Scalar(std::pow(_tmp203, Scalar(2)) * _tmp211 + 1));
  const Scalar _tmp214 = Scalar(9.6622558468725703) * _tmp198;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp38 * _tmp39 * fv1 * std::cosh(_tmp37) -
                    (-Scalar(0.1034955) * _tmp34 * _tmp38 +
                     _tmp35 * (Scalar(9.6622558468725703) * _tmp31 * _tmp39 - _tmp33)) *
                        std::cosh(_tmp36)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp36) -
               Scalar(9.6622558468725703) * std::sinh(_tmp37);
  _res(1, 0) = _tmp162 * (-Scalar(1.0) * _tmp173 * std::cosh(_tmp166) -
                          (-Scalar(0.1034955) * _tmp163 * _tmp172 +
                           _tmp164 * (-_tmp161 * _tmp168 - _tmp162 * _tmp173)) *
                              std::cosh(_tmp165)) +
               _tmp168 * (-std::sinh(_tmp165) - std::sinh(_tmp166));
  _res(2, 0) = _tmp190 * (-Scalar(1.0) * _tmp189 * std::cosh(_tmp185) -
                          (-Scalar(0.1034955) * _tmp188 * _tmp191 +
                           _tmp192 * (-_tmp184 * _tmp194 - _tmp189 * _tmp190)) *
                              std::cosh(_tmp193)) +
               _tmp194 * (-std::sinh(_tmp185) - std::sinh(_tmp193));
  _res(3, 0) = Scalar(9.6622558468725703) * _tmp210 * (-std::sinh(_tmp208) - std::sinh(_tmp209)) +
               _tmp214 * (-Scalar(1.0) * _tmp213 * std::cosh(_tmp209) -
                          (-Scalar(0.1034955) * _tmp206 * _tmp212 +
                           _tmp207 * (-_tmp205 * _tmp210 - _tmp213 * _tmp214)) *
                              std::cosh(_tmp208));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
